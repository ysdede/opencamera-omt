...
public abstract class CameraController {
    ...
    public static final String SCENE_MODE_DEFAULT = "auto"; // chosen to match Camera.Parameters.SCENE_MODE_AUTO, but we
                                                            // also use compatible values for Camera2 API
    public static final String COLOR_EFFECT_DEFAULT = "none"; // chosen to match Camera.Parameters.EFFECT_NONE, but we
                                                              // also use compatible values for Camera2 API
    public static final String WHITE_BALANCE_DEFAULT = "auto"; // chosen to match Camera.Parameters.WHITE_BALANCE_AUTO,
                                                               // but we also use compatible values for Camera2 API
    public static final String ANTIBANDING_DEFAULT = "auto"; // chosen to match Camera.Parameters.ANTIBANDING_AUTO, but
                                                             // we also use compatible values for Camera2 API
    ...
    public static final long EXPOSURE_TIME_DEFAULT = 1000000000L / 30; // note, responsibility of callers to check that
                                                                       // this is within the valid min/max range
    ...
    public volatile boolean test_release_during_photo; // for Camera2 API, will force takePictureAfterPrecapture() to
                                                       // call release() on UI thread
    public volatile int test_capture_results; // for Camera2 API, how many capture requests completed with
                                              // RequestTagType.CAPTURE
    public volatile int test_fake_flash_focus; // for Camera2 API, records torch turning on for fake flash during
                                               // autofocus
    public volatile int test_fake_flash_precapture; // for Camera2 API, records torch turning on for fake flash during
                                                    // precapture
    public volatile int test_fake_flash_photo; // for Camera2 API, records torch turning on for fake flash for photo
                                               // capture
    public volatile int test_af_state_null_focus; // for Camera2 API, records af_state being null even when we've
                                                  // requested autofocus
    ...
    public volatile boolean test_force_run_post_capture; // for Camera2 API, test using adjustPreview() /
                                                         // RequestTagType.RUN_POST_CAPTURE
    public static volatile boolean test_force_slow_preview_start; // for Camera2 API, test waiting for 6s when starting
                                                                  // preview
    ...
    public static class CameraFeaturesCache {
        ...
        CameraFeaturesCache(CameraFeatures camera_features,
                Map<Integer, List<android.util.Size>> extension_picture_sizes_map,
                Map<Integer, List<android.util.Size>> extension_preview_sizes_map) {
    ...
    public static class CameraFeatures {
        public Set<String> physical_camera_ids; // if non-null, this camera is part of a logical camera that exposes
                                                // these physical camera IDs
        ...
        public List<Integer> supported_extensions_zoom; // if non-null, list of camera vendor extensions that support
                                                        // zoom
        ...
        /**
         * @param return_closest If true, return a match for the width/height, even if
         *                       the fps doesn't
         *                       match.
         */
    ...
    public static class Size {
        ...
        public List<Integer> supported_extensions; // for photo and preview: if non-null, list of supported camera
                                                   // vendor extensions
    ...
    /**
     * An area has values from [-1000,-1000] (for top-left) to [1000,1000] (for
     * bottom-right) for whatever is
    ...
    /**
     * Interface to define callbacks related to taking photos. These callbacks are
     * all called on the UI thread.
     */
    public interface PictureCallback {
        void onStarted(); // called immediately before we start capturing the picture

        void onCompleted(); // called after all relevant on*PictureTaken() callbacks have been called and
                            // returned
        ...
        /**
         * Reports percentage progress for vendor camera extensions. Note that not all
         * devices support this being called.
         */
        void onExtensionProgress(int progress);

        /*
         * This is called for when burst mode is BURSTTYPE_FOCUS or
         * BURSTTYPE_CONTINUOUS, to ask whether it's safe to take
        ...
        /*
         * This is called for flash_frontscreen_auto or flash_frontscreen_on mode to
         * indicate the caller should light up the screen
         * (for flash_frontscreen_auto it will only be called if the scene is considered
         * dark enough to require the screen flash).
    ...
    /**
     * Interface to define callback for autofocus completing. This callback may be
     * called on the UI thread (CameraController1)
    ...
    /**
     * Interface to define callback for continuous focus starting/stopping. This
     * callback may be called on the
    ...
    public static class Face {
        ...
        /*
         * The rect has values from [-1000,-1000] (for top-left) to [1000,1000] (for
         * bottom-right) for whatever is
         * the current field of view (i.e., taking zoom into account).
         */
    ...
    public abstract void onError(); // triggers error mechanism - should only be called externally for testing
                                    // purposes
    ...
    /**
     * For CameraController2 only. Applications should cover the preview textureview
     * if since last resuming, camera_controller
     * has never been non-null or this method has never returned false.
     * Otherwise there is a risk when opening the camera that the textureview still
     * shows an image from when
     * the camera was previously opened (e.g., from pausing and resuming the
     * application). This returns false (for CameraController2)
     * when the camera has received its first frame.
     * Update: on more recent Android versions this didn't work very well, possibly
     * due to a screenshot being used for "recent apps"
     * view; on Android 13+, the activity can make use of shouldCoverPreview(false)
     * for this.
     */
    ...
    /**
     * For CameraController2 only. After calling this, shouldCoverPreview() will
     * return true, until a new
    ...
    /**
     * @return Returns true iff changing the scene mode can affect the available
     *         camera functionality
     *         (e.g., changing to Night scene mode might mean flash modes are no
     *         longer available).
     */
    ...
    /**
     * Switch between auto and manual ISO mode. Only supported if supports_iso_range
     * is true.
     * 
     * @param manual_iso Whether to switch to manual mode or back to auto.
     * @param iso        If manual_iso is true, this specifies the desired ISO
     *                   value. If this is outside
     *                   the min_iso/max_iso, the value will be snapped so it does
    ...
    /**
     * Specify a specific ISO value. Only supported if supports_iso_range is true.
     * Callers should
    ...
    public enum BurstType {
        ...
        BURSTTYPE_CONTINUOUS // as BURSTTYPE_NORMAL, but bursts will fire continually until
                             // stopContinuousBurst() is called.
    }
    ...
    /**
     * Only relevant if setBurstType() is also called with BURSTTYPE_NORMAL. Sets
     * the number of
    ...
    /**
     * Only relevant if setBurstType() is also called with BURSTTYPE_NORMAL. If this
     * method is
     * called with burst_for_noise_reduction, then the number of burst images, and
     * other settings,
    ...
    public abstract void setBurstForNoiseReduction(boolean burst_for_noise_reduction,
            boolean noise_reduction_low_light);
    ...
    /**
     * Only relevant if setBurstType() is also called with BURSTTYPE_EXPO. Sets the
     * number of
    ...
    /**
     * Whether the current BurstType is one that requires the camera driver to
     * capture the images
     * as a burst at a fast rate. If true, we should not use high resolutions that
     * don't support a
     * capture burst (for Camera2 API, see
    ...
    /**
     * If isCapturingBurst() is true, then this returns the number of images in the
     * current burst
     * captured so far.
     */
    ...
    /**
     * If isCapturingBurst() is true, then this returns the total number of images
     * in the current
    ...
    /**
     * @param want_jpeg_r Whether to enable taking photos in JPEG_R (Ultra HDR)
     *                    format.
     */
     ...
     * @param max_raw_images The maximum number of unclosed DNG images that may be
     *                       held in memory at any one
     *                       time. Trying to take a photo, when the number of
     *                       unclosed DNG images is already
     *                       equal to this number, will result in an exception
     *                       (java.lang.IllegalStateException
     *                       - note, the exception will come from a
     *                       CameraController2 callback, so can't be
    ...
    /**
     * setUseCamera2FakeFlash() should be called after creating the
     * CameraController, and before calling getCameraFeatures() or
     * starting the preview (as it changes the available flash modes).
     * "Fake flash" is an alternative mode for handling flash, for devices that have
     * poor Camera2 support - typical symptoms
     * include precapture never starting, flash not firing, photos being over or
     * under exposed.
     * Instead, we fake the precapture and flash simply by turning on the torch.
     * After turning on torch, we wait for ae to stop
     * scanning (and af too, as it can start scanning in continuous mode) - this is
     * effectively the equivalent of precapture -
     * before taking the photo.
     * In auto-focus mode, we make the decision ourselves based on the current ISO.
     * We also handle the flash firing for autofocus by turning the torch on and off
     * too. Advantages are:
     * - The flash tends to be brighter, and the photo can end up overexposed as a
     * result if capture follows the autofocus.
     * - Some devices also don't seem to fire flash for autofocus in Camera2 mode
     * (e.g., Samsung S7)
     * - When capture follows autofocus, we need to make the same decision for
    ...
    /**
     * Whether to enable digital video stabilization. Should only be set to true
     * when intending to
     * capture video.
     */
     ...
     * @param log_profile_strength Only relevant if tonemap_profile set to
     *                             TONEMAPPROFILE_LOG.
     * @param gamma                Only relevant if tonemap_profile set to
     *                             TONEMAPPROFILE_GAMMA
     */
    ...
    /**
     * Returns the current zoom. The returned value is an index into the
     * CameraFeatures.zoom_ratios
     * array.
     */
    ...
    /**
     * Set the zoom. Unlike setZoom(value), this allows specifying any zoom level
     * within the
     * supported range.
     * 
     * @param value       The index into the CameraFeatures.zoom_ratios array.
     * @param smooth_zoom The desired zoom. With CameraController1 (old Camera API),
     *                    this is ignored.
     *                    With CameraController2 (Camera2 API), this is used instead
     *                    of the zoom_ratios
     *                    value. Note that getZoom() will return the value passed to
     *                    this method, so
     *                    passing an appropriate value (e.g., whatever zoom_ratio is
     *                    closest to the
     *                    smooth_zoom) is still useful if you want to make use of
     *                    getZoom().
     *                    smooth_zoom must still be within the supported range of
     *                    zoom values.
     */
    ...
    /**
     * Only relevant if setBurstType() is also called with BURSTTYPE_FOCUS. Sets the
     * number of
    ...
    /**
     * Only relevant if setBurstType() is also called with BURSTTYPE_FOCUS. If set
     * to true, an
     ...
     * Only relevant if setBurstType() is also called with BURSTTYPE_FOCUS. Sets the
     * source focus
     * distance to match the camera's current focus distance (typically useful if
     * running in a
    ...
    /**
     * This should be called when using a TextureView, and the texture view has
     * reported a change
     ...
     * Starts the camera preview.
     * 
     * @param wait_until_started Whether to wait until the preview is started. Only
     *                           relevant for
     *                           CameraController2; CameraController1 will always
     *                           wait.
     * @param runnable           If non-null, a runnable to be called once preview
     *                           is started. If
     *                           wait_until_started==true, or using
     *                           CameraController1, this will be
     *                           called on the current thread, before this method
     *                           exits. Otherwise,
     *                           this will be called on the UI thread, after this
     *                           method exits (once
     *                           the preview has started).
     * @param on_failed          If non-null, a runnable to be called if the preview
     *                           fails to start.
     *                           Only relevant for wait_until_started==false and
     *                           when using
     *                           CameraController2. In such cases, failing to start
     *                           the camera preview
     *                           may result in either CameraControllerException
     *                           being thrown, or
     *                           on_failed being called on the UI thread after this
     *                           method exits
     *                           (depending on when the failure occurs). If either
     *                           of these happens,
     *                           the "runnable" runnable will not be called.
     * @throws CameraControllerException Failed to start preview. In this case, the
     *                                   runnable will not
     *                                   be called.
     */
    public abstract void startPreview(boolean wait_until_started, Runnable runnable, Runnable on_failed)
            throws CameraControllerException;

    /**
     * Only relevant for CameraController2: stops the repeating burst for the
     * previous (so effectively
     * stops the preview), but does not close the capture session for the preview
     * (for that, using
    ...
    /**
     * @param cb                             Callback to be called when autofocus
     *                                       completes.
     * @param capture_follows_autofocus_hint Set to true if you intend to take a
     *                                       photo immediately after autofocus. If
     *                                       the
     *                                       decision changes after autofocus has
     *                                       started (e.g., user initiates
     *                                       autofocus,
     *                                       then takes photo before autofocus has
     *                                       completed), use
    ...
    /**
     * See autoFocus() for details - used to update the
     * capture_follows_autofocus_hint setting.
     */
    ...
    /**
     * Call to initialise video recording, should call before
    ...
    /**
     * Call to initialise video recording, should call after
     * MediaRecorder.prepare(), but before MediaRecorder.start().
     * 
     * @param video_recorder             The media recorder object.
     * @param want_photo_video_recording Whether support for taking photos whilst
     *                                   video recording is required. If this
     *                                   feature isn't supported, the option has no
     *                                   effect.
     */
    public abstract void initVideoRecorderPostPrepare(MediaRecorder video_recorder, boolean want_photo_video_recording)
            throws CameraControllerException;
    ...
    // gets the available values of a generic mode, e.g., scene, color etc, and
    // makes sure the requested mode is available
    SupportedValues checkModeIsSupported(List<String> values, String value, String default_value) {
        if (values != null && values.size() > 1) { // n.b., if there is only 1 supported value, we also return null, as
                                                   // no point offering the choice to the user (there are some devices,
                                                   // e.g., Samsung, that only have a scene mode of "auto")
            if (MyDebug.LOG) {
