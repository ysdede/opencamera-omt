...
public class CameraController2 extends CameraController {
    ...
    private final Map<String, CameraFeaturesCache> camera_features_caches; // used to improve performance for subsequent
                                                                           // CameraController2 objects; key is the
                                                                           // cameraIdS string, value is a
                                                                           // CameraFeaturesCache object
    ...
    // characteristics of camera - if a specific physical camera is being used,
    // these are characteristics for the physical camera
    private CameraCharacteristics characteristics;
    private CameraExtensionCharacteristics extension_characteristics;
    private CameraFeaturesCache camera_features_cache; // if non-null, this is the cache obtained from
                                                       // camera_features_caches
    // cached characteristics (use this for values that need to be frequently
    // accessed, e.g., per frame, to improve performance);
    private int characteristics_sensor_orientation;
    private Facing characteristics_facing;

    // camera features that we save (either to avoid repeatedly accessing, or we do
    ...
    private String initial_focus_mode; // if non-null, focus mode to use if not set by Preview (rather than relying on
                                       // the Builder template's default, which can be one that isn't supported, at
                                       // least on Android emulator with its LIMITED camera!)
    ...
    private static final long autofocus_timeout_c = 1000; // timeout for calling autofocus_cb (applies for both auto and
                                                          // continuous focus)
    ...
    private final Object open_camera_lock = new Object(); // lock to wait for camera to be opened from
                                                          // CameraDevice.StateCallback
    private final Object background_camera_lock = new Object(); // lock to synchronize between UI thread and the
                                                                // background "CameraBackground" thread/handler
    ...
    private boolean burst_for_noise_reduction; // chooses number of burst images and other settings for Open Camera's
                                               // noise reduction (NR) photo mode
    private boolean noise_reduction_low_light; // if burst_for_noise_reduction==true, whether to optimise for low light
                                               // scenes
    private int burst_requested_n_images; // if burst_for_noise_reduction==false, this gives the number of images for
                                          // the burst
    ...
    // Whether to enable a workaround hack for some Galaxy devices - take an
    // additional dummy photo
    // when taking an expo/HDR burst, to avoid problem where manual exposure is
    // ignored for the
    // first image.
    ...
    private int n_burst_total; // total number of expected burst images in this capture (if known) (same for
                               // JPEG and RAW)
    private int n_burst_raw; // number of expected (remaining) burst RAW images in this capture
    private boolean burst_single_request; // if true then the burst images are returned in a single call to
                                          // onBurstPictureTaken(), if false, then multiple calls to onPictureTaken()
                                          // are made as soon as the image is available
    private final List<byte[]> pending_burst_images = new ArrayList<>(); // burst images that have been captured so far,
                                                                         // but not yet sent to the application
    private final List<RawImage> pending_burst_images_raw = new ArrayList<>();
    private List<CaptureRequest> slow_burst_capture_requests; // the set of burst capture requests - used when not using
                                                              // captureBurst() (e.g., when use_expo_fast_burst==false,
                                                              // or for focus bracketing)
    private long slow_burst_start_ms = 0; // time when burst started (used for measuring performance of captures when
                                          // not using captureBurst())
    private RawImage pending_raw_image; // used to ensure that when taking JPEG+RAW, the JPEG picture callback is called
                                        // first (only used for non-burst cases)
    ...
    private Surface omt_streaming_surface;

    public void setOmtStreamingSurface(Surface surface) {
    ...
    private boolean use_fake_precapture; // see CameraController.setUseCamera2FakeFlash() for details - this is the
                                         // user/application setting, see use_fake_precapture_mode for whether fake
                                         // precapture is enabled (as we may do this for other purposes, e.g., front
                                         // screen flash)
    private boolean use_fake_precapture_mode; // true if either use_fake_precapture is true, or we're temporarily using
                                              // fake precapture mode (e.g., for front screen flash or exposure
                                              // bracketing)
    private boolean fake_precapture_torch_performed; // whether we turned on torch to do a fake precapture
    private boolean fake_precapture_torch_focus_performed; // whether we turned on torch to do an autofocus, in fake
                                                           // precapture mode
    ...
    private boolean is_flash_required; // whether capture_result_ae suggests FLASH_REQUIRED? Or in neither
                                       // FLASH_REQUIRED nor CONVERGED, this stores the last known result
    private boolean modified_from_camera_settings;
    // if modified_from_camera_settings set to true, then we've temporarily
    // requested captures with settings such as
     ...
     */
    /**
     * Even if using long exposure, we want to set a maximum for the preview to
     * avoid very low
     * frame rates.
     * Originally this was 1/12s, but I think we can get away with 1/5s - for this
     * range, having
     * a WYSIWYG preview is probably still better than the reduced framerate. Also
     * as a side-benefit,
     * it reduces the impact of the Samsung Galaxy Android 11 bug where manual
     * exposure is ignored if
    ...
    private enum RequestTagType {
        ...
        CAPTURE, // request is either for a regular non-burst capture, or the last of a burst
                 // capture sequence
        CAPTURE_BURST_IN_PROGRESS // request is for a burst capture, but isn't the last of the burst capture
                                  // sequence
        // NONE // should be treated the same as if no tag had been set on the request -
        // but allows the request tag type to be changed later
    }

    /*
     * The class that we use for setTag() and getTag() for capture requests.
     * We use this class instead of assigning the RequestTagType directly, so we can
     * modify it
    ...
    private class CameraSettings {
        // keys that we need to store, to pass to the stillBuilder, but doesn't need to
        // be passed to previewBuilder (should set sensible defaults)
        private int rotation;
        private Location location;
        private byte jpeg_quality = 90;

        // keys that we have passed to the previewBuilder, that we need to store to also
        // pass to the stillBuilder (should set sensible defaults, or use a has_ boolean
        // if we don't want to set a default)
        ...
        private Rect scalar_crop_region; // zoom for older Android versions; no need for has_scalar_crop_region, as we
                                         // can set to null instead
        ...
        private float focus_distance_manual; // saved setting when in manual mode (so if user switches to infinity mode
                                             // and back, we'll still remember the manual focus distance)
        ...
        private Integer default_tonemap_mode; // since we don't know what a device's tonemap mode is, we save it so we
        ...
        private int getExifOrientation() {
            ...
            switch ((rotation + 360) % 360) {
                case 0:
                    ...
                    break;
                case 90:
                    exif_orientation = (getFacing() == Facing.FACING_FRONT) ? ExifInterface.ORIENTATION_ROTATE_270
                ...
                case 180:
                    ...
                    break;
                case 270:
                    exif_orientation = (getFacing() == Facing.FACING_FRONT) ? ExifInterface.ORIENTATION_ROTATE_90
        ...
        private void setupBuilder(CaptureRequest.Builder builder, boolean is_still) {
            ...
            if (is_still) {
                if (location != null && sessionType != SessionType.SESSIONTYPE_EXTENSION) {
                    ...
                    // be set by the caller when receiving the image data (see
                    // ImageSaver.modifyExif(),
             ...
             * builder.set(CaptureRequest.TONEMAP_PRESET_CURVE,
            ...
            if (MyDebug.LOG) {
                if (is_still) {
                    ...
                    /*
                     * if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.N ) {
                     * Integer raw_sensitivity_boost =
                     * builder.get(CaptureRequest.CONTROL_POST_RAW_SENSITIVITY_BOOST);
                     * Log.d(TAG, "raw_sensitivity_boost: " + (raw_sensitivity_boost==null ? "null"
                     * : raw_sensitivity_boost));
                     * }
                     */
                }
                // Integer ois_mode =
        ...
        private boolean setSceneMode(CaptureRequest.Builder builder) {
            ...
            if (has_face_detect_mode) {
                ...
                if (current_mode == null || current_mode != CameraMetadata.CONTROL_MODE_USE_SCENE_MODE
                        || current_scene_mode == null
                        || current_scene_mode != CameraMetadata.CONTROL_SCENE_MODE_FACE_PRIORITY)
                    return true;
            } else {
                if (MyDebug.LOG)
                ...
                if (scene_mode == CameraMetadata.CONTROL_SCENE_MODE_DISABLED) {
                    // note we set CONTROL_MODE_AUTO even if using manual exposure, focus or awb, as
                    // we set that separately via
                ...
                if (current_mode == null || current_mode != new_mode || current_scene_mode == null
                        || current_scene_mode != scene_mode)
                    return true;
            }
            return false;
        }
        ...
        private boolean setColorEffect(CaptureRequest.Builder builder) {
             ...
             * color_effect == CameraMetadata.CONTROL_EFFECT_MODE_OFF ) {
             * // can leave off
             * }
             */
            else if (builder.get(CaptureRequest.CONTROL_EFFECT_MODE) == null
        ...
        private boolean setWhiteBalance(CaptureRequest.Builder builder) {
             ...
             * white_balance == CameraMetadata.CONTROL_AWB_MODE_AUTO ) {
             * // can leave off
             * }
             */
            else if (builder.get(CaptureRequest.CONTROL_AWB_MODE) == null
                ...
                if (has_default_color_correction) {
                    if (builder.get(CaptureRequest.COLOR_CORRECTION_MODE) != null
                            && !builder.get(CaptureRequest.COLOR_CORRECTION_MODE).equals(default_color_correction)) {
                        builder.set(CaptureRequest.COLOR_CORRECTION_MODE, default_color_correction);
                    }
                    has_default_color_correction = false; // set to false, as only need to set COLOR_CORRECTION_MODE
                                                          // back to default when changing from manual back to
                                                          // non-manual white balance
                }
            ...
            if (white_balance == CameraMetadata.CONTROL_AWB_MODE_OFF) {
                ...
                if (MyDebug.LOG) {
                    Log.d(TAG, "original color_correction_transform: "
                            + builder.get(CaptureRequest.COLOR_CORRECTION_TRANSFORM));
                }
                // need to set COLOR_CORRECTION_TRANSFORM on some devices (e.g. Pixel 6 Pro) as
                // they don't have it set by default
        ...
        private boolean setAntiBanding(CaptureRequest.Builder builder) {
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                if (builder.get(CaptureRequest.CONTROL_AE_ANTIBANDING_MODE) == null
                        || builder.get(CaptureRequest.CONTROL_AE_ANTIBANDING_MODE) != antibanding) {
                    if (MyDebug.LOG)
        ...
        private boolean setEdgeMode(CaptureRequest.Builder builder) {
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                if (builder.get(CaptureRequest.EDGE_MODE) == null
                        || builder.get(CaptureRequest.EDGE_MODE) != edge_mode) {
                    if (MyDebug.LOG)
                ...
                if (builder.get(CaptureRequest.EDGE_MODE) != null
                        && !builder.get(CaptureRequest.EDGE_MODE).equals(default_edge_mode)) {
        ...
        private boolean setNoiseReductionMode(CaptureRequest.Builder builder) {
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                if (builder.get(CaptureRequest.NOISE_REDUCTION_MODE) == null
                        || builder.get(CaptureRequest.NOISE_REDUCTION_MODE) != noise_reduction_mode) {
                    if (MyDebug.LOG)
                ...
                if (builder.get(CaptureRequest.NOISE_REDUCTION_MODE) != null
                        && !builder.get(CaptureRequest.NOISE_REDUCTION_MODE).equals(default_noise_reduction_mode)) {
        ...
        private boolean setAEMode(CaptureRequest.Builder builder, boolean is_still) {
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                 ...
                 * supports_low_light_boost && !is_still ) {
                 * builder.set(CaptureRequest.CONTROL_AE_MODE,
            ...
            if (has_iso) {
                ...
                // only need to account for FLASH_MODE_TORCH, otherwise we use fake flash mode
                // for manual ISO
                ...
                // prefer to set flash via the ae mode (otherwise get even worse results),
                // except for torch which we can't
                switch (flash_value) {
                    case "flash_off":
                        ...
                        break;
                    case "flash_auto":
                        // note we set this even in fake flash mode (where we manually turn torch on and
                        // off to simulate flash) so we
                        ...
                        break;
                    case "flash_on":
                        // see note above for "flash_auto" for why we set this even fake flash mode -
                        // arguably we don't need to know
        ...
        private void setRawMode(CaptureRequest.Builder builder) {
            // DngCreator says "For best quality DNG files, it is strongly recommended that
            // lens shading map output is enabled if supported"
            // docs also say "ON is always supported on devices with the RAW capability", so
            // we don't check for STATISTICS_LENS_SHADING_MAP_MODE_ON being available
        ...
        private void setStabilization(CaptureRequest.Builder builder) {
            ...
            if (supports_optical_stabilization) {
                if (video_stabilization) {
                    ...
                    if (builder.get(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE) != null
                            && !builder.get(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE)
                                    .equals(default_optical_stabilization)) {
                        if (MyDebug.LOG)
        ...
        private void setTonemapProfile(CaptureRequest.Builder builder) {
            ...
            // to use test_new, also need to uncomment the test code in setFocusValue() to
            // call setTonemapProfile()
            // boolean test_new = this.af_mode == CaptureRequest.CONTROL_AF_MODE_AUTO; //
            // testing
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                if (use_preset_curve && tonemap_profile == TonemapProfile.TONEMAPPROFILE_REC709
                ...
                } else if (use_preset_curve && tonemap_profile == TonemapProfile.TONEMAPPROFILE_SRGB
                        && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (MyDebug.LOG)
                    ...
                    switch (tonemap_profile) {
                        ...
                        case TONEMAPPROFILE_GAMMA: {
                            // better to use uniformly spaced values, otherwise we get a weird looking
                            // effect - this can be
                            // seen most prominently when using gamma 1.0f, which should look linear (and
                            // hence be independent
                            // of the x values we use)
                            // can be reproduced on at least OnePlus 3T and Galaxy S10e (although the exact
                            // behaviour of the
                            // poor results is different on those devices)
                            int n_values = tonemap_log_max_curve_points_c;
                            if (is_samsung) {
                                // unfortunately odd bug on Samsung devices (at least S7 and S10e) where if more
                                // than 32 control points,
                                // the maximum brightness value is reduced (can best be seen with 64 points, and
                                // using gamma==1.0)
                                // note that Samsung devices also need at least 16 control points - or in some
                                // cases 32, see comments for
                                // enforceMinTonemapCurvePoints().
                                // 32 is better than 16 anyway, as better to have more points for finer curve
                                // where possible.
                                n_values = 32;
                            }
                            ...
                            for (int i = 0; i < n_values; i++) {
                                ...
                                float out = (tonemap_profile == TonemapProfile.TONEMAPPROFILE_LOG) ? getLogProfile(in)
                                        : getGammaProfile(in);
                                values[2 * i] = in;
                                values[2 * i + 1] = out;
                            }
                             ...
                             * float out = (tonemap_profile==TonemapProfile.TONEMAPPROFILE_LOG) ?
                             * getLogProfile(in) : getGammaProfile(in);
                             ...
                             * }
                             * values[c++] = 1.0f;
                             * float last_out = (tonemap_profile==TonemapProfile.TONEMAPPROFILE_LOG) ?
                             * getLogProfile(1.0f) : getGammaProfile(1.0f);
                             * if( tonemap_profile==TonemapProfile.TONEMAPPROFILE_LOG )
                             ...
                             * }
                             * values[c++] = 1.0f;
                             * values[c++] = (tonemap_profile==TonemapProfile.TONEMAPPROFILE_LOG) ?
                             * getLogProfile(1.0f) : getGammaProfile(1.0f);
                             * values = Arrays.copyOfRange(values,0,c);
                             * }
                             */
                    ...
                    // sRGB:
                    /*
                     * values = new float []{0.0000f, 0.0000f, 0.0667f, 0.2864f, 0.1333f, 0.4007f,
                     * 0.2000f, 0.4845f,
                     * 0.2667f, 0.5532f, 0.3333f, 0.6125f, 0.4000f, 0.6652f, 0.4667f, 0.7130f,
                     * 0.5333f, 0.7569f, 0.6000f, 0.7977f, 0.6667f, 0.8360f, 0.7333f, 0.8721f,
                     * 0.8000f, 0.9063f, 0.8667f, 0.9389f, 0.9333f, 0.9701f, 1.0000f, 1.0000f};
                     */
                    /*
                     * values = new float []{0.0000f, 0.0000f, 0.05f, 0.3f, 0.1f, 0.4f, 0.2000f,
                     * 0.4845f,
                     * 0.2667f, 0.5532f, 0.3333f, 0.6125f, 0.4000f, 0.6652f,
                     * 0.5f, 0.78f, 1.0000f, 1.0000f};
                     */
                    /*
                     * values = new float []{0.0f, 0.0f, 0.05f, 0.4f, 0.1f, 0.54f, 0.2f, 0.6f, 0.3f,
                     * 0.65f, 0.4f, 0.7f,
                     * 0.5f, 0.78f, 1.0f, 1.0f};
                     */
                    /*
                     * values = new float[]{0.0f, 0.0f, 0.0667f, 0.2864f, 0.1333f, 0.4007f, 0.2000f,
                     * 0.4845f,
                     * 1.0f, 1.0f};
                     */
                    // values = new float []{0.0f, 0.5f, 0.05f, 0.6f, 0.1f, 0.7f, 0.2f, 0.8f, 0.5f,
                    // 0.9f, 1.0f, 1.0f};
                    /*
    ...
    /**
     * Converts a white balance temperature to red, green even, green odd and blue
     * components.
     */
    ...
    public static int convertRggbVectorToTemperature(RggbChannelVector rggbChannelVector) {
        return convertRggbToTemperature(new float[] { rggbChannelVector.getRed(), rggbChannelVector.getGreenEven(),
                rggbChannelVector.getGreenOdd(), rggbChannelVector.getBlue() });
    }

    /**
     * Converts a red, green even, green odd and blue components to a white balance
     * temperature.
     * Note that this is not necessarily an inverse of convertTemperatureToRggb,
     * since many rggb
                ...
                if (camera != null && hasCaptureSession()) { // make sure camera wasn't released in the meantime
                    ...
                    if (picture_cb.imageQueueWouldBlock(imageReaderRaw != null ? 1 : 0, 1)) {
                        ...
                        if (burst_type == BurstType.BURSTTYPE_FOCUS) {
                            // For focus bracketing mode, we play the shutter sound per shot (so the user
                            // can tell when the sequence is complete).
                            // From a user mode, the gap between shots in focus bracketing mode makes this
                            // more analogous to the auto-repeat mode
                            // (at the Preview level), which makes the shutter sound per shot.

                            playSound(shutter_click_sound);
                        }
                        try {
                            captureSession.capture(slow_burst_capture_requests.get(n_burst_taken),
                                    previewCaptureCallback, handler);
    ...
    private class OnImageAvailableListener implements ImageReader.OnImageAvailableListener {
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            if (picture_cb == null || !jpeg_todo) {
                // in theory this shouldn't happen - but if this happens, still free the image
                // to avoid risk of memory leak,
                // or strange behaviour where an old image appears when the user next takes a
                // photo
            ...
            Image image = reader.acquireNextImage();
            if (image == null) {
                // can happen if camera closed whilst taking photo - this happens in
                ...
                if (burst_single_request) {
                    ...
                    if (pending_burst_images.size() >= n_burst) { // shouldn't ever be greater, but just in case
                        ...
                        if (pending_burst_images.size() > n_burst) {
                            Log.e(TAG, "pending_burst_images size " + pending_burst_images.size()
                                    + " is greater than n_burst " + n_burst);
                        }
            ...
            synchronized (background_camera_lock) {
                if (single_burst_complete_images != null) {
                    ...
                    if (burst_type == BurstType.BURSTTYPE_CONTINUOUS && !continuous_burst_requested_last_capture) {
                        // even if n_burst is 0, we don't want to give up if we're still in continuous
                        // burst mode
                        // also note if we do have continuous_burst_requested_last_capture==true, we
                        // still check for
                        // n_burst==0 below (as there may have been more than one image still to be
                        // received)
                        if (MyDebug.LOG)
            ...
            // need to call outside of lock (because they can lead to calls to external
            // callbacks)
        ...
        /**
         * Called when an image has been received, but we're in a burst mode, and not
         * all images have
         * been received.
         */
            ...
            synchronized (background_camera_lock) {
                if (slow_burst_capture_requests != null) {
                    ...
                    if (burst_type != BurstType.BURSTTYPE_FOCUS) {
                        /*
                         * try {
                         * if( camera != null && hasCaptureSession() ) { // make sure camera wasn't
                         ...
                         * }
                         */
                        // see note in takePictureBurstBracketing() for why we also set preview for slow
                        // burst with expo bracketing -
                        // helps Samsung Galaxy devices
                        if (previewBuilder != null) { // make sure camera wasn't released in the meantime
                            try {
                                long exposure_time = slow_burst_capture_requests.get(n_burst_taken)
                                ...
                                MyDebug.logStackTrace(TAG, "failed to take set exposure for next expo bracketing burst",
                                        e);
                                jpeg_todo = false;
                                raw_todo = false;
                                picture_cb = null;
                        ...
                        if (!focus_bracketing_in_progress) {
                            ...
                            // ideally we'd stop altogether, but instead we take one last shot, so that we
                            // can mark it with the
                            // RequestTagType.CAPTURE tag, so onCaptureCompleted() is called knowing it's
                            // for the last image
                            if (MyDebug.LOG) {
                                Log.d(TAG,
                                        "slow_burst_capture_requests size was: " + slow_burst_capture_requests.size());
                                Log.d(TAG, "n_burst size was: " + n_burst);
                                Log.d(TAG, "n_burst_taken: " + n_burst_taken);
                            }
                            slow_burst_capture_requests.subList(n_burst_taken + 1, slow_burst_capture_requests.size())
                                    .clear(); // resize to n_burst_taken
                            // if burst_single_request==true, n_burst is constant and we stop when
                            // pending_burst_images.size() >= n_burst
                            // if burst_single_request==false, n_burst counts down and we stop when
                            // n_burst==0
                            if (burst_single_request) {
                            ...
                            RequestTagObject requestTag = (RequestTagObject) slow_burst_capture_requests
                                    .get(slow_burst_capture_requests.size() - 1).getTag();
                            requestTag.setType(RequestTagType.CAPTURE);
                        }

                        // code for focus bracketing
                        try {
                            float focus_distance = slow_burst_capture_requests.get(n_burst_taken)
                                    .get(CaptureRequest.LENS_FOCUS_DISTANCE);
                            if (MyDebug.LOG) {
                            ...
                            MyDebug.logStackTrace(TAG,
                                    "failed to take set focus distance for next focus bracketing burst", e);
                            jpeg_todo = false;
                            raw_todo = false;
                            picture_cb = null;
                            push_take_picture_error_cb = take_picture_error_cb;
        ...
        /**
         * Called when an image has been received, but either we're not in a burst mode,
         * or we are
        ...
        private void takePhotoCompleted() {
            ...
            // need to set jpeg_todo to false before calling onCompleted, as that may
            // reenter CameraController to take another photo (if in auto-repeat burst mode)
            // - see testTakePhotoRepeat()
            synchronized (background_camera_lock) {
                jpeg_todo = false;
            }
    ...
    private class OnRawImageAvailableListener implements ImageReader.OnImageAvailableListener {
        ...
        void setCaptureResult(CaptureResult capture_result) {
            ...
            synchronized (background_camera_lock) {
                /*
                 * synchronize, as we don't want to set the capture_result, at the same time
                 * that onImageAvailable() is called, as
                ...
                if (!images.isEmpty()) {
                    ...
                    // should call processImage() on UI thread, to be consistent with
                    // onImageAvailable()->processImage()
                    // important to avoid crash when pause preview is option, tested in
                    // testTakePhotoRawWaitCaptureResult()
        ...
        private void processImage() {
            ...
            synchronized (background_camera_lock) {
                ...
                if (n_burst_total == 1 && burst_type != BurstType.BURSTTYPE_CONTINUOUS) {
                    // Rather than call onRawPictureTaken straight away, we set pending_raw_image so
                    // that
                    // it's called in checkImagesCompleted, to ensure the RAW callback is taken
                    // after the JPEG callback.
                    // This isn't required, but can give an appearance of better performance to the
                    // user, as the thumbnail
                    // animation for a photo having been taken comes from the JPEG.
                    // We don't do this for burst mode, as it would get too complicated trying to
                    // enforce an ordering...
                    ...
                    if (pending_burst_images_raw.size() >= n_burst_raw) { // shouldn't ever be greater, but just in case
                        ...
                        if (pending_burst_images_raw.size() > n_burst_raw) {
                            Log.e(TAG, "pending_burst_images_raw size " + pending_burst_images_raw.size()
                                    + " is greater than n_burst_raw " + n_burst_raw);
                        }
                ...
                synchronized (background_camera_lock) {
                    if (single_burst_complete_images != null) {
                        ...
                        if (MyDebug.LOG)
                            Log.d(TAG, "n_burst_raw is now " + n_burst_raw);
                        if (burst_type == BurstType.BURSTTYPE_CONTINUOUS && !continuous_burst_requested_last_capture) {
                            // even if n_burst_raw is 0, we don't want to give up if we're still in
                            // continuous burst mode
                            // also note if we do have continuous_burst_requested_last_capture==true, we
                            // still check for
                            // n_burst_raw==0 below (as there may have been more than one image still to be
                            // received)
                            if (MyDebug.LOG)
                                Log.d(TAG, "continuous burst mode still in progress");
    ...
    private CaptureRequest fake_precapture_turn_on_torch_id = null; // the CaptureRequest used to turn on torch when
                                                                    // starting the "fake" precapture
    ...
    private void onError(@NonNull CameraDevice cam) {
        ...
        // need to set the camera to null first, as closing the camera may take some
        // time, and we don't want any other operations to continue (if called from main
        // thread)
        this.camera = null;
        if (MyDebug.LOG)
        ...
        if (camera_already_opened) {
            ...
            // n.b., as this is potentially serious error, we always log even if MyDebug.LOG
            // is false
     ...
     * @param cameraIdSPhysical      If non-null, specifies a physical camera to use
     *                               (must be a member of
     *                               CameraFeatures.physical_camera_ids for this
     *                               camera or the corresponding logical camera)
     * @param camera_features_caches This should be supplied as an initially empty
     *                               map, which CameraController2 can use to improve
     *                               performance on subsequent creations of
     *                               CameraController2.
     *                               The same camera_features_caches should be
     *                               supplied to future new CameraController2
     *                               objects in order to benefit.
     * @param preview_error_cb       onError() will be called if the preview stops
     *                               due to error.
     * @param camera_error_cb        onError() will be called if the camera closes
     *                               due to serious error. No more calls to the
     *                               CameraController2 object should be made (though
     *                               a new one can be created, to try reopening the
     *                               camera).
     * @throws CameraControllerException if the camera device fails to open.
     */
    public CameraController2(Context context, int cameraId, String cameraIdSPhysical,
            Map<String, CameraFeaturesCache> camera_features_caches, final ErrorCallback preview_error_cb,
            final ErrorCallback camera_error_cb) throws CameraControllerException {
        super(cameraId);
        if (MyDebug.LOG) {
        ...
        this.is_samsung_galaxy_s = is_samsung && (Build.MODEL.toLowerCase(Locale.US).contains("sm-g")
                || Build.MODEL.toLowerCase(Locale.US).contains("sm-s"));
        ...
        class MyStateCallback extends CameraDevice.StateCallback {
            ...
            boolean first_callback = true; // Google Camera says we may get multiple callbacks, but only the first
                                           // indicates the status of the camera opening operation
            ...
            public void onOpened(@NonNull CameraDevice cam) {
                ...
                if (first_callback) {
                    ...
                    try {
                        // we should be able to get characteristics at any time, but Google Camera only
                        // does so when camera opened - so do so similarly to be safe
                        if (MyDebug.LOG)
                            Log.d(TAG, "try to get camera characteristics");
                        characteristics = manager
                                .getCameraCharacteristics(cameraIdSPhysical != null ? cameraIdSPhysical : cameraIdS);
                        if (MyDebug.LOG)
                            Log.d(TAG, "successfully obtained camera characteristics");
                        // now read cached values
                        characteristics_sensor_orientation = characteristics
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && cameraIdSPhysical == null) {
                            // n.b., getCameraExtensionCharacteristics is documented as saying this must be
                            // the standalone cameraID that can be directly opened with openCamera()
                            // however on Pixel 6 Pro at least, night mode extension only ever uses the wide
                            // camera, even if telephoto or ultrawide is set as a physical camera,
                            // so don't support for now
                            extension_characteristics = manager.getCameraExtensionCharacteristics(cameraIdS);
                            if (MyDebug.LOG)
                                Log.d(TAG, "successfully obtained camera characteristics");

                            // if we update the key used for camera_features_caches, remember to also update
                            // the code
                        ...
                        // note, this won't start the preview yet, but we create the previewBuilder in
                        // order to start setting camera parameters
                        createPreviewRequest();
                    } catch (CameraAccessException e) {
                        MyDebug.logStackTrace(TAG, "failed to get camera characteristics", e);
                        // don't throw CameraControllerException here - instead error is handled by
                        // setting callback_done to callback_done, and the fact that camera will still
                        // be null
                    }
        ...
        try {
            ...
            // Have seen this from Google Play - even though the Preview should have checked
            // the
        ...
        // set up a timeout - sometimes if the camera has got in a state where it can't
        // be opened until after a reboot, we'll never even get a myStateCallback
        // callback called
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
        ...
        // whilst this blocks, this should be running on a background thread anyway (see
        // Preview.openCamera()) - due to maintaining
        // compatibility with the way the old camera API works, it's easier to handle
        // running on a background thread at a higher level,
         ...
         * myStateCallback.onError(camera,
         ...
         * manager.getCameraCharacteristics(cameraIdSPhysical != null ?
         * cameraIdSPhysical : cameraIdS);
         * StreamConfigurationMap configs =
         * characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
         * android.util.Size [] camera_picture_sizes =
         * configs.getOutputSizes(ImageFormat.JPEG);
         * imageReader = ImageReader.newInstance(camera_picture_sizes[0].getWidth(), ,
         * ImageFormat.JPEG, 2);
         */

        // preload sounds to reduce latency - important so that START_VIDEO_RECORDING
        // sound doesn't play after video has started (which means it'll be heard in the
        // resultant video)
        ...
        // Samsung Galaxy devices have bug where MediaActionSound always plays at 100%
        // volume - the SHUTTER_CLICK sounds
    ...
    public void release() {
        ...
        synchronized (background_camera_lock) {
            // set all to null straight away, as this can be called on background thread,
            // but also
            // don't want to be in an incomplete state for other threads where camera is
            // non-null but
         ...
         */
        if (thread != null) {
            // should only close thread after closing the camera, otherwise we get messages
            // "sending message to a Handler on a dead thread"
            // see
            // https://sourceforge.net/p/opencamera/discussion/general/thread/32c2b01b/?limit=25
            thread.quitSafely();
            try {
                thread.join();
                thread = null;
    ...
    /**
     * Enforce a minimum number of points in tonemap curves - needed due to Galaxy
     * S10e having wrong behaviour if fewer
     * than 16 or in some cases 32 points?! OnePlus 3T meanwhile has more gradual
     * behaviour where it gets better at 64 points.
     */
    private float[] enforceMinTonemapCurvePoints(float[] in_values) {
        ...
        if (is_samsung) {
            ...
            // the maximum brightness value is reduced (can best be seen with 64 points, and
            // using gamma==1.0).
            // Also note that Samsung devices also need at least 16 control points, or in
            // some cases 32, due to problem
    ...
    public static int computeZoomRatios(List<Integer> ratios, float min_zoom, float max_zoom) {
        ...
        if (zoom_ratios_above_one.isEmpty()
                || zoom_ratios_above_one.get(zoom_ratios_above_one.size() - 1) != max_zoom_ratio) {
        ...
        if (ratios.get(0) / 100.0f < min_zoom) {
            ...
            // e.g. if min_zoom = 0.666, we'd have stored a zoom ratio of 66 which then
            // would
        ...
        if (ratios.get(0) < 100) {
            ...
            // if the min zoom is < 1.0, we add multiple entries for 1x zoom, when using the
            // zoom
            ...
            // add values for 1.0f (we add repeated values so for cameras with min_zoom <
            // 1x, the zoom seekbar will snap to 1x)
    ...
    public CameraFeatures getCameraFeatures() throws CameraControllerException {
        ...
        if (MyDebug.LOG) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                Capability[] capabilities = characteristics
                        .get(CameraCharacteristics.CONTROL_AVAILABLE_EXTENDED_SCENE_MODE_CAPABILITIES);
        ...
        if (cameraIdSPhysical != null) {
            // don't support zoom for physical lenses - problem on Galaxy S24+ that zooming
            // on physical lense gives random colours!
            // but in general, the exposed zoom ranges don't seem correct for physical
            // lenses
            // both the above are true for CONTROL_ZOOM_RATIO_RANGE and
            // SCALER_AVAILABLE_MAX_DIGITAL_ZOOM
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // use CONTROL_ZOOM_RATIO_RANGE on Android 11+, to support multiple cameras with
            // zoom ratios
            // less than 1
            try {
                ...
                // have had this crash from
                // characteristics.get(CameraCharacteristics.CONTROL_ZOOM_RATIO_RANGE) on Google
                // Play for some older Samsung Galaxy A* and Nokia devices
        ...
        for (int face_mode : face_modes) {
            ...
            // note that devices that support STATISTICS_FACE_DETECT_MODE_FULL (e.g., Nexus
            // 6) don't return
        ...
        if (cameraIdSPhysical != null) {
            // If we have a physical camera ID, characteristics refer to the physical camera
            // ID. But for some things,
            ...
            try {
                ...
                logical_capabilities = logical_characteristics
        ...
        for (int capability : capabilities) {
             ...
             * // At least some Huawei devices (at least, the Huawei device model FIG-LX3,
             * device code-name hi6250) don't
             * // have REQUEST_AVAILABLE_CAPABILITIES_MANUAL_SENSOR, but I had a user
             * complain that HDR mode and manual ISO
             * // had previously worked for them. Note that we still check below for
             * SENSOR_INFO_SENSITIVITY_RANGE and
             * // SENSOR_INFO_EXPOSURE_TIME_RANGE, so not checking
             * REQUEST_AVAILABLE_CAPABILITIES_MANUAL_SENSOR shouldn't
             * // enable manual ISO/exposure on devices that don't support it.
             * // Also may affect Samsung Galaxy A8(2018).
             * // Instead we just block LEGACY devices (probably don't need to, again
             * because we check
             * // SENSOR_INFO_SENSITIVITY_RANGE and SENSOR_INFO_EXPOSURE_TIME_RANGE, but
             * just in case).
             ...
             */
                    ...
                    && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                // we test for at least Android M just to be safe (this is needed for
            ...
            } else if (capability == CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_ULTRA_HIGH_RESOLUTION_SENSOR
        ...
        for (int capability : logical_capabilities) {
            // to be safe, we check the REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA
            // from the logical camera
            if (capability == CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA
                    && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                // we test for at least Android 9 just to be safe (this is needed for
        ...
        // At least some Huawei devices (at least, the Huawei device model FIG-LX3,
        // device code-name hi6250) don't have
        // REQUEST_AVAILABLE_CAPABILITIES_BURST_CAPTURE, but I had a user complain that
        // NR mode at least had previously
        // (before 1.45) worked for them. It might be that this can still work, just not
        // at 20fps.
        // So instead set to true for all LIMITED devices. Still keep block for LEGACY
        // devices (which definitely shouldn't
        // support fast burst - and which Open Camera never allowed with Camera2 before
        // 1.45).
        // Also may affect Samsung Galaxy A8(2018).
        camera_features.supports_burst = CameraControllerManager2.isHardwareLevelSupported(characteristics,
         ...
         * Boolean bool =
         * characteristics.get(CameraCharacteristics.CONTROL_AUTOFRAMING_AVAILABLE);
         * if( bool != null && bool) {
        ...
        try {
            ...
            // have had IllegalArgumentException crashes from Google Play - unclear what the
            // cause is, but at least fail gracefully
            // similarly for NullPointerException - note, these aren't from characteristics
            // being null, but from
            // com.android.internal.util.Preconditions.checkArrayElementsNotNull
            // (Preconditions.java:395) - all are from
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE && capabilities_10bit) {
            ...
            if (jpeg_r_camera_picture_sizes != null) {
                ...
                // For simplicity, we only support JPEG_R if it has the same support as for
                // JPEG.
                // Further checks are done below for getHighResolutionOutputSizes.
                // Note that extensions don't support JPEG_R
                // (extension_characteristics.getExtensionSupportedSizes
                // is documented that it throws IllegalArgumentException if not JPEG or
                // YUV_420_888).
                ...
                if (camera_features.supports_jpeg_r) {
                    // documentation says HLG10 must be supported by all devices with
                    // REQUEST_AVAILABLE_CAPABILITIES_DYNAMIC_RANGE_TEN_BIT,
                    // but check just to be safe
                    DynamicRangeProfiles profiles = characteristics
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (camera_picture_sizes_hires != null) {
                for (android.util.Size camera_size : camera_picture_sizes_hires) {
                    if (MyDebug.LOG)
                        Log.d(TAG, "high resolution picture size: " + camera_size.getWidth() + " x "
                                + camera_size.getHeight());
                    // Check not already listed? If it's listed in both, we'll add it later on when
                    // scanning camera_picture_sizes
                    ...
                    if (!found) {
                        if (MyDebug.LOG)
                            Log.d(TAG, "high resolution [non-burst] picture size: " + camera_size.getWidth() + " x "
                                    + camera_size.getHeight());
                        CameraController.Size size = new CameraController.Size(camera_size.getWidth(),
                                camera_size.getHeight());
                ...
                if (camera_features.supports_jpeg_r && Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                    android.util.Size[] camera_picture_sizes_hires_jpeg_r = configs
                            .getHighResolutionOutputSizes(ImageFormat.JPEG_R);
        ...
        if (camera_picture_sizes == null) {
            // camera_picture_sizes is null on Samsung Galaxy Note 10+ and S20 for camera ID
            // 4!
            ...
            for (android.util.Size camera_size : camera_picture_sizes) {
                ...
                camera_features.picture_sizes
                        .add(new CameraController.Size(camera_size.getWidth(), camera_size.getHeight()));
            }
        }
        ...
        if (camera_video_sizes == null) {
            // camera_video_sizes is null on Samsung Galaxy Note 10+ and S20 for camera ID
            // 4!
            ...
            for (android.util.Size camera_size : camera_video_sizes) {
                ...
                CameraController.Size normal_video_size = new CameraController.Size(camera_size.getWidth(),
                        camera_size.getHeight(), fr, false);
        ...
        // don't support high speed if physical camera specified - seems unreliable on
        // Pixel 6 Pro and Galaxy S24+
        if (capabilities_high_speed_video && cameraIdSPhysical == null) {
            ...
            for (Range<Integer> r : configs.getHighSpeedVideoFpsRanges()) {
                // Some devices e.g. Pixel 6 Pro have high-speed fps ranges like [30-120]. We
                // skip these because:
                // Firstly we'd risk choosing this for 60fps, when 60fps shouldn't require
                // high-speed.
                // Secondly captureSessionHighSpeed.createHighSpeedRequestList() documentation
                // says fps range
                // should have min==max, so we don't want to include high speed ranges where
                // this isn't true.
                // Without this fix, Slow motion 0.5x (which uses 60fps) fails to start
                // recording on Pixel 6 Pro.
            ...
            for (android.util.Size camera_size : camera_video_sizes_high_speed) {
                ...
                for (Range<Integer> r : configs.getHighSpeedVideoFpsRangesFor(camera_size)) {
                    ...
                    // In theory, all fps ranges returned by getHighSpeedVideoFpsRangesFor() should
                    // surely be
                    // a subset of fps ranges returned by getHighSpeedVideoFpsRanges(), but we check
                    // just in case
                    // (when deciding whether slow motion or high speed frame rates are supported,
                    // this means we
                    ...
                    if (!found) {
                        if (MyDebug.LOG)
                            Log.e(TAG,
                                    "video size " + camera_size + " has high speed frame rate "
                                            + Arrays.toString(this_fps_range)
                                            + " that wasn't returned by configs.getHighSpeedVideoFpsRanges()");
                        continue;
                    }
                ...
                CameraController.Size hs_video_size = new CameraController.Size(camera_size.getWidth(),
                        camera_size.getHeight(), fr, true);
                if (MyDebug.LOG) {
        ...
        // display_size.x > display_size.y as preview resolutions also have width >
        // height
        ...
        if (camera_preview_sizes == null) {
            // camera_preview_sizes is null on Samsung Galaxy Note 10+ and S20 for camera ID
            // 4!
            ...
            for (android.util.Size camera_size : camera_preview_sizes) {
                ...
                if (camera_size.getWidth() > Math.max(display_size.x, 1280)
                        || camera_size.getHeight() > Math.max(display_size.y, 720)) {
                    // Nexus 6 returns these, even though not supported?! (get green corruption
                    // lines if we allow these)
                    // Google Camera filters anything larger than height 1080, with a todo saying to
                    // use device's measurements
                    // But we only considering filtering if also greater than width 1280 or height
                    // 720, as these should be fine - also
                    // need to account for running in multi-window mode - we don't want to exclude
                    // all preview resolutions just because the
                    // window size was small!
                    continue;
                }
                camera_features.preview_sizes
                        .add(new CameraController.Size(camera_size.getWidth(), camera_size.getHeight()));
            }
        }
        ...
        if (extension_characteristics == null) {
            ...
            if (camera_features_cache.supported_extensions_zoom != null)
                camera_features.supported_extensions_zoom = new ArrayList<>(
                        camera_features_cache.supported_extensions_zoom);
            ...
            if (camera_features.supported_extensions != null) {
                for (int extension : camera_features.supported_extensions) {
                    ...
                    List<android.util.Size> extension_picture_sizes = camera_features_cache.extension_picture_sizes_map
                            .get(extension);
                    List<android.util.Size> extension_preview_sizes = camera_features_cache.extension_preview_sizes_map
                            .get(extension);
                    boolean has_picture_resolution = updatePictureSizesForExtension(camera_features.picture_sizes,
                            extension_picture_sizes, extension);
                    boolean has_preview_resolution = updatePreviewSizesForExtension(camera_features.preview_sizes,
                            extension_preview_sizes, extension);
            ...
            if (extensions != null) {
                ...
                for (int extension : extensions) {
                    ...
                    try {
                        // we assume that the allowed extension sizes are a subset of the full sizes -
                        // makes things easier to manage

                        List<android.util.Size> extension_picture_sizes = extension_characteristics
                                .getExtensionSupportedSizes(extension, ImageFormat.JPEG);
                        if (MyDebug.LOG)
                            Log.d(TAG, "    extension_picture_sizes: " + extension_picture_sizes);
                        boolean has_picture_resolution = updatePictureSizesForExtension(camera_features.picture_sizes,
                                extension_picture_sizes, extension);

                        List<android.util.Size> extension_preview_sizes = extension_characteristics
                                .getExtensionSupportedSizes(extension, SurfaceTexture.class);
                        if (MyDebug.LOG)
                        ...
                        if (has_picture_resolution && has_preview_resolution) {
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                                Set<CaptureRequest.Key> extension_supported_request_keys = extension_characteristics
                                        .getAvailableCaptureRequestKeys(extension);
                                ...
                                Set<CaptureResult.Key> extension_supported_result_keys = extension_characteristics
                                        .getAvailableCaptureResultKeys(extension);
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                                if (MyDebug.LOG) {
                                    Log.d(TAG, "    isCaptureProcessProgressAvailable: "
                                            + extension_characteristics.isCaptureProcessProgressAvailable(extension));
                                }
                            }
                        }
                    } catch (Exception exception) {
                        // have IllegalArgumentException from getExtensionSupportedSizes() and
                        // getAvailableCaptureRequestKeys() at least from Google Play crashes
                        if (MyDebug.LOG)
            ...
            // add to cache
            CameraFeaturesCache cache = new CameraFeaturesCache(camera_features, extension_picture_sizes_map,
                    extension_preview_sizes_map);
        ...
        if (characteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE)) {
            int[] supported_flash_modes_arr = characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES); // Android
                                                                                                                     // format
            ...
            // documentation for CONTROL_AE_AVAILABLE_MODES says the following modes are
            // always supported:
            ...
            /*
             * if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM &&
             * supported_flash_modes.contains(CaptureRequest.
        ...
        Float minimum_focus_distance_f = characteristics.get(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE); // may
                                                                                                                      // be
                                                                                                                      // null
                                                                                                                      // on
                                                                                                                      // some
        ...
        int[] supported_focus_modes = characteristics.get(CameraCharacteristics.CONTROL_AF_AVAILABLE_MODES); // Android
                                                                                                             // format
        camera_features.supported_focus_values = convertFocusModesToValues(supported_focus_modes); // convert to our
                                                                                                   // format (also
                                                                                                   // resorts)
        if (camera_features.supported_focus_values != null
                && camera_features.supported_focus_values.contains("focus_mode_manual2")) {
        ...
        int[] supported_optical_stabilization_modes = characteristics
        ...
        int[] supported_video_stabilization_modes = characteristics
        ...
        camera_features.is_photo_video_recording_supported = CameraControllerManager2
                .isHardwareLevelSupported(characteristics, CameraMetadata.INFO_SUPPORTED_HARDWARE_LEVEL_LIMITED);
        ...
        if (white_balance_modes != null) {
            for (int value : white_balance_modes) {
                // n.b., Galaxy S10e for front and ultra-wide cameras offers
                // CONTROL_AWB_MODE_OFF despite
                // capabilities_manual_post_processing==false; if we don't check for
                // capabilities_manual_post_processing,
                // adjusting white balance temperature seems to work, but seems safest to
                // require
                // capabilities_manual_post_processing anyway
                if (value == CameraMetadata.CONTROL_AWB_MODE_OFF && capabilities_manual_post_processing
                        && allowManualWB()) {
        ...
        if (CameraControllerManager2.isHardwareLevelSupported(characteristics,
                CameraMetadata.INFO_SUPPORTED_HARDWARE_LEVEL_LIMITED)) {
            Range<Integer> iso_range = characteristics.get(CameraCharacteristics.SENSOR_INFO_SENSITIVITY_RANGE); // may
                                                                                                                 // be
                                                                                                                 // null
                                                                                                                 // on
                                                                                                                 // some
            ...
            if (iso_range != null) {
                ...
                Range<Long> exposure_time_range = characteristics
                        .get(CameraCharacteristics.SENSOR_INFO_EXPOSURE_TIME_RANGE); // may be null on some devices
                if (exposure_time_range != null) {
                    ...
                    if ((is_samsung_galaxy_s || is_samsung_galaxy_f)
                            && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        // seems we can get away with longer exposure on some devices (e.g., Galaxy S10e
                        // claims only max of 0.1s, but works with 1/3s)
                        // but Android 11 on Samsung devices also introduces a bug where manual exposure
                        // gets ignored if different to the preview,
                        // and since the max preview rate is limited to 1/5s (see
                        // max_preview_exposure_time_c), there's no point
                        // going above this!
                        // update: as of 1.54, we now can go above the max_preview_exposure_time_c, by
                        // using RequestTagType.RUN_POST_CAPTURE
                        // (see adjustPreviewToStill())
                        if (MyDebug.LOG)
                            Log.d(TAG, "boost max_exposure_time, was: " + max_exposure_time);
                        camera_features.max_exposure_time = Math.max(camera_features.max_exposure_time,
                                1000000000L / 2);
                    }
                }
            }
        ...
        camera_features.exposure_step = characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_STEP)
                .floatValue();
        ...
        if (capabilities_manual_post_processing) {
            ...
            if (tonemap_max_curve_points != null) {
                ...
                if (tonemap_modes == null) {
                    ...
                    // if supports_tonemap_contrast_curve==false, don't bother supporting
                    // tonemapping (in theory we could support the preset curves alone, but not
                    // supported for simplicity)
                    // if supports_tonemap_contrast_curve==true but
                    // supports_tonemap_preset_curve==false, we'll still support tonemapping, but
                    // always use contrast curves
                    if (supports_tonemap_contrast_curve) {
                        camera_features.tonemap_max_curve_points = tonemap_max_curve_points;
                        // for now we only expose supporting of custom tonemap curves if there are
                        // enough curve points for all the
                        // profiles we support
                        // remember to divide by 2 if we're comparing against the raw array length!
                        camera_features.supports_tonemap_curve = tonemap_max_curve_points >= tonemap_log_max_curve_points_c
                                &&
        ...
        // float [] apertures = new float[]{1.5f, 1.9f, 2.0f, 2.2f, 2.4f, 4.0f, 8.0f,
        // 16.0f}; // test
        if (MyDebug.LOG)
    ...
    /**
     * Returns true iff every entry in camera_sizes is also a member of
     * alt_camera_sizes (order
     * doesn't matter).
     */
    public static boolean sizeSubset(int[] camera_widths, int[] camera_heights, int[] alt_camera_widths,
            int[] alt_camera_heights) {
    ...
    /**
     * For each of the picture_sizes, update the
     * CameraController.Size.supported_extensions field to record if that resolution
     ...
     * @return If false, then none of the picture_sizes are supported by this
     *         extension.
     */
    private boolean updatePictureSizesForExtension(List<CameraController.Size> picture_sizes,
            List<android.util.Size> extension_picture_sizes, int extension) {
    ...
    /**
     * For each of the preview_sizes, update the
     * CameraController.Size.supported_extensions field to record if that resolution
     * supports the supplied extension.
     * 
     ...
     * @param extension               Extension to test.
     * @return If false, then none of the preview_sizes are supported by this
     *         extension.
     */
    private boolean updatePreviewSizesForExtension(List<CameraController.Size> preview_sizes,
            List<android.util.Size> extension_preview_sizes, int extension) {
    ...
    public SupportedValues setSceneMode(String value) {
        ...
        if (values2 != null) {
            // CONTROL_AVAILABLE_SCENE_MODES is supposed to always be available, but have
            // had some (rare) crashes from Google Play due to being null
            for (int value2 : values2) {
    ...
    public boolean sceneModeAffectsFunctionality() {
        // Camera2 API doesn't seem to have any warnings that changing scene mode can
    ...
    /**
     * Whether we should allow manual white balance, even if the device supports
    ...
    public SupportedValues setAntiBanding(String value) {
        ...
        if (supported_values != null) {
            // for antibanding, if the requested value isn't available, we don't modify it
            // at all
    ...
    public SupportedValues setEdgeMode(String value) {
        ...
        if (supported_values != null) {
            // for edge mode, if the requested value isn't available, we don't modify it at
            // all
            if (supported_values.selected_value.equals(value)) {
    ...
    public SupportedValues setNoiseReductionMode(String value) {
        ...
        if (supported_values != null) {
            // for noise reduction, if the requested value isn't available, we don't modify
            // it at all
            if (supported_values.selected_value.equals(value)) {
                    ...
                    switch (supported_values.selected_value) {
                        ...
                        case "minimal":
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                ...
                                // shouldn't ever be here, as NOISE_REDUCTION_MODE_MINIMAL shouldn't be a
                                // supported value!
                ...
                if (camera_settings.has_noise_reduction_mode != has_noise_reduction_mode
    ...
    public SupportedValues setISO(String value) {
        // not supported for CameraController2 - but Camera2 devices that don't support
        // manual ISO can call this,
    ...
    public void setPictureSize(int width, int height) {
        ...
        if (hasCaptureSession()) {
            // can only call this when captureSession not created - as the surface of the
            // imageReader we create has to match the surface we pass to the captureSession
            if (MyDebug.LOG)
    ...
    public void setJpegR(boolean want_jpeg_r) {
        ...
        if (hasCaptureSession()) {
            // can only call this when captureSession not created - as it affects how we
    ...
    public void setCameraExtension(boolean enabled, int extension) {
        ...
        if (sessionType == (enabled ? SessionType.SESSIONTYPE_EXTENSION : SessionType.SESSIONTYPE_NORMAL)
                && this.camera_extension == (enabled ? extension : 0)) {
            // quick exit
            if (MyDebug.LOG)
                Log.d(TAG, "    no change");
            return;
        ...
        if (enabled != (sessionType == SessionType.SESSIONTYPE_EXTENSION)) {
            ...
            // Ideally we'd probably only create the previewBuilder when starting the
            // preview (so we
            // start off with a "fresh" one), but for now at least ensure we start off with
            // a fresh
            // previewBuilder when enabling extensions (and might as well do so when
            // disabling
            // extensions too).
            // This saves us having to set capture request parameters back to their
            // defaults, and is
            // also useful for modes like CONTROL_AE_ANTIBANDING_MODE where there isn't an
            // obvious
            // "default" to set (in theory extensions mode should just ignore such keys, but
            // it'd be
    ...
    public void setBurstType(BurstType burst_type) {
        ...
        updateUseFakePrecaptureMode(camera_settings.flash_value);
        camera_settings.setAEMode(previewBuilder, false); // may need to set the ae mode, as flash is disabled for burst
                                                          // modes
    }
    ...
    public void setExpoBracketingNImages(int n_images) {
        ...
        if (n_images <= 1 || (n_images % 2) == 0) {
            ...
            throw new RuntimeException("n_images should be an odd number greater than 1"); // throw as RuntimeException,
                                                                                           // as this is a programming
                                                                                           // error
        }
    ...
    public boolean isCaptureFastBurst() {
        // BURSTTYPE_FOCUS photos are captured at a slow rate, so fine to return false
        // for that (means
    ...
    public void setUseCamera2FakeFlash(boolean use_fake_precapture) {
        ...
        // no need to call updateUseFakePrecaptureMode(), as this method should only be
        // called after first creating camera controller
    }
    ...
    private void createPictureImageReader() {
        ...
        // maxImages only needs to be 2, as we always read the JPEG data and close the
        // image straight away in the imageReader
        imageReader = ImageReader.newInstance(picture_width, picture_height,
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE && want_jpeg_r ? ImageFormat.JPEG_R
                        : ImageFormat.JPEG,
                2);
        // imageReader = ImageReader.newInstance(picture_width, picture_height,
        ...
        // It's intentional that we pass a handler on null, so the
        // OnImageAvailableListener runs on the UI thread.
        // If ever we want to change this on future, we should ensure that all image
        // available listeners (JPEG+RAW) are
        ...
        if (want_raw && raw_size != null && !previewIsVideoMode) {
            // unlike the JPEG imageReader, we can't read the data and close the image
            // straight away, so we need to allow a larger
            // value for maxImages
            imageReaderRaw = ImageReader.newInstance(raw_size.getWidth(), raw_size.getHeight(), ImageFormat.RAW_SENSOR,
                    max_raw_images);
            if (MyDebug.LOG) {
                Log.d(TAG, "created new imageReaderRaw: " + imageReaderRaw);
                Log.d(TAG, "imageReaderRaw surface: " + imageReaderRaw.getSurface().toString());
            }
            // see note above for imageReader.setOnImageAvailableListener for why we use a
            // null handler
            imageReaderRaw.setOnImageAvailableListener(onRawImageAvailableListener = new OnRawImageAvailableListener(),
                    null);
        }
    }
    ...
    private void takePendingRaw() {
        ...
        // takePendingRaw() always called on UI thread, and pending_raw_image only used
        // on UI thread, so shouldn't need to
        ...
        if (pending_raw_image != null) {
            ...
            // pending_raw_image should be closed by the application (we don't do it here,
            // so that applications can keep hold of the data, e.g., in a queue for
    ...
    private void checkImagesCompleted() {
        ...
        if (completed) {
            // need to set picture_cb to null before calling onCompleted, as that may
            // reenter CameraController to take another photo (if in auto-repeat burst mode)
            // - see testTakePhotoRepeat()
    ...
    public void setPreviewSize(int width, int height) {
         ...
         * previewImageReader.close();
         * }
         * previewImageReader = ImageReader.newInstance(width, height,
         * ImageFormat.YUV_420_888, 2);
         */
    }
    ...
    public void clearPreviewFpsRange() {
        ...
        // needed e.g. on Nokia 8 when switching back from slow motion to regular speed,
        // in order to reset to the regular
        // frame rate
        if (camera_settings.ae_target_fps_range != null || camera_settings.sensor_frame_duration != 0) {
            ...
            createPreviewRequest();
            // createPreviewRequest() needed so that the values in the previewBuilder reset
            // to default values, for
    ...
    public void setFocusValue(String focus_value) {
        ...
        camera_settings.setFocusDistance(previewBuilder); // also need to set distance, in case changed between
                                                          // infinity, manual or other modes
        // camera_settings.setTonemapProfile(previewBuilder); // testing - if using
        // focus mode to test video profiles, see test_new flag
        try {
            setRepeatingRequest();
        } catch (CameraAccessException e) {
    ...
    private void updateUseFakePrecaptureMode(String flash_value) {
        ...
        boolean frontscreen_flash = flash_value.equals("flash_frontscreen_auto")
                || flash_value.equals("flash_frontscreen_on");
    ...
    public void setFlashValue(String flash_value) {
        ...
        try {
            ...
            if (camera_settings.flash_value.equals("flash_torch") && !flash_value.equals("flash_off")) {
                // hack - if switching to something other than flash_off, we first need to turn
                // torch off, otherwise torch remains on (at least on Nexus 6 and Nokia 8)
    ...
    private void playSound(int soundName) {
        if (sounds_enabled) {
            // on some devices (e.g., Samsung Galaxy S10e), need to check whether phone on
            // silent!
     ...
     * We need this as callers will pass in (or expect returned)
     * CameraController.Area values that
     * are relative to the current view (i.e., taking zoom into account) (the old
     * Camera API in
     * CameraController1 always works in terms of the current view, whilst Camera2
     * works in terms
     * of the full view always). Similarly for the rect field in
    ...
    private Rect convertRectToCamera2(Rect crop_rect, Rect rect) {
        // CameraController.Area is always [-1000, -1000] to [1000, 1000] for the
        // viewable region
    ...
    public boolean setFocusAndMeteringArea(List<Area> areas) {
        ...
        if (MyDebug.LOG)
            Log.d(TAG, "sensor_rect: " + sensor_rect.left + " , " + sensor_rect.top + " x " + sensor_rect.right + " , "
                    + sensor_rect.bottom);
    ...
    public void clearFocusAndMetering() {
        ...
        if (sensor_rect.width() <= 0 || sensor_rect.height() <= 0) {
            // had a crash on Google Play due to creating a MeteringRectangle with -ve
            // width/height ?!
            camera_settings.af_regions = null;
            camera_settings.ae_regions = null;
        } else {
            if (characteristics.get(CameraCharacteristics.CONTROL_MAX_REGIONS_AF) > 0) {
                ...
                camera_settings.af_regions[0] = new MeteringRectangle(0, 0, sensor_rect.width() - 1,
                        sensor_rect.height() - 1, 0);
                camera_settings.setAFRegions(previewBuilder);
            } else
                camera_settings.af_regions = null;
            if (characteristics.get(CameraCharacteristics.CONTROL_MAX_REGIONS_AE) > 0) {
                ...
                camera_settings.ae_regions[0] = new MeteringRectangle(0, 0, sensor_rect.width() - 1,
                        sensor_rect.height() - 1, 0);
                camera_settings.setAERegions(previewBuilder);
            } else
                camera_settings.ae_regions = null;
    ...
    public List<Area> getFocusAreas() {
        ...
        if (camera_settings.af_regions == null) {
            // needed to fix failure on Android emulator in testTakePhotoContinuousNoTouch -
            // can happen when CONTROL_MAX_REGIONS_AF > 0, but Camera only has 1 focus mode
            // so Preview doesn't set focus areas
            return null;
        }
        ...
        if (metering_rectangles == null)
            return null;
        Rect sensor_rect = getViewableRect();
        if (metering_rectangles.length == 1 && metering_rectangles[0].getRect().left == 0
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                    ...
                    List<CaptureRequest> mPreviewBuilderBurst = captureSessionHighSpeed
                ...
                // got this as a Google Play exception (from
                // onCaptureCompleted->processCompleted) - this means the capture session is
                // already closed
            }
        }
    }
    ...
    /**
     * Performs a "capture" - note that in practice this isn't used for taking
     * photos, but for
    ...
    private List<OutputConfiguration> createOutputConfigurationList(List<Surface> surfaces, Surface preview_surface) {
        ...
        for (Surface surface : surfaces) {
            ...
            // On Galaxy S24+ at least, we seem to get Ultra HDR photos even without setting
            // DynamicRangeProfiles.HLG10
            // furthermore, calling setDynamicRangeProfile with HLG10 gives photos with much
            // lower saturation, so have
            // disabled this
            /*
             * if( want_jpeg_r && surface == preview_surface && Build.VERSION.SDK_INT >=
    ...
    /**
     * Function to support calling a function either on background thread or not
     * depending on wait_until_started.
     */
    private void launchCameraSession(boolean wait_until_started, CreateCaptureSessionFunction function,
            Runnable on_failed) throws CameraAccessException {
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        function.call();
                    } catch (CameraAccessException | NullPointerException | IllegalArgumentException
                            | UnsupportedOperationException e) {
                        // see notes below in createCaptureSession() for why we also catch
                        // NullPointerException, IllegalArgumentException, UnsupportedOperationException
                        // need to catch separately when wait_until_started==false due to this running
                        // on a background thread
                        MyDebug.logStackTrace(TAG, "exception create extension session on background thread", e);
                        // myStateCallback.onConfigureFailed();
                        if (on_failed != null) {
                            // if waiting, failure will be indicated via CameraControllerException thrown
                            // from this method
                            final Activity activity = (Activity) context;
                            activity.runOnUiThread(new Runnable() {
                                @Override
    ...
    private void createCaptureSession(boolean wait_until_started, Runnable runnable, Runnable on_failed,
            final MediaRecorder video_recorder, boolean want_photo_video_recording) throws CameraControllerException {
        if (MyDebug.LOG)
        ...
        if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
            ...
            // we check here rather than when setting those parameters, to avoid problems
            // with
        ...
        try {
            if (video_recorder != null) {
                ...
                // in some cases need to recreate picture imageReader and the texture default
                // buffer size (e.g., see test testTakePhotoPreviewPaused())
                createPictureImageReader();
            }
            if (texture != null) {
                ...
                // also need to create a new surface for the texture, in case the size has
                // changed - but make sure we remove the old one first!
                synchronized (background_camera_lock) {
            ...
            /*
             * if( MyDebug.LOG )
             * Log.d(TAG, "preview size: " + previewImageReader.getWidth() + " x " +
             * previewImageReader.getHeight());
             */
            if (MyDebug.LOG)
            ...
            class MyStateCallback extends CameraCaptureSession.StateCallback {
                ...
                private void onFailure() {
                    if (on_failed != null && !wait_until_started) {
                        // if waiting, failure will be indicated on main thread below via
                ...
                void onConfigured(@NonNull CameraCaptureSession session, @NonNull CameraExtensionSession eSession) {
                    ...
                    synchronized (background_camera_lock) {
                        if (camera == null) {
                            ...
                            // don't call onFailure() - if camera has closed in the meantime, no need to
                            // report to user (e.g. this might be going to Settings
                        ...
                        if (omt_streaming_surface != null && !want_video_high_speed) {
                            if (MyDebug.LOG) {
                                Log.d(TAG, "add omt streaming surface to previewBuilder: " + omt_streaming_surface);
                            }
                            previewBuilder.addTarget(omt_streaming_surface);
                        }
                        try {
                            setRepeatingRequest();
                            success = true;
                            ...
                            // we indicate that we failed to start the preview by setting captureSession
                            // back to null
                            // this will cause a CameraControllerException to be thrown below (if
                ...
                void onConfigureFailed() {
                    ...
                    onFailure();
                    // don't throw CameraControllerException here, as won't be caught - instead we
                    // throw CameraControllerException below (if wait_until_started==true),
                    // or via the on_failed callback (if wait_until_started==false)
                }
            ...
            synchronized (background_camera_lock) {
                ...
                List<Surface> surfaces_list = new ArrayList<>();
                surfaces_list.add(preview_surface);
                if (video_recorder != null) {
                    surfaces_list.add(video_recorder_surface);
                    if (supports_photo_video_recording && !want_video_high_speed && want_photo_video_recording) {
                        surfaces_list.add(imageReader.getSurface());
                    }
                    // n.b., raw not supported for photo snapshots while video recording
                } else if (want_video_high_speed) {
                    // future proofing - at the time of writing want_video_high_speed is only set
                    // when recording video,
                    // but if ever this is changed, can only support the preview_surface as a target
                    // also OMT not supported in high speed
                } else if (imageReaderRaw != null) {
                    surfaces_list.add(imageReader.getSurface());
                    surfaces_list.add(imageReaderRaw.getSurface());
                } else {
                    surfaces_list.add(imageReader.getSurface());
                }

                // Add OMT streaming surface if available (and not high speed)
                if (omt_streaming_surface != null && !want_video_high_speed) {
                    if (MyDebug.LOG)
                        Log.d(TAG, "adding omt_streaming_surface: " + omt_streaming_surface);
                    surfaces_list.add(omt_streaming_surface);
                }
            ...
            if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    ...
                    launchCameraSession(wait_until_started, new CreateCaptureSessionFunction() {
                        ...
                        public void call() throws CameraAccessException {
                            if (camera == null) {
                                // just in case - don't throw exception as we don't want to show error toast, as
                                // it may be that another request to start preview is already active
            ...
            } else if (video_recorder != null && want_video_high_speed
                    && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if ((cameraIdSPhysical != null || want_jpeg_r) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    ...
                    SessionConfiguration sessionConfiguration = new SessionConfiguration(
                            SessionConfiguration.SESSION_HIGH_SPEED, outputs, executor, myStateCallback);
                    launchCameraSession(wait_until_started, new CreateCaptureSessionFunction() {
                        @Override
                        public void call() throws CameraAccessException {
                            if (camera == null) {
                ...
                try {
                    if ((cameraIdSPhysical != null || want_jpeg_r) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                         ...
                         */
                        SessionConfiguration sessionConfiguration = new SessionConfiguration(
                                SessionConfiguration.SESSION_REGULAR, outputs, executor, myStateCallback);
                        launchCameraSession(wait_until_started, new CreateCaptureSessionFunction() {
                            @Override
                            public void call() throws CameraAccessException {
                                if (camera == null) {
                    ...
                    // have had this from some devices on Google Play, from deep within
                    // createCaptureSession
                    // note, we put the catch here rather than below, so as to not mask
            ...
            if (wait_until_started) {
                ...
                // n.b., we use the background_camera_lock lock instead of a separate lock, so
                // that it's safe to call this
                // method under the background_camera_lock (if we did so but used a separate
                // lock, we'd hang here, because
                // MyStateCallback.onConfigured() needs to lock on background_camera_lock,
                // before it completes and sets
            ...
            // have had crashes from Google Play, from both
            // createConstrainedHighSpeedCaptureSession and
            // createCaptureSession
    ...
    @Override
    public void startPreview(boolean wait_until_started, Runnable runnable, Runnable on_failed)
            throws CameraControllerException {
        if (MyDebug.LOG)
            Log.d(TAG, "startPreview");

        if (!camera_settings.has_af_mode && initial_focus_mode != null
                && sessionType != SessionType.SESSIONTYPE_EXTENSION) {
            if (MyDebug.LOG)
                Log.d(TAG, "user didn't specify focus, so set to: " + initial_focus_mode);
            // If the caller hasn't set a focus mode, but focus modes are supported, it's
            // still better to explicitly set one rather than leaving to the
            // builder's default - e.g., problem on Android emulator with LIMITED camera
            // where it only supported infinity focus (CONTROL_AF_MODE_OFF), but
            // the preview builder defaults to CONTROL_AF_MODE_CONTINUOUS_PICTURE! This
            // meant we froze when trying to take a photo, because we thought
            // we were in continuous picture mode and so waited in state
            // STATE_WAITING_AUTOFOCUS, but the focus never occurred.
            // Ideally the caller to CameraController2 (Preview) should always explicitly
            // set a focus mode if at least 1 focus mode is supported. At the
            // time of writing, Preview only sets a focus if at least 2 focus modes are
            // supported. But even if we fix that in future, still good to have
        ...
        synchronized (background_camera_lock) {
            if (hasCaptureSession()) {
                try {
                    ...
                    MyDebug.logStackTrace(TAG, "failed to start preview", e);
                    // do via CameraControllerException instead of preview_error_cb, so caller
                    // immediately knows preview has failed
                    throw new CameraControllerException();
                }
                if (runnable != null) {
                    runnable.run();
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                ...
                try {
                    ...
                    // we still call close() below, as it has no effect if captureSession is already
                    // closed
                }
                if (close_capture_session) {
                    // although stopRepeating() alone will pause the preview, seems better to close
                    // captureSession altogether - this allows the app to make changes such as
            ...
            // simulate CameraController1 behaviour where face detection is stopped when we
            // stop preview
            if (camera_settings.has_face_detect_mode && close_capture_session) {
                ...
                // no need to call setRepeatingRequest(), we're just setting the camera_settings
                // for when we restart the preview
            }
        }
    }
    ...
    public boolean startFaceDetection() {
        ...
        BLOCK_FOR_EXTENSIONS();
        if (previewBuilder.get(CaptureRequest.STATISTICS_FACE_DETECT_MODE) != null && previewBuilder
                .get(CaptureRequest.STATISTICS_FACE_DETECT_MODE) != CaptureRequest.STATISTICS_FACE_DETECT_MODE_OFF) {
            if (MyDebug.LOG)
    ...
    /*
     * If do_af_trigger_for_continuous is false, doing an autoFocus() in continuous
     * focus mode just
     * means we call the autofocus callback the moment focus is not scanning (as
     * with old Camera API).
     * If do_af_trigger_for_continuous is true, we set CONTROL_AF_TRIGGER_START, and
     * wait for
     * CONTROL_AF_STATE_FOCUSED_LOCKED or CONTROL_AF_STATE_NOT_FOCUSED_LOCKED,
     * similar to other focus
     * methods.
     * do_af_trigger_for_continuous==true used to have advantages:
     * - On Nexus 6 for flash auto, it means ae state is set to FLASH_REQUIRED if it
     * is required
     * when it comes to taking the photo. If do_af_trigger_for_continuous==false,
     * sometimes
     * it's set to CONTROL_AE_STATE_CONVERGED even for dark scenes, so we think we
     * can skip
     * the precapture, causing photos to come out dark (or we can force always doing
     * precapture,
     * but that makes things slower when flash isn't needed)
     * Update: this now seems hard to reproduce.
     * - On OnePlus 3T, with do_af_trigger_for_continuous==false photos come out
     * with blue tinge
     * if the scene is not dark (but still dark enough that you'd want flash).
     * do_af_trigger_for_continuous==true fixes this for cases where the flash fires
     * for autofocus.
     * Note that the problem is still not fixed for flash on where the scene is
     * bright enough to
     ...
     * - On both Nexus 6 and OnePlus 3T, taking photos with flash is longer, as we
     * have flash firing
     * for autofocus and precapture. Though note this is the case with autofocus
     * mode anyway.
     * Note for fake flash mode, we still can use
     * do_af_trigger_for_continuous==false (and doing the
     * af trigger for fake flash mode can sometimes mean flash fires for too long
     * and we get a worse
     * result).
     */
    ...
    public void autoFocus(final AutoFocusCallback cb, boolean capture_follows_autofocus_hint) {
        ...
        synchronized (background_camera_lock) {
            ...
            if (camera == null || !hasCaptureSession()) {
                if (MyDebug.LOG)
                    Log.d(TAG, "no camera or capture session");
                // should call the callback, so the application isn't left waiting (e.g., when
                // we autofocus before trying to take a photo)
            ...
            if (focus_mode == null) {
                // we preserve the old Camera API where calling autoFocus() on a device without
                // autofocus immediately calls the callback
                // (unclear if Open Camera needs this, but just to be safe and consistent
                // between camera APIs)
                if (MyDebug.LOG)
                ...
                // CONTROL_AF_TRIGGER_IDLE/CONTROL_AF_TRIGGER_START not supported for high speed
                // video
                cb.onAutoFocus(true);
                return;
            }
            ...
            if (MyDebug.LOG) {
                {
                    ...
                    for (int i = 0; areas != null && i < areas.length; i++) {
                        Log.d(TAG,
                                i + " focus area: " + areas[i].getX() + " , " + areas[i].getY() + " : "
                                        + areas[i].getWidth() + " x " + areas[i].getHeight() + " weight "
                ...
                }
                {
                    ...
                    for (int i = 0; areas != null && i < areas.length; i++) {
                        Log.d(TAG,
                                i + " metering area: " + areas[i].getX() + " , " + areas[i].getY() + " : "
                                        + areas[i].getWidth() + " x " + areas[i].getHeight() + " weight "
                                        + areas[i].getMeteringWeight());
                    }
                }
            ...
            try {
                if (use_fake_precapture_mode) {
                    ...
                    if (camera_settings.flash_value.equals("flash_auto")
                        ...
                        // calling fireAutoFlash() also caches the decision on whether to flash -
                        // otherwise if the flash fires now, we'll then think the scene is bright enough
                        // to not need the flash!
                        if (fireAutoFlash())
                            want_flash = true;
                    ...
                    if (want_flash) {
                        ...
                        if (!camera_settings.has_iso) {
                            // in auto-mode, need to ensure CONTROL_AE_MODE isn't est to flash auto/on for
                            // torch to work
                        ...
                        // We sleep for a short time as on some devices (e.g., OnePlus 3T), the torch
                        // will turn off when autofocus
                        // completes even if we don't want that (because we'll be taking a photo).
                        // Note that on other devices such as Nexus 6, this problem doesn't occur even
                        // if we don't have a separate
                        // setRepeatingRequest.
                        // Update for 1.37: now we do need this for Nexus 6 too, after switching to
                        // setting CONTROL_AE_MODE_ON_AUTO_FLASH
                        // or CONTROL_AE_MODE_ON_ALWAYS_FLASH even for fake flash (see note in
                        // CameraSettings.setAEMode()) - and we
                        // needed to increase to 200ms! Otherwise photos come out too dark for flash on
                        // if doing touch to focus then
                ...
                // Google Camera sets to idle with a repeating request, then sets af trigger to
                ...
                // got IllegalStateException as a Google Play exception - this means the capture
            ...
            afBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER, CameraMetadata.CONTROL_AF_TRIGGER_IDLE); // ensure set back
                                                                                                      // to idle
        }
    ...
    public void cancelAutoFocus() {
        ...
        synchronized (background_camera_lock) {
            ...
            // Camera2Basic does a capture then sets a repeating request - do the same here
            // just to be safe
            try {
                capture();
            } catch (CameraAccessException e) {
                ...
                // got this as a Google Play exception - this means the capture session is
                // already closed
            }
    ...
    /**
     * Whether the stillRequest has a manual exposure time different to the preview,
     * and if so,
     * whether we first need to set the preview exposure to match (needed for
     * Samsung Galaxy devices,
     * which don't honor a manual exposure that's different to the current preview
     * exposure).
     */
    ...
    /**
     * Adjusts the preview's manual exposure to match the stillRequest's manual
     * exposure. Should only
     * be called if adjustPreview() returns true.
     * We use RUN_POST_CAPTURE, so we can be sure that the request to adjust the
     * preview's exposure has
     * completed.
     */
    private void adjustPreviewToStill(CaptureRequest stillRequest, PostCapture post_capture)
    ...
    private void takePictureAfterPrecapture() {
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                ...
                // important to use TEMPLATE_MANUAL for manual exposure: this fixes bug on Pixel
                // 6 Pro where manual exposure is ignored when longer than the
                // preview exposure time (oddly Galaxy S10e has the same bug since Android 11,
                // but that isn't fixed with using TEMPLATE_MANUAL)
                stillBuilder = camera.createCaptureRequest(previewIsVideoMode ? CameraDevice.TEMPLATE_VIDEO_SNAPSHOT
                        : camera_settings.has_iso ? CameraDevice.TEMPLATE_MANUAL : CameraDevice.TEMPLATE_STILL_CAPTURE);
                ...
                // Versions previous to 1.51 would switch to manual mode and underexpose in
                // bright scenes; however on more modern devices such as Samsung and
                // Pixels, this means that we lose the benefit of manufacturer algorithms
                // creating a worse result. So we're better off staying in auto mode.
                // (Even on old versions, we didn't do this on OnePlus devices due to OnePlus 3T
                // having preview corruption / camera freezing problems when
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
                    ...
                    // this is also required to enable HDR+ on Google Pixel devices when using
                    // Camera2: https://opensource.google.com/projects/pixelvisualcorecamera
                    // but don't set for extension sessions (in theory it should be ignored, but
                    // just in case)
                ...
                burst_single_request = false;
                if (!previewIsVideoMode) {
                    // need to stop preview before capture (as done in Camera2Basic; otherwise we
                    // get bugs such as flash remaining on after taking a photo with flash)
                    // but don't do this in video mode - if we're taking photo snapshots while video
                    // recording, we don't want to pause video!
                    // update: bug with flash may have been device specific (things are fine with
                    // Nokia 8)
                    if (sessionType != SessionType.SESSIONTYPE_EXTENSION)
                        captureSession.stopRepeating();
                }
            ...
            synchronized (background_camera_lock) {
                ...
                try {
                    ...
                    if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                        ...
                        // captureSession.capture(stillBuilder.build(), new
                        // CameraCaptureSession.CaptureCallback() {
                        // }, handler);
                    }
                    playSound(shutter_click_sound); // play shutter sound asap, otherwise user has the illusion of being
                                                    // slow to take photos
                } catch (CameraAccessException e) {
                    MyDebug.logStackTrace(TAG, "failed to take picture", e);
    ...
    public static List<Float> setupFocusBracketingDistances(float source, float target, int count) {
        ...
        focus_distance_s = Math.max(focus_distance_s, max_focus_bracket_distance_c); // since we'll dealing with
                                                                                     // 1/distance, use Math.max
        focus_distance_e = Math.max(focus_distance_e, max_focus_bracket_distance_c); // since we'll dealing with
                                                                                     // 1/distance, use Math.max
        if (MyDebug.LOG) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // for first and last, we still use the real focus distances; for intermediate
            // values, we interpolate
            ...
            if (i == 0) {
                ...
                // rather than linear interpolation, we use log, see
                ...
                if (real_focus_distance_s > real_focus_distance_e) {
                    // if source is further than target, we still want the interpolation distances
                    // to be the same, but in reversed order
                    value = count - 1 - i;
                }
    ...
    private void takePictureBurstBracketing() {
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                ...
                CaptureRequest.Builder stillBuilder = camera
                        .createCaptureRequest((burst_type == BurstType.BURSTTYPE_EXPO || camera_settings.has_iso)
                                ? CameraDevice.TEMPLATE_MANUAL
                                : CameraDevice.TEMPLATE_STILL_CAPTURE);
                // Needs to be TEMPLATE_MANUAL! Otherwise first image in burst may come out
                // incorrectly (on Pixel 6 Pro,
                // the first image incorrectly had HDR+ applied, which we don't want here). Also
                // problem on Pixel 6 Pro
                // where manual exposure is ignored when longer than the preview exposure.
                // Update: but only when doing burst for expo bracketing, not focus bracketing
                // (unless actually doing that
                // in manual mode)! (Only manual exposure should use TEMPLATE_MANUAL, otherwise
                // focus bracketing images
                // come out underexposed on Pixel 6 Pro).
                // n.b., don't set RequestTagType.CAPTURE here - we only do it for the last of
                // the burst captures (see below)
                camera_settings.setupBuilder(stillBuilder, true);
                ...
                clearPending();
                // shouldn't add preview surface as a target - see note in
                // takePictureAfterPrecapture()
                // but also, adding the preview surface causes the dark/light exposures to be
                // visible, which we don't want
                stillBuilder.addTarget(imageReader.getSurface());
                if (raw_todo)
                    stillBuilder.addTarget(imageReaderRaw.getSurface());
                ...
                if (burst_type == BurstType.BURSTTYPE_EXPO) {
                    ...
                    if (iso_range == null) {
                        ...
                        // set ISO
                        int iso = 800;
                        // obtain current ISO/etc settings from the capture result - but if we're in
                        // manual ISO mode,
                        // might as well use the settings the user has actually requested (also useful
                        // for workaround for
                        // OnePlus 3T bug where the reported ISO and exposure_time are wrong in dark
                        // scenes)
                        if (camera_settings.has_iso)
                            iso = camera_settings.iso;
                        else if (capture_result_has_iso)
                            iso = capture_result_iso;
                        // see https://sourceforge.net/p/opencamera/tickets/321/ - some devices may have
                        // auto ISO that's
                    ...
                    for (int i = 0; i < n_half_images; i++) {
                        ...
                        if (supports_exposure_time) {
                            ...
                            // it's intentional that we divide long by double, and implicitly cast back to
                            // long
                            // (the suggested solution is to first cast the double to long before dividing,
                            // which
                            // we don't want)
                    ...
                    for (int i = 0; i < n_half_images; i++) {
                        ...
                        if (supports_exposure_time) {
                            ...
                            for (int j = 0; j < i; j++)
                                this_scale *= scale;
                            // warning "Possibly lossy implicit cast in compound assignment" suppressed:
                            // it's intentional that we multiply long by double, and implicitly cast back to
                            // long
                            // (the suggested solution is to first cast the double to long before
                            // multiplying, which
                            // we don't want)
                            // noinspection lossy-conversions
                            exposure_time *= this_scale;
                            ...
                            if (i == n_half_images - 1) {
                                // RequestTagType.CAPTURE should only be set for the last request, otherwise
                                // we'll may do things like turning
                                // off torch (for fake flash) before all images are received
                                // More generally, doesn't seem a good idea to be doing the post-capture
                                // commands (resetting ae state etc)
                    ...
                    stillBuilder.set(CaptureRequest.CONTROL_AF_MODE, CameraMetadata.CONTROL_AF_MODE_OFF); // just in
                                                                                                          // case
                    ...
                    List<Float> focus_distances = setupFocusBracketingDistances(focus_bracketing_source_distance,
                            focus_bracketing_target_distance, focus_bracketing_n_images);
                    ...
                    for (int i = 0; i < focus_distances.size(); i++) {
                        ...
                        // stillBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE,
                        // focus_distances.get(focus_distances.size()-1));
                        if (i == focus_distances.size() - 1) {
                            stillBuilder.setTag(new RequestTagObject(RequestTagType.CAPTURE)); // set capture tag for
                                                                                               // last only
                        } else {
                            // note, even if we didn't need to set CAPTURE_BURST_IN_PROGRESS, we'd still
                            // want
                    ...
                    burst_single_request = false; // we set to false for focus bracketing, as we support bracketing with
                                                  // large numbers of images in this mode
                    // burst_single_request = true; // test
                }

                /*
                 * // testing:
                 * stillBuilder.setTag(new
                 * RequestTagObject(RequestTagType.CAPTURE_BURST_IN_PROGRESS));
                 * requests.add( stillBuilder.build() );
                 * stillBuilder.setTag(new
                 * RequestTagObject(RequestTagType.CAPTURE_BURST_IN_PROGRESS));
                 * requests.add( stillBuilder.build() );
        ...
        if (ok) {
            synchronized (background_camera_lock) {
                ...
                try {
                    ...
                    if (use_expo_fast_burst && burst_type == BurstType.BURSTTYPE_EXPO) { // always use slow burst for
                                                                                         // focus bracketing
                        if (MyDebug.LOG)
                        ...
                        if (burst_type == BurstType.BURSTTYPE_EXPO) {
                            // Set preview to match - some devices (e.g. Samsung Galaxy) don't produce
                            // photos with correct exposure if
                            // the exposure is set to a different value than the preview.
                            // Although we don't/can't do this for fast burst, doing so here means such
                            // devices can use HDR/expo when
                            // using use_expo_fast_burst==false.
                            try {
                                previewBuilder.set(CaptureRequest.CONTROL_AE_MODE, CameraMetadata.CONTROL_AE_MODE_OFF);
                                previewBuilder.set(CaptureRequest.SENSOR_SENSITIVITY, slow_burst_capture_requests
                                        .get(n_burst_taken).get(CaptureRequest.SENSOR_SENSITIVITY));
                                previewBuilder.set(CaptureRequest.SENSOR_FRAME_DURATION, slow_burst_capture_requests
                                        .get(n_burst_taken).get(CaptureRequest.SENSOR_FRAME_DURATION));
                            ...
                            // no need to set preview for first focus bracketing shot, the first focus
                            // bracketing always
    ...
    private void doTakePhotoBurst(CaptureRequest request, CaptureRequest last_request) throws CameraAccessException {
        if (burst_type == BurstType.BURSTTYPE_CONTINUOUS) {
            ...
            captureSession.capture(continuous_burst_in_progress ? request : last_request, previewCaptureCallback,
                    handler);
            ...
            if (continuous_burst_in_progress) {
                ...
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        // note, even if continuous_burst_in_progress has become false by this point,
                        // still take one last
                        // photo, as need to ensure that we have a request with RequestTagType.CAPTURE,
                        // as well as ensuring
                        ...
                        if (n_burst >= 10 || n_burst_raw >= 10) {
                            // Nokia 8 in std mode without post-processing options doesn't hit this limit
                            // (we only hit this
    ...
    private void takePictureBurst(boolean continuing_fast_burst) {
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                ...
                CaptureRequest.Builder stillBuilder = camera.createCaptureRequest(previewIsVideoMode
                        ? CameraDevice.TEMPLATE_VIDEO_SNAPSHOT
                        : camera_settings.has_iso ? CameraDevice.TEMPLATE_MANUAL : CameraDevice.TEMPLATE_STILL_CAPTURE);
                // N.B., takePictureBurst() not currently called if previewIsVideoMode==true,
                // but have put this code here for possible future use.
                // Important to use TEMPLATE_MANUAL for manual exposure: this fixes bug on Pixel
                // 6 Pro where manual exposure is ignored when longer than the
                ...
                if (!is_samsung && burst_type == BurstType.BURSTTYPE_NORMAL && burst_for_noise_reduction) {
                    // Must be done after calling setupBuilder(), so we override the default
                    // EDGE_MODE and NOISE_REDUCTION_MODE.
                    // We disable noise-reduction etc for photo mode NR because on many devices this
                    // smears out detail that we actually
                    // aim to recover by averaging a stack of multiple images.
                    // Disabled for Samsung - firstly at least on Galaxy S24+ this has no effect
                    // except for unstable situations (e.g.,
                    // if Ultra HDR / JPEG_R is enabled then switching from STD to NR mode means
                    // this works for some reason, even though we
                    // don't enable JPEG_R for NR mode...). We could fix it by also changing for the
                    // preview, although this makes the
                    // code more complicated (we'd need to save the old values, and also avoid
                    // interactions with setNoiseReductionMode() and
                    // setEdgeMode()). But Galaxy S24+ at least seems to have better noise reduction
                    // such that detail is less likely to be
                    // smeared out, and overall quality of photos in NR mode seems better if run
                    // noise reduction etc as normal.
                    if (MyDebug.LOG)
                    ...
                    stillBuilder.set(CaptureRequest.COLOR_CORRECTION_ABERRATION_MODE,
                            CaptureRequest.COLOR_CORRECTION_ABERRATION_MODE_OFF);
                ...
                if (burst_type == BurstType.BURSTTYPE_CONTINUOUS) {
                    ...
                    if (continuing_fast_burst) {
                        ...
                        is_new_burst = false;
                        /*
                         * if( !continuous_burst_in_progress ) // test bug where we call callback
                         * onCompleted() before all burst images are received
                         * n_burst = 1;
                         */
                    } else {
                        if (MyDebug.LOG)
                    ...
                    if (capture_result_has_iso && capture_result_has_exposure_time) {
                        if (HDRProcessor.sceneIsLowLight(capture_result_iso, capture_result_exposure_time)) {
                            ...
                            // Versions previous to 1.51 would switch to manual mode and overexpose in
                            // bright scenes; however on more modern devices such as Samsung and
                            // Pixels, this means that we lose the benefit of manufacturer algorithms
                            // creating a worse result. So we're better off staying in auto mode.
                            // (Even on old versions, we didn't do this for OnePlus devices, due to bug on
                            // OnePlus 3T where manual mode can't be set above 800, so this
                            ...
                            if (exposure_time <= fixed_exposure_time) {
                                ...
                                n_burst = 3;
                                // Versions previous to 1.51 would switch to manual mode and underexpose in
                                // bright scenes; however on more modern devices such as
                                // Samsung and Pixels, this means that we lose the benefit of manufacturer
                                // algorithms creating a worse result. So we're better off
        ...
        if (ok) {
            synchronized (background_camera_lock) {
                ...
                try {
                    ...
                    if (!continuing_fast_burst) {
                        playSound(shutter_click_sound); // play shutter sound asap, otherwise user has the illusion of
                                                        // being slow to take photos
                    }
                } catch (CameraAccessException e) {
                    MyDebug.logStackTrace(TAG, "failed to take picture burst", e);
                    // noinspection UnusedAssignment
    ...
    private void runPrecapture() {
        ...
        synchronized (background_camera_lock) {
            ...
            try {
                // Use a separate builder for precapture - otherwise have problem that if we
                // take photo with flash auto/on of dark scene, then point to a bright scene,
                // the autoexposure isn't running until we autofocus again.
                // Important that this is TEMPLATE_PREVIEW not TEMPLATE_STILL_CAPTURE, otherwise
                // we have various problems with flash:
                ...
                final CaptureRequest.Builder precaptureBuilder = camera
                ...
                // STATE_WAITING_PRECAPTURE_START, because precapture already occurred whilst
                // autofocusing, and it doesn't occur again unless we first set the precapture
                // trigger to idle
                if (MyDebug.LOG)
        ...
        synchronized (background_camera_lock) {
            switch (camera_settings.flash_value) {
                ...
                default:
                    if (MyDebug.LOG)
                        Log.e(TAG,
                                "runFakePrecapture called with unexpected flash value: " + camera_settings.flash_value);
                    break;
            }
        }
    ...
    private boolean fireAutoFlashFrontScreen() {
        // iso_threshold fine-tuned for Nexus 6 - front camera ISO never goes above 805,
        // but a threshold of 700 is too low
        final int iso_threshold = 750;
        return capture_result_has_iso && capture_result_iso >= iso_threshold;
    }

    /**
     * Used in use_fake_precapture mode when flash is auto, this returns whether we
     * fire the flash.
     * If the decision was recently calculated, we return that same decision - used
     * to fix problem that if
     * we fire flash during autofocus (for autofocus mode), we don't then want to
     * decide the scene is too
    ...
    private boolean fireAutoFlash() {
        ...
        final long cache_time_ms = 3000; // needs to be at least the time of a typical autoflash, see comment for this
                                         // function above
        ...
        // We only cache the result if we decide to turn on torch, as that mucks up our
        // ability to tell if we need the flash (since once the torch
        // is on, the ae_state thinks it's bright enough to not need flash!)
        // But if we don't turn on torch, this problem doesn't occur, so no need to
        // cache - and good that the next time we should make an up-to-date
        // decision.
    ...
    public void takePicture(final PictureCallback picture, final ErrorCallback error) {
        ...
        synchronized (background_camera_lock) {
...

            {
                ...
                if (sessionType == SessionType.SESSIONTYPE_EXTENSION) {
                ...
                } else if (camera_settings.flash_value.equals("flash_off")
                        || camera_settings.flash_value.equals("flash_torch")
                        || camera_settings.flash_value.equals("flash_frontscreen_torch")) {
                    // Don't need precapture if flash off or torch
                    call_takePictureAfterPrecapture = true;
                } else if (use_fake_precapture_mode) {
                    // fake flash auto/on mode
                    // fake precapture works by turning on torch (or using a "front screen flash"),
                    // so we can't use the camera's own decision for flash auto
                    // instead we check the current ISO value
                    boolean auto_flash = camera_settings.flash_value.equals("flash_auto")
                            || camera_settings.flash_value.equals("flash_frontscreen_auto");
                    ...
                    if (auto_flash && !fireAutoFlash()) {
                        ...
                        // On some devices (e.g., OnePlus 3T), if we've already turned on torch for an
                        // autofocus immediately before
                        // taking the photo, ae convergence may have already occurred - so if we called
                        // runFakePrecapture(), we'd just get
                        // stuck waiting for CONTROL_AE_STATE_SEARCHING which will never happen, until
                        // we hit the timeout - it works,
                        ...
                        // (Note, we don't want to go straight to takePictureAfterPrecapture(), as it
                        // might be that ae scanning is still
                        // taking place.)
                        // An alternative solution would be to switch torch off and back on again to
                        // cause ae scanning to start - but
                        // at worst this is tricky to get working, and at best, taking photos would be
                        // slower.
                    ...
                    // note that we don't call needsFlash() (or use is_flash_required) - as if ae
                    // state is neither CONVERGED nor FLASH_REQUIRED, we err on the side
                    // of caution and don't skip the precapture
                    // boolean needs_flash = capture_result_ae != null && capture_result_ae ==
                    // CaptureResult.CONTROL_AE_STATE_FLASH_REQUIRED;
                    boolean needs_flash = capture_result_ae != null
                            && capture_result_ae != CaptureResult.CONTROL_AE_STATE_CONVERGED;
                    if (camera_settings.flash_value.equals("flash_auto") && !needs_flash) {
                        // if we call precapture anyway, flash wouldn't fire - but we tend to have a
                        // pause
        ...
        // important to call functions outside of locks, so that they can in turn call
    ...
    public void setDisplayOrientation(int degrees) {
        // for CameraController2, the preview display orientation is handled via the
        // TextureView's transform
        if (MyDebug.LOG)
    ...
    public int getCameraOrientation() {
        // cached for performance, as this method is frequently called from
        // Preview.onOrientationChanged
    ...
    public void initVideoRecorderPrePrepare(MediaRecorder video_recorder) {
        // if we change where we play the START_VIDEO_RECORDING sound, make sure it
        // can't be heard in resultant video
        BLOCK_FOR_EXTENSIONS(); // not supported for extension sessions
    ...
    public void initVideoRecorderPostPrepare(MediaRecorder video_recorder, boolean want_photo_video_recording)
        ...
        try {
            ...
            /*
             * if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE ) {
             * previewBuilder.set(CaptureRequest.CONTROL_AUTOFRAMING,
             * CaptureRequest.CONTROL_AUTOFRAMING_ON); // test
             * }
             */
    ...
    public void reconnect() throws CameraControllerException {
        if (MyDebug.LOG)
            Log.d(TAG, "reconnect");
        // if we change where we play the STOP_VIDEO_RECORDING sound, make sure it can't
        // be heard in resultant video
    ...
    public int captureResultWhiteBalanceTemperature() {
        // for performance reasons, we don't convert from rggb to temperature in every
        // frame, rather only when requested
    ...
    private class MyExtensionCaptureCallback extends CameraExtensionSession.ExtensionCaptureCallback {
        ...
        public void onCaptureStarted(@NonNull CameraExtensionSession session,
            ...
            if (MyDebug.LOG) {
                ...
                } else if (previewCaptureCallback
                        .getRequestTagType(request) == RequestTagType.CAPTURE_BURST_IN_PROGRESS) {
                    Log.d(TAG, "onCaptureStarted: capture burst in progress");
                }
            }

            // for previewCaptureCallback, we set has_received_frame in
            // onCaptureCompleted(), but
            // that method doesn't exist for ExtensionCaptureCallback, and the other methods
            // such as
            // onCaptureSequenceCompleted aren't called for the preview captures;
            // onCaptureResultAvailable meanwhile is only called if
            // CameraExtensionCharacteristics.getAvailableCaptureResultKeys() returns a
            // non-empty
            // list
        ...
        public void onCaptureSequenceCompleted(@NonNull CameraExtensionSession session,
            ...
            // RequestTagType.CAPTURE - but this method should only be called for photo
            // captures
            // anyway
        ...
        @Override
        public void onCaptureResultAvailable(@NonNull CameraExtensionSession session, @NonNull CaptureRequest request,
    ...
    private class MyCaptureCallback extends CameraCaptureSession.CaptureCallback {
        ...
        public void onCaptureFailed(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request,
            ...
            super.onCaptureFailed(session, request, failure); // API docs say this does nothing, but call it just to be
                                                              // safe
        }
        ...
        public void onCaptureSequenceAborted(@NonNull CameraCaptureSession session, int sequenceId) {
            ...
            super.onCaptureSequenceAborted(session, sequenceId); // API docs say this does nothing, but call it just to
                                                                 // be safe
        }
        ...
        public void onCaptureSequenceCompleted(@NonNull CameraCaptureSession session, int sequenceId,
            ...
            super.onCaptureSequenceCompleted(session, sequenceId, frameNumber); // API docs say this does nothing, but
                                                                                // call it just to be safe
        }
        ...
        public void onCaptureStarted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request,
            ...
            // n.b., we don't play the shutter sound here for RequestTagType.CAPTURE, as it
            // typically sounds "too late"
            // (if ever we changed this, would also need to fix for burst, where we only set
            // the RequestTagType.CAPTURE for the last image)
        ...
        public void onCaptureProgressed(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request,
            ...
            // Note that we shouldn't try to process partial results - or if in future we
            // decide to, remember that it's documented that
            // not all results may be available. E.g., OnePlus 3T on Android 7 (OxygenOS
            // 4.0.2) reports null for AF_STATE from this method.
            // We'd also need to fix up the discarding of old frames in process(), as we
            // probably don't want to be discarding the
            // complete results from onCaptureCompleted()!
            super.onCaptureProgressed(session, request, partialResult); // API docs say this does nothing, but call it
                                                                        // just to be safe (as with Google Camera)
        }
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request,
            ...
            super.onCaptureCompleted(session, request, result); // API docs say this does nothing, but call it just to
                                                                // be safe (as with Google Camera)
        }

        /**
         * Updates cached information regarding the capture result status related to
         * auto-exposure.
         */
        private void updateCachedAECaptureStatus(CaptureResult result) {
            ...
            if (use_fake_precapture_mode && (fake_precapture_torch_focus_performed || fake_precapture_torch_performed)
                    && flash_mode != null && flash_mode == CameraMetadata.FLASH_MODE_TORCH) {
                ...
                // capture_result_ae should always be non-null here, as we've already handled
                // ae_state separately
        ...
        private void handleStateChange(CaptureRequest request, CaptureResult result) {
            // use Integer instead of int, so can compare to null: Google Play crashes
            // confirmed that this can happen; Google Camera also ignores cases with null af
            // state
             ...
             * }
             */
            // CONTROL_AE_STATE can be null on some devices, so as with af_state, use
            // Integer
            Integer ae_state = result.get(CaptureResult.CONTROL_AE_STATE);
            /*
            ...
            boolean autofocus_timeout = autofocus_time_ms != -1
                    && System.currentTimeMillis() > autofocus_time_ms + autofocus_timeout_c;
            ...
            if (af_state != null && af_state == CaptureResult.CONTROL_AF_STATE_PASSIVE_SCAN && !autofocus_timeout) {
                ...
                if (autofocus_cb != null && (!do_af_trigger_for_continuous || use_fake_precapture_mode)
                        && focusIsContinuous()) {
                    Integer focus_mode = previewBuilder.get(CaptureRequest.CONTROL_AF_MODE);
                    if (focus_mode != null && focus_mode == CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE) {
                        ...
                        // need to check af_state != null, I received Google Play crash in 1.33 where it
                        // was null
                        boolean focus_success = af_state != null
            ...
            if (state == STATE_NORMAL) {
                ...
                if (af_state == null) {
                    // autofocus shouldn't really be requested if af not available, but still allow
                    // this rather than getting stuck waiting for autofocus to complete
                    if (MyDebug.LOG)
                    ...
                    if (autofocus_timeout || af_state == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED
                        ...
                        if (use_fake_precapture_mode && fake_precapture_torch_focus_performed) {
                            ...
                            if (!capture_follows_autofocus_hint) {
                                ...
                                try {
                                    ...
                                    MyDebug.logStackTrace(TAG, "failed to do capture to turn off torch after autofocus",
                                            e);
                                }
                                ...
                                try {
                                    ...
                                    MyDebug.logStackTrace(TAG,
                                            "failed to set repeating request to turn off torch after autofocus", e);
                                }
                            } else {
                                if (MyDebug.LOG)
                                    Log.d(TAG,
                ...
                if (ae_state == null || ae_state == CaptureResult.CONTROL_AE_STATE_PRECAPTURE /*
                    ...
                    // results with flash:
                    // varying levels of brightness, sometimes too bright or too dark, sometimes
                    // with blue tinge, sometimes even with green corruption
                    // similarly photos with flash come out too dark on OnePlus 3T
                    if (MyDebug.LOG) {
                        Log.d(TAG, "precapture started after: "
                    ...
                    precapture_state_change_time_ms = System.currentTimeMillis();
                        ...
                        && System.currentTimeMillis() - precapture_state_change_time_ms > precapture_start_timeout_c) {
                    // hack - give up waiting - sometimes we never get a CONTROL_AE_STATE_PRECAPTURE
                    // so would end up stuck
                    // always log error, so we can look for it when manually testing with logging
                    // disabled
                ...
                if (ae_state == null || ae_state != CaptureResult.CONTROL_AE_STATE_PRECAPTURE) {
                    if (MyDebug.LOG) {
                        Log.d(TAG, "precapture completed after: "
                                + (System.currentTimeMillis() - precapture_state_change_time_ms));
                    }
                    state = STATE_NORMAL;
                    precapture_state_change_time_ms = -1;
                    takePictureAfterPrecapture();
                        ...
                        && System.currentTimeMillis() - precapture_state_change_time_ms > precapture_done_timeout_c) {
                    // just in case
                    // always log error, so we can look for it when manually testing with logging
                    // disabled
                ...
                if (fake_precapture_turn_on_torch_id == null
                    ...
                    if (MyDebug.LOG) {
                        Log.d(TAG, "fake precapture started after: "
                                + (System.currentTimeMillis() - precapture_state_change_time_ms));
                    }
                    state = STATE_WAITING_FAKE_PRECAPTURE_DONE;
                    precapture_state_change_time_ms = System.currentTimeMillis();
                } else if (fake_precapture_turn_on_torch_id == null && camera_settings.has_iso
                        && precapture_state_change_time_ms != -1
                        && System.currentTimeMillis() - precapture_state_change_time_ms > 100) {
                    // When using manual ISO, we can't make use of changes to the ae_state - but at
                    // the same time, we don't
                    // need ISO/exposure to re-adjust anyway.
                    // If fake_precapture_turn_on_torch_id != null, we still wait for the physical
                    // torch to turn on.
                    // But if fake_precapture_turn_on_torch_id==null (i.e., for
                    // flash_frontscreen_torch), just wait a short
                ...
                // wait for af and ae scanning to end (need to check af too, as in continuous
                // focus mode, a focus may start again after switching torch on for the fake
                // precapture)
                if (ready_for_capture && (ae_state == null || ae_state != CaptureResult.CONTROL_AE_STATE_SEARCHING)) {
                    if (MyDebug.LOG) {
                        Log.d(TAG, "fake precapture completed after: "
                                + (System.currentTimeMillis() - precapture_state_change_time_ms));
                    }
                    state = STATE_NORMAL;
                    precapture_state_change_time_ms = -1;
                    takePictureAfterPrecapture();
                        ...
                        && System.currentTimeMillis() - precapture_state_change_time_ms > precapture_done_timeout_c) {
                    // sometimes camera can take a while to stop ae/af scanning, better to just go
                    // ahead and take photo
                    // always log error, so we can look for it when manually testing with logging
                    // disabled
        ...
        private void handleContinuousFocusMove(CaptureResult result) {
            ...
            } else if (af_state != null && last_af_state == CaptureResult.CONTROL_AF_STATE_PASSIVE_SCAN
                    && af_state != last_af_state) {
                /*
                 * if( MyDebug.LOG )
        ...
        private void process(CaptureRequest request, CaptureResult result) {
            ...
            if (result.getFrameNumber() < last_process_frame_number) {
                /*
                 * if( MyDebug.LOG )
                 * Log.d(TAG, "processAF discarded outdated frame " + result.getFrameNumber() +
                 * " vs " + last_process_frame_number);
                 */
                return;
            }
            /*
            ...
            if (af_state != null && af_state != last_af_state) {
                /*
                 * if( MyDebug.LOG )
                 * Log.d(TAG, "CONTROL_AF_STATE changed from " + last_af_state + " to " +
                 * af_state);
                 */
        ...
        private void updateCachedCaptureResult(CaptureResult result) {
            if (modified_from_camera_settings) {
                ...
                // otherwise have problem taking HDR photos twice in a row, the second one will
                // pick up the exposure time as
                 ...
                 */
                /*
                 * if( camera_settings.has_iso && Math.abs(camera_settings.iso -
                 * capture_result_iso) > 10 && previewBuilder != null ) {
                 * // ugly hack: problem (on Nexus 6 at least) that when we start recording
                 * video (video_recorder.start() call), this often causes the ISO setting to
                 * reset to the wrong value!
                 * // seems to happen more often with shorter exposure time
                 * // seems to happen on other camera apps with Camera2 API too
                 * // update: allow some tolerance, as on OnePlus 3T it's normal to have some
                 * slight difference between requested and actual
                 * // this workaround still means a brief flash with incorrect ISO, but is best
                 * we can do for now!
                 * // check previewBuilder != null as we have had Google Play crashes from the
                 * setRepeatingRequest() call via here
                 * // Update 20180326: can no longer reproduce original problem on Nexus 6 (at
                 * FullHD or 4K); no evidence of
                 * // problems on OnePlus 3T or Nokia 8.
                 * // Also note that this code was being activated whenever manual ISO is
                 * changed (since we don't immediately
                 * // update to the new ISO). At the least, this should be restricted to when
                 * recording video, but best to
                 * // disable completely now that we don't seem to need it.
                 * if( MyDebug.LOG ) {
                 * Log.d(TAG, "ISO " + capture_result_iso + " different to requested ISO " +
                 * camera_settings.iso);
                 * Log.d(TAG, "    requested ISO was: " +
                 * request.get(CaptureRequest.SENSOR_SENSITIVITY));
                 * Log.d(TAG, "    requested AE mode was: " +
                 ...
                 * }
                 * catch(CameraAccessException e) {
                 * MyDebug.logStackTrace(TAG, "failed to set repeating request after ISO hack",
                 * e);
                 * }
                 * }
                 */
            ...
            if (modified_from_camera_settings) {
                ...
                // If using manual exposure time longer than max_preview_exposure_time_c, the
                // preview will be fixed to
                // max_preview_exposure_time_c, so we should just use the requested manual
                // exposure time.
                // (This affects the exposure time shown on on-screen preview - whilst showing
                // the preview exposure time
                // isn't necessarily wrong, it tended to confuse people, thinking that manual
                // exposure time wasn't working
                // when set above max_preview_exposure_time_c.)
                // Update: but on some devices (e.g., Galaxy S10e) the reported exposure time
                // can become inaccurate when
                // we set longer preview exposure times (fine at 1/15s, 1/10s, but wrong at 0.2s
                // and 0.3s), possibly this is
                // by design if the preview along supports certain rates(?), but best to fall
                // back to the requested exposure
                // time in manual mode if requested exposure is longer than 1/12s OR the
                // max_preview_exposure_time_c.
                if (camera_settings.has_iso
                        && camera_settings.exposure_time > Math.min(max_preview_exposure_time_c, 1000000000L / 12))
             ...
             * Log.d(TAG, "capture_result_frame_duration: " +
             * capture_result_frame_duration);
             * }
             * }
             */
            /*
            ...
            if (modified_from_camera_settings) {
                ...
                /*
                 * if( MyDebug.LOG ) {
                 * Log.d(TAG, "capture_result_focus_distance: " +
                 * capture_result_focus_distance);
             ...
             * RggbChannelVector vector = result.get(CaptureResult.COLOR_CORRECTION_GAINS);
             * if( vector != null ) {
             * convertRggbVectorToTemperature(vector); // logging will occur in this
             * function
             * }
             * }
             */
                ...
                if (face_detect_mode != null && face_detect_mode != CaptureRequest.STATISTICS_FACE_DETECT_MODE_OFF) {
                    ...
                    if (camera_faces != null) {
                        if (camera_faces.length == 0 && last_faces_detected == 0) {
                            // no point continually calling the callback if 0 faces detected (same behaviour
                            // as CameraController1)
                        } else {
        ...
        private void handleRawCaptureResult(CaptureResult result) {
            ...
            if (test_wait_capture_result) {
                ...
                // Also with JPEG only capture, there are problems with repeat mode and
                // continuous focus if
                // onImageAvailable() is called before this code is called, because it means
                // here we cancel the
                // focus and lose the focus callback that was going to trigger the next repeat
                // photo! This shows
                // up on testContinuousPictureFocusRepeat() on Nexus 7, but can be autotested on
                // other devices
                // with the flag, see testContinuousPictureFocusRepeatWaitCaptureResult().
                try {
                    if (MyDebug.LOG)
                        Log.d(TAG, "test_wait_capture_result: waiting...");
                    // 200ms is enough to test the problem with testTakePhotoRawWaitCaptureResult()
                    // on Nexus 6, but use 500ms to be sure
                    // 200ms is enough to test the problem with
                    // testContinuousPictureFocusRepeatWaitCaptureResult() on Nokia 8, but use 500ms
                    // to be sure
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    MyDebug.logStackTrace(TAG, "InterruptedException from sleep", e);
        ...
        /**
         * This should be called when a capture result corresponds to a capture that is
         * for a burst
         * sequence, that isn't the last capture (for the last capture,
         * handleCaptureCompleted() is
         * instead called).
         */
        ...
        /**
         * This should be called when a capture result corresponds to a capture that has
         * completed.
         */
        private void handleCaptureCompleted(CaptureResult result) {
            ...
            // actual parsing of image data is done in the imageReader's
            // OnImageAvailableListener()
            // need to cancel the autofocus, and restart the preview after taking the photo
            // Camera2Basic does a capture then sets a repeating request - do the same here
            // just to be safe
            // update: this is also important when we do an expo burst (BURSTTYPE_EXPO) with
            // option
            // use_expo_fast_burst==false, since that changes the exposure of the preview,
            // so we need to
            // reset it here
            if (previewBuilder != null) {
                previewBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER, CameraMetadata.CONTROL_AF_TRIGGER_CANCEL);
                ...
                // if not using fake precapture, not sure if we need to set the ae mode, but the
                // AE mode is set again in Camera2Basic
                camera_settings.setAEMode(previewBuilder, false);
                // n.b., if capture/setRepeatingRequest throw exception, we don't call the
                // take_picture_error_cb.onError() callback, as the photo should have been taken
                // by this point
                try {
                    capture();
                } catch (CameraAccessException e) {
                ...
                previewBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER, CameraMetadata.CONTROL_AF_TRIGGER_IDLE); // ensure
                                                                                                               // set
                                                                                                               // back
                                                                                                               // to
                                                                                                               // idle
            ...
            if (burst_type == BurstType.BURSTTYPE_FOCUS && previewBuilder != null) { // make sure camera wasn't released
                                                                                     // in the meantime
                if (MyDebug.LOG)
        ...
        private void callCheckImagesCompleted() {
            // Important that we only call the picture onCompleted callback after we've
            // received the capture request, so
            // we need to check if we already received all the images.
            // Also needs to be run on UI thread.
            // Needed for testContinuousPictureFocusRepeat on Nexus 7; also testable on
            // other devices via
        ...
        private void processCompleted(CaptureRequest request, CaptureResult result) {
            ...
            /*
             * if( push_set_ae_lock && push_set_ae_lock_id == request && previewBuilder !=
             * null ) {
             * if( MyDebug.LOG )
             * Log.d(TAG, "received push_set_ae_lock");
             * // hack - needed to fix bug on Nexus 6 where auto-exposure sometimes locks
             * when taking a photo of bright scene with flash on!
             * // this doesn't completely resolve the issue, but seems to make it far less
             * common; also when it does happen, taking another photo usually fixes it
            ...
            if (tag_type == RequestTagType.CAPTURE) {
                ...
                if (CameraController2.this.run_post_capture != null) {
                    ...
                    if (camera != null && hasCaptureSession()) { // make sure camera wasn't released in the meantime
                        // need to wait a further ~500ms for Galaxy S10e at least (although on Galaxy
                        // S24+, it's fine if we don't do via a postDelayed at all)
                        handler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                try {
                                    ...
                                    MyDebug.logStackTrace(TAG,
                                            "failed to take picture after delay for long manual exposure", e);
                                    jpeg_todo = false;
                                    raw_todo = false;
                                    picture_cb = null;
                                    if (take_picture_error_cb != null) {
