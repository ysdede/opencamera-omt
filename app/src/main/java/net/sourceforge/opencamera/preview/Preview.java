 ...
 * support vendor camera extensions). In general we shouldn't do that
 * restriction
 ...
 * There are some exceptions where we need to restrict at the Preview level,
 * e.g.:
...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private boolean use_preview_bitmap_small, use_preview_bitmap_full; // whether we want downsized and/or full preview
                                                                       // bitmaps
    private Bitmap preview_bitmap; // downsided bitmap from preview
    private int preview_bitmap_full_w = -1, preview_bitmap_full_h = -1; // for full bitmaps, we generate copies on the
                                                                        // fly (as these need to be saved for preshots
                                                                        // feature)
    ...
    private boolean want_histogram; // whether to generate a histogram, requires want_preview_bitmap==true and
                                    // use_preview_bitmap_small==true
    ...
    private boolean want_zebra_stripes; // whether to generate zebra stripes bitmap, requires want_preview_bitmap==true
                                        // and use_preview_bitmap_small==true
    private int zebra_stripes_threshold; // pixels with max rgb value equal to or greater than this threshold are marked
                                         // with zebra stripes
    ...
    private boolean want_pre_shots; // whether to store pre-shots from preview bitmap, requires
                                    // want_preview_bitmap==true and use_preview_bitmap_full==true
    ...
    private boolean is_paused = true; // whether Preview.onPause() is called - note this could include the application
                                      // pausing the preview, even if app_is_paused==false
    ...
    private final Map<String, CameraController.CameraFeaturesCache> camera_features_caches = new Hashtable<>(); // cache
                                                                                                                // for
                                                                                                                // CameraController2

    enum CameraOpenState {
        CAMERAOPENSTATE_CLOSED, // have yet to attempt to open the camera (either at all, or since the camera
                                // was closed)
        CAMERAOPENSTATE_OPENING, // the camera is currently being opened (on a background thread)
        CAMERAOPENSTATE_OPENED, // either the camera is open (if camera_controller!=null) or we failed to open
                                // the camera (if camera_controller==null)
    ...
    private boolean has_permissions = true; // whether we have permissions necessary to operate the camera (camera,
                                            // storage); assume true until we've been denied one of them
    ...
    private android.view.Surface omt_streaming_surface; // OMT streaming surface
    private long video_start_time; // system time when the video recording was started, or last resumed if it was
                                   // paused
    private long video_accumulated_time; // this time should be added to (System.currentTimeMillis() - video_start_time)
                                         // to find the true video duration, that takes into account pausing/resuming,
                                         // as well as any auto-restarts from max filesize
    private long video_time_last_maxfilesize_restart; // when the video last restarted due to maxfilesize (or otherwise
                                                      // 0) - note this is time in ms relative to the recorded video,
                                                      // and not system time
    ...
    private static final long min_safe_restart_video_time = 1000; // if the remaining max time after restart is less
                                                                  // than this, don't restart

    /**
     * Stores the file (or similar) to record a video.
     * Important to call close() when the video recording is finished, to free up
     * any resources
    ...
    private static class VideoFileInfo {
        ...
        private final ParcelFileDescriptor video_pfd_saf; // for VideoMethod.SAF, VideoMethod.URI or
        ...
        VideoFileInfo(ApplicationInterface.VideoMethod video_method, Uri video_uri, String video_filename,
                ParcelFileDescriptor video_pfd_saf) {
    ...
    private VideoFileInfo nextVideoFileInfo; // used for Android 8+ to handle seamless restart (see
    ...
    private double natural_level_angle; // "level" angle of device in degrees, before applying any calibration and
                                        // without accounting for screen orientation
    private double level_angle; // "level" angle of device in degrees, including calibration
    private double orig_level_angle; // "level" angle of device in degrees, including calibration, but without
    ...
    // if applicationInterface.allowZoom() returns false, then has_zoom will be
    // false, but camera_controller_supports_zoom
    // supports whether the camera controller supported zoom
    // similarly for camera_controller_max_zoom_factor,
    // camera_controller_zoom_ratios - needed for initZoom()
    ...
    private int current_flash_index = -1; // this is an index into the supported_flash_values array, or -1 if no flash
                                          // modes available

    private List<String> supported_focus_values; // our "values" format
    private int current_focus_index = -1; // this is an index into the supported_focus_values array, or -1 if no focus
                                          // modes available
    ...
    private List<String> noise_reduction_modes; // n.b., this is for the Camera2 API setting, not for Open Camera's
    ...
    private Set<String> physical_camera_ids; // if non-null, this camera is part of a logical camera that exposes these
                                             // physical camera IDs
    ...
    public List<Integer> supported_extensions; // if non-null, list of supported camera vendor extensions, see
    ...
    private float capture_rate_factor = 1.0f; // should be 1.0f if has_capture_rate_factor is false; set lower than 1
                                              // for slow motion, higher than 1 for timelapse
    private boolean video_high_speed; // whether the current video mode requires high speed frame rate (note this may
                                      // still be true even if is_video==false, so potentially we could switch
                                      // photo/video modes without setting up the flag)
    ...
    private boolean take_photo_after_autofocus; // set to take a photo when the in-progress autofocus has completed; if
                                                // setting, remember to call
    ...
    // use use '0' instead of '#' to display e.g. 1.20 instead of 1.2, so that text
    // lengths are consistent (e.g., for the
    // toasts shown when changing sliders for manual focus distance or exposure
    // compensation).
    private final DecimalFormat decimal_format_2dp_force0 = new DecimalFormat("0.00");

    /*
     * If the user touches to focus in continuous mode, and in photo mode, we switch
     * the camera_controller to autofocus mode.
     * autofocus_in_continuous_mode is set to true when this happens; the runnable
    ...
    private boolean focus_set_for_target_distance; // if true, then the focus has been set to manual focus distance for
                                                   // the target (for focus bracketing)
    ...
    public volatile boolean test_runtime_on_video_stop; // force throwing a RuntimeException when stopping video (this
                                                        // usually happens naturally when stopping video too soon)
    ...
    public Preview(ApplicationInterface applicationInterface, ViewGroup parent) {
        ...
        if (using_android_l) {
            // use a TextureView for Android L - had bugs with SurfaceView not resizing
            // properly on Nexus 7; and good to use a TextureView anyway
            // ideally we'd use a TextureView for older camera API too, but sticking with
            // SurfaceView to avoid risk of breaking behaviour
            using_texture_view = true;
        }

        if (using_texture_view) {
            this.cameraSurface = new MyTextureView(getContext(), this);
            // a TextureView can't be used both as a camera preview, and used for drawing
            // on, so we use a separate CanvasView
    ...
    // If this code is changed, important to test that face detection and touch to
    // focus still works as expected, for front and back
    // cameras, for old and new API, including with zoom. Also test with
    // MainActivity.setWindowFlagsForCamera() setting orientation as
    ...
    private void calculateCameraToPreviewMatrix() {
        ...
        if (!using_android_l) {
            ...
            // Unfortunately the transformation for Android L API isn't documented, but this
            // seems to work for Nexus 6.
            // This is the equivalent code for
            // android.hardware.Camera.setDisplayOrientation, but we don't actually use
            // setDisplayOrientation()
            // for CameraController2, except testing on Nexus 6 shows that we shouldn't
            // change "result" for front facing camera.
            boolean mirror = (camera_controller.getFacing() == CameraController.Facing.FACING_FRONT);
    ...
    /**
     * Return a focus area from supplied point. Supplied coordinates should be in
     * camera
     * coordinates.
     */
    ...
    public boolean touchEvent(MotionEvent event) {
        ...
        // doesn't seem a bad idea to clear fake toasts (touching screen gets rid of
        // standard toasts on Android 10+ at least)
        ...
        if (takePhotoOnDoubleTap()) {
            // need to wait until onSingleTapConfirmed() before calling handleSingleTouch(),
            // e.g., don't
    ...
    private boolean handleSingleTouch(MotionEvent event, boolean was_paused) {
        ...
        // note, we always try to force start the preview (in case is_preview_paused has
        // become false)
        // except if recording video (firstly, the preview should be running; secondly,
        // we don't want to reset the phase!)
        if (!this.is_video) {
            startCameraPreview(true, null);
        }

        // whether to clear focus area instead of setting new one
        // we don't rely purely on isFocusWaiting(), as sometimes the focus can be
        // really quick
        if (MyDebug.LOG)
            Log.d(TAG, "focus_started_time: " + focus_started_time);
        boolean clear_focus_areas = has_focus_area && focus_area_time != -1
                && (System.currentTimeMillis() - focus_area_time) < ViewConfiguration.getDoubleTapTimeout();
        cancelAutoFocus();
        ...
        if (camera_controller != null && !this.using_face_detection && !was_paused && !touch_capture
                && !camera_controller.isCameraExtension()) {
            if (clear_focus_areas) {
                // double tap to clear focus areas
                // also if we were in autofocus_in_continuous_mode mode, reset back to
                // continuous mode
                if (MyDebug.LOG)
        ...
        if (!was_paused && touch_capture) {
            ...
            // Interpret as if user had clicked take photo/video button, except that we set
            // the focus/metering areas.
            // We go via ApplicationInterface instead of going direct to
            // Preview.takePicturePressed(), so that
            // the application can handle same as if user had pressed shutter button (needed
            // so that this works
        ...
        if (!was_paused) {
            // if clear_focus_areas==true, don't want to reenter
            // autofocus_in_continuous_mode mode
    ...
    // When pinch zooming, we'd normally have the problem that zooming is too fast,
    // because we can
    // only zoom to the limited set of values in the zoom_ratios array. So when
    // pinch zooming, we
    ...
    /**
     * Returns true if the user is currently pinch zooming, and the Preview has
     * already handled setting
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public void onScaleEnd(@NonNull ScaleGestureDetector detector) {
            ...
            if (has_multitouch_start_zoom_factor && has_zoom && camera_controller != null && zoom_ratios.get(0) < 100) {
                // when the minimum zoom is less than 1x, we should support snapping to 1x, so
                // it's easy for the user to
                ...
                if (end_zoom >= 90 && end_zoom <= 110 && start_zoom != 100 && end_zoom != 100) {
                    ...
                    if (Math.signum(start_diff) == Math.signum(end_diff)
                            && Math.abs(end_diff) >= Math.abs(start_diff)) {
    ...
    public boolean onDoubleTap() {
        ...
        if (applicationInterface.getTouchCapturePref()) {
            // return true to disable double-tap-drag zoom gesture, as we don't want this
            // when using single tap to capture either
            return true;
        }
    ...
    private class DoubleTapListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapConfirmed(@NonNull MotionEvent e) {
            ...
            // If we're taking a photo on double tap, then for single taps we need to wait
            // until these are confirmed
            // otherwise we handle via Preview.touchEvent().
            // Arguably we could handle everything via onSingleTapConfirmed(), but want to
            // avoid
            // unexpected changes of behaviour - plus it would mean a slight delay for touch
            // to
            // focus (since onSingleTapConfirmed obviously has to wait to be sure this isn't
            // a
            // double tap).
            if (takePhotoOnDoubleTap()) {
    ...
    public void clearFocusAreas() {
        ...
        // don't cancelAutoFocus() here, otherwise we get sluggish zoom behaviour on
        // Camera2 API
        if (!camera_controller.isCameraExtension()) {
            // if using camera extensions, we could never have set focus and metering in the
            // first place
    ...
    public void getMeasureSpec(int[] spec, int widthSpec, int heightSpec) {
        ...
        int result;
        if (camera_controller != null) {
            // We shouldn't assume that previewWidth > previewHeight means the device is in
            // landscape
            // orientation - this isn't necessarily true for split-screen or multi-window
            // mode.
            // Important to use prefer_later==true, as in split-screen or multi-window mode,
            // we don't always get a call
            // to MainActivity.onConfigurationChanged() when device orientation changes, so
            // have no way to know to
        ...
        if (result % 180 != 0) {
            // Usually this means the device is in portrait mode, and hence e.g. an aspect
            // ratio of
            // 4:3 should give an on-screen preview of 3:4 (since the device is rotated 90
            // degrees
            // compared to the natural camera orientation).
            // It's important to use this code instead of checking if the display is in
            // portrait
            // (or that previewWidth < previewHeight), for split-screen or multi-window
            // displays.
            // E.g., if the device orientation is in portrait, it might still be that Open
            // Camera
            // is running in landscape with previewWidth > previewHeight, because of running
            // in
            // split-screen mode, or more generally in multi-window mode where the window is
            // resized
            // to landscape orientation.
            // See
            // https://developer.android.com/training/camera2/camera-preview#relative_rotation
            // .
    ...
    private void mySurfaceChanged() {
        // surface size is now changed to match the aspect ratio of camera preview - so
        // we shouldn't change the preview to match the surface size, so no need to
    ...
    public void onSurfaceTextureSizeChanged(@NonNull SurfaceTexture texture, int width, int height) {
        if (MyDebug.LOG) {
            ...
            // Log.d(TAG, "surface texture is now: " +
            // ((TextureView)cameraSurface).getSurfaceTexture());
        }
        ...
        if (camera_controller != null) {
            ...
            if (set_preview_size && (width != preview_w || height != preview_h)) {
                ...
                // Needed to fix problem if Open Camera is already running, and the aspect ratio
                // changes (e.g.,
                // change of resolution, or switching between photo and video mode). When
                // starting up in a "default",
                // aspect ratio, the camera is opened via onSurfaceTextureAvailable(), and
                // although we then call setAspectRatio(),
                // there are no calls to onSurfaceTextureSizeChanged(). But when already
                // running, or if
                // an aspect ratio change for the view is required, changing the aspect ratio
                // causes a call to
                // onSurfaceTextureSizeChanged(), which results in the texture view's surface
                // texture's buffer size being reset!
                // (This can be seen in the source code of TextureView: onSizeChanged() calls
                // setDefaultBufferSize() before
                // calling onSurfaceTextureSizeChanged()!) So we need to call
                // setDefaultBufferSize() again to reset
    ...
    public void stopVideo(boolean from_restart) {
        ...
        if (video_recorder == null) {
            ...
            // (important to exit, otherwise we'll momentarily switch the take photo icon to
            // video mode in MyApplicationInterface.stoppingVideo() when opening the
        ...
        if (video_recorder != null) { // check again, just to be safe
            ...
            try {
                if (using_android_l && video_high_speed) {
                    // Needed to fix problems with 0.125x and 0.25x slow motion on Pixel 6 Pro -
                    // otherwise although
                    // the video is recorded, we are unable to restart the preview after stopping
                    // video.
                    // Beware of enabling this for non-high-speed - would need careful testing to
                    // ensure this doesn't cause unstable
                    // behaviour.
                    if (MyDebug.LOG)
                ...
                // stop() can throw a RuntimeException if stop is called too soon after start -
                // this indicates the video file is corrupt, and should be deleted
                if (MyDebug.LOG)
                    Log.d(TAG, "runtime exception when stopping video");
                videoFileInfo.close();
                applicationInterface.deleteUnusedVideo(videoFileInfo.video_method, videoFileInfo.video_uri,
                        videoFileInfo.video_filename);

                videoFileInfo = new VideoFileInfo();
                if (nextVideoFileInfo != null)
                    nextVideoFileInfo.close();
                nextVideoFileInfo = null;
                // if video recording is stopped quickly after starting, it's normal that we
                // might not have saved a valid file, so no need to display a message
    ...
    private void videoRecordingStopped() {
        ...
        reconnectCamera(false); // n.b., if something went wrong with video, then we reopen the camera - which
                                // may fail (or simply not reopen, e.g., if app is now paused)
        videoFileInfo.close();
        applicationInterface.stoppedVideo(videoFileInfo.video_method, videoFileInfo.video_uri,
                videoFileInfo.video_filename);
        if (nextVideoFileInfo != null) {
            // if nextVideoFileInfo is not-null, it means we received
            // MEDIA_RECORDER_INFO_MAX_FILESIZE_APPROACHING but not
            // MEDIA_RECORDER_INFO_NEXT_OUTPUT_FILE_STARTED, so it is the application
            // responsibility to create the zero-size
            // video file that will have been created
            if (MyDebug.LOG)
                Log.d(TAG, "delete unused next video file");
            nextVideoFileInfo.close();
            applicationInterface.deleteUnusedVideo(nextVideoFileInfo.video_method, nextVideoFileInfo.video_uri,
                    nextVideoFileInfo.video_filename);
        }
        videoFileInfo = new VideoFileInfo();
        nextVideoFileInfo = null;
    }
    ...
    /**
     * Restart video - either due to hitting maximum filesize (for pre-Android 8
     * when not able to restart seamlessly), or maximum duration.
     */
        ...
        if (video_recorder != null) {
            ...
            if (due_to_max_filesize) {
                ...
                if (video_max_duration > 0) {
                    ...
                    if (video_max_duration < min_safe_restart_video_time) {
                        // if there's less than 1s to go, ignore it - don't want to risk the resultant
                        // video being corrupt or throwing error, due to stopping too soon
            ...
            if (due_to_max_filesize || remaining_restart_video > 0) {
                if (is_video) {
                    ...
                    if (!due_to_max_filesize) {
                        showToast(null, toast, true); // show the toast afterwards, as we're hogging the UI thread here,
                                                      // and media recorder takes time to start up
                        // must decrement after calling takePicture(), so that takePicture() doesn't
                        // reset the value of remaining_restart_video
    ...
    private void reconnectCamera(boolean quiet) {
        ...
        if (camera_controller != null) { // just to be safe
            ...
            try {
                ...
                // this happens on Nexus 7 if trying to record video at bitrate 50Mbits or
                // higher - it's fair enough that it fails, but we need to recover without a
                // crash!
                // not safe to call closeCamera, as any call to getParameters may cause a
                // RuntimeException
     ...
     * Closes the camera.
     * 
     * @param async               Whether to close the camera on a background
     *                            thread.
     * @param closeCameraCallback If async is true, closeCameraCallback.onClosed()
     *                            will be called,
     *                            from the UI thread, once the camera is closed. If
     *                            async is false,
    ...
    private void closeCamera(boolean async, final CloseCameraCallback closeCameraCallback) {
        ...
        synchronized (this) {
            ...
            // no need to call camera_controller.setCaptureFollowAutofocusHint() as we're
            // closing the camera
        }
        set_flash_value_after_autofocus = "";
        successfully_focused = false;
        preview_targetRatio = 0.0;
        // n.b., don't reset has_set_location, as we can remember the location when
        // switching camera
        ...
        if (camera_controller != null) {
            ...
            // need to check for camera being non-null again - if an error occurred stopping
            // the video, we will have closed the camera, and may not be able to reopen
            if (camera_controller != null) {
                ...
                pausePreview(false);
                // we set camera_controller to null before starting background thread, so that
                // other callers won't try
                // to use it
                ...
                if (async) {
                    ...
                    if (MyDebug.LOG) {
                        Log.d(TAG, "closeCamera: about to release camera controller: "
            ...
            if (closeCameraCallback != null) {
                // still need to call the callback though! (otherwise if camera fails to open,
                // switch camera button won't work!)
                if (MyDebug.LOG)
    ...
    /**
     * @param stop_preview Whether to call camera_controller.stopPreview(). Normally
     *                     this should be
     *                     true, but can be set to false if the callers is going to
     *                     handle calling
    ...
    public void pausePreview(boolean stop_preview) {
        ...
        // returns and records video, the preview freezes and the video is corrupted
        // so to be safe, we always reset to continuous video mode
        // although I've now fixed this at the level where we close the settings, I've
        // put this guard here, just in case the problem occurs from elsewhere
        this.updateFocusForVideo();
        this.setPreviewPaused(false);
        if (stop_preview) {
            if (MyDebug.LOG) {
     ...
     * If camera_open_state is already CAMERAOPENSTATE_OPENING, this method does
     * nothing.
     */
    private void openCamera() {
        ...
        if (applicationInterface.isPreviewInBackground()) {
            ...
            // note, even if the application never tries to reopen the camera in the
            // background, we still need this check to avoid the camera
            // opening from mySurfaceCreated()
            // for example, this is needed when the application is recreated when settings
            // are open (a new Preview and surface is created, but
            // we don't want the camera to be opened) - to test this, go to settings then
            // turn screen off and on (and unlock)
            return;
        ...
        // need to init everything now, in case we don't open the camera (but these may
        // already be initialised from an earlier call - e.g., if we are now switching
        // to another camera)
        // n.b., don't reset has_set_location, as we can remember the location when
        // switching camera
        // is_preview_started = false; // theoretically should be false anyway, but I
        // had one RuntimeException from
        // surfaceCreated()->openCamera()->setupCamera()->setPreviewSize() because
        // is_preview_started was true, even though the preview couldn't have been
        // started
        this.preview_started_state = PREVIEW_NOT_STARTED; // theoretically should be PREVIEW_NOT_STARTED anyway, but I
                                                          // had one RuntimeException from
                                                          // surfaceCreated()->openCamera()->setupCamera()->setPreviewSize()
                                                          // because is_preview_started was true, even though the
                                                          // preview couldn't have been started
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (ContextCompat.checkSelfPermission(getContext(),
                ...
                // return for now - the application should try to reopen the camera if
                // permission is granted
                return;
            }
            if (applicationInterface.needsStoragePermission() && ContextCompat.checkSelfPermission(getContext(),
        ...
        /*
         * Opening camera on background thread is important so that we don't block the
         * UI thread:
         * - For old Camera API, this is recommended behaviour by Google for
         * Camera.open().
         * - For Camera2, the manager.openCamera() call is asynchronous, but
         * CameraController2
         * waits for it to open, so it's still important that we run that in a
         * background thread.
         * In theory this works for all Android versions, but this caused problems of
         * Galaxy Nexus
         * with tests testTakePhotoAutoLevel(), testTakePhotoAutoLevelAngles() (various
         * camera
         * errors/exceptions, failing to taking photos). Since this is a significant
         * change, this is
        ...
        if (use_background_thread) {
            ...
            open_camera_task = new AsyncTask<Void, Void, CameraController>() {
                ...
                protected void onPostExecute(CameraController camera_controller) {
                    ...
                    cameraOpened();
                    // set camera_open_state after cameraOpened, just in case a non-UI thread is
                    // listening for this - also
                    // important for test code waitUntilCameraOpened(), as test code runs on a
                    // different thread
                ...
                protected void onCancelled(CameraController camera_controller) {
                    ...
                    // this typically means the application has paused whilst we were opening camera
                    // in background - so should just
                    // dispose of the camera controller
                    if (camera_controller != null) {
                        // this is the local camera_controller, not Preview.this.camera_controller!
                        camera_controller.release();
                    }
                    camera_open_state = CameraOpenState.CAMERAOPENSTATE_OPENED; // n.b., still set OPENED state -
                                                                                // important for test thread to know
                                                                                // that this callback is complete
                    open_camera_task = null; // just to be safe
                    if (MyDebug.LOG)
    ...
    /**
     * Open the camera - this should be called from background thread, to avoid
    ...
    private CameraController openCameraCore(int cameraId, String cameraIdSPhysical) {
        ...
        // We pass a camera controller back to the UI thread rather than assigning to
        // camera_controller here, because:
        // * If we set camera_controller directly, we'd need to synchronize, otherwise
        // risk of memory barrier issues
        // * Risk of race conditions if UI thread accesses camera_controller before we
        // have called cameraOpened().
        ...
        try {
            ...
            if (using_android_l) {
                ...
                camera_controller_local = new CameraController2(Preview.this.getContext(), cameraId, cameraIdSPhysical,
                        camera_features_caches, previewErrorCallback, cameraErrorCallback);
    ...
    /**
     * Called from UI thread after openCameraCore() completes on the background
     * thread.
     */
    private void cameraOpened() {
        ...
        if (camera_controller != null) {
            camera_controller.setOmtStreamingSurface(omt_streaming_surface);
             ...
             * ) {
             * take_photo =
             * activity.getIntent().getExtras().getBoolean(TakePhoto.TAKE_PHOTO);
            ...
            if (MyDebug.LOG) {
                Log.d(TAG,
                        "openCamera: time after setting preview display: " + (System.currentTimeMillis() - debug_time));
            }

            // final boolean wait_until_started = true;
            final boolean wait_until_started = !(using_android_l
    ...
    /**
     * Try to reopen the camera, if not currently open (e.g., permission wasn't
     * granted, but now it is).
     * The camera will be opened on a background thread, so won't be available upon
     * exit of this function.
     * If camera_open_state is already CAMERAOPENSTATE_OPENING, or the camera is
     * already open,
    ...
    public void setOmtStreamingSurface(android.view.Surface surface) {
        if (MyDebug.LOG)
            Log.d(TAG, "setOmtStreamingSurface: " + surface);
        this.omt_streaming_surface = surface;
        reopenCamera();
    }

    /**
     * Closes and reopens the camera.
     * The camera will be closed and opened on a background thread, so won't be
     * available upon
     * exit of this function.
     */
    ...
    /**
     * Returns false if we failed to open the camera because camera or storage
    ...
    /**
     * Returns true iff the camera is currently being opened on background thread
     * (openCamera() called, but
    ...
    /**
     * Returns true iff we've tried to open the camera (whether or not it was
     * successful).
     */
     ...
     * Important to call this when switching between photo and video mode, as
     * ApplicationInterface
     * preferences/parameters may be different (since we can support taking photos
     * in video snapshot
     * mode, but this may have different parameters).
     * 
     * @param take_photo         take_photo is true if we have been called from the
     *                           TakePhoto widget
     *                           (which means we'll take a photo immediately after
     *                           startup).
    ...
    public void setupCamera(boolean take_photo, boolean wait_until_started, Runnable preview_opened) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && this.supported_extensions != null
                && applicationInterface.isCameraExtensionPref()) {
            ...
            if (this.supported_extensions.contains(extension)) {
                ...
                // and disable ae and awb lock (as normally we don't set this when
                // stopping/starting preview)
        ...
        if (this.is_video) {
            ...
            float video_log_profile_strength = (tonemap_profile == CameraController.TonemapProfile.TONEMAPPROFILE_LOG)
                    ? applicationInterface.getVideoLogProfileStrength()
                    : 0.0f;
            float video_gamma = (tonemap_profile == CameraController.TonemapProfile.TONEMAPPROFILE_GAMMA)
                    ? applicationInterface.getVideoProfileGamma()
                    : 0.0f;
            if (MyDebug.LOG) {
        ...
        // Setup for high speed - must be done after setupCameraParameters() and
        // switching to video mode, but before setPreviewSize() and
        // startCameraPreview().
        // In theory it shouldn't matter if we call setVideoHighSpeed(true) if
        // is_video==false, as it should only have an effect
        // when recording video; but don't set high speed mode in photo mode just to be
        // safe.
        ...
        if (do_startup_focus && using_android_l && camera_controller.supportsAutoFocus()) {
            // need to switch flash off for autofocus - and for Android L, need to do this
            // before starting preview (otherwise it won't work in time); for old camera
            // API, need to do this after starting preview!
            set_flash_value_after_autofocus = "";
            String old_flash_value = camera_controller.getFlashValue();
            // getFlashValue() may return "" if flash not supported!
            // also set flash_torch - otherwise we get bug where torch doesn't turn on when
            // starting up in video mode (and it's not like we want to turn torch off for
            // startup focus, anyway)
            if (!old_flash_value.isEmpty() && !old_flash_value.equals("flash_off")
...

        {
            ...
            if (is_burst || is_extension) {
                ...
                if (current_size != null) {
                    if (MyDebug.LOG) {
                        Log.d(TAG, "current_size: " + current_size.width + " x " + current_size.height
                                + " supports_burst? " + current_size.supports_burst);
                    }
                    if (!current_size.supportsRequirements(is_burst, is_extension, extension)) {
                        ...
                        if (new_size == null) {
                            ...
                            for (int i = 0; i < photo_sizes.size(); i++) {
                                ...
                                if (size.supportsRequirements(is_burst, is_extension, extension)) {
                                    if (new_size == null
                                            || size.width * size.height > new_size.width * new_size.height) {
                                        current_size_index = i;
                                        new_size = size;
                                    }
                                }
                        ...
                        // if we set a new size, we don't save this to applicationinterface (so that if
                        // user switches to a burst mode or extension mode and back
                        // when the original resolution doesn't support burst/extension we revert to the
                        // original resolution)
                    }
                }
            }
        ...
        setPreviewSize(); // need to call this when we switch cameras, not just when we run for the first
                          // time
        if (MyDebug.LOG) {
        ...
        // Must call startCameraPreview after checking if face detection is present -
        // probably best to call it after setting all parameters that we want
        startCameraPreview(wait_until_started, new Runnable() {
            ...
            public void run() {
                ...
                if (has_zoom) {
                    ...
                    if (MyDebug.LOG) {
                        Log.d(TAG,
                                "setupCamera: total time after zoomTo: " + (System.currentTimeMillis() - debug_time_f));
                    }
                    ...
                    // if the application switches zoom off via ApplicationInterface.allowZoom(), we
                    // need to support
                    // resetting the zoom (in case the application called setupCamera() rather than
                 ...
                 * if( MyDebug.LOG )
                 * Log.d(TAG, "switch to video for take_photo widget");
                 * this.switchVideo(false); // set during_startup to false, as we now need to
                 * reset the preview
                 * }
                 * }
                 */

                applicationInterface.cameraSetup(); // must call this after the above take_photo code for calling
                                                    // switchVideo
                if (MyDebug.LOG) {
                    Log.d(TAG, "setupCamera: total time after cameraSetup: "
                            + (System.currentTimeMillis() - debug_time_f));
                }

                if (take_photo) {
                    // take photo after a delay - otherwise we sometimes get a black image?!
                    // also need a longer delay for continuous picture focus, to allow a chance to
                    // focus - 1000ms seems to work okay for Nexus 6, put 1500ms to be safe
                    String focus_value = getCurrentFocusValue();
                    final int delay = (focus_value != null && focus_value.equals("focus_mode_continuous_picture"))
                            ? 1500
                            : 500;
                    if (MyDebug.LOG)
                ...
                if (do_startup_focus) {
                    ...
                    handler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            tryAutoFocus(true, false); // so we get the autofocus when starting up - we do this on a
                                                       // delay, as calling it immediately means the autofocus doesn't
                                                       // seem to work properly sometimes (at least on Galaxy Nexus)
                        }
                    }, 500);
                }
            ...
            if (applicationInterface.getBurstForNoiseReduction()) {
                if (this.supports_exposure_time) { // noise reduction mode also needs manual exposure
                    ...
                    camera_controller.setBurstForNoiseReduction(true,
                            nr_mode == ApplicationInterface.NRModePref.NRMODE_LOW_LIGHT);
                } else {
    ...
    private void initCameraParameters() throws CameraControllerException {
            ...
            Log.d(TAG, "initCameraParameters()");
        {
            ...
            // "Changing scene mode may override other parameters (such as flash mode, focus
            // mode, white balance).
            // For example, suppose originally flash mode is on and supported flash modes
            // are on/off. In night
            // scene mode, both flash mode and supported flash mode may be changed to off.
            // After setting scene
            // mode, applications should call getParameters to know if some parameters are
            // changed."
            // this doesn't appear to apply to Camera2 API, but we still might as well set
            // scene mode first
            if (MyDebug.LOG)
            ...
            if (supported_values != null) {
                ...
                // delete key in case it's present (e.g., if feature no longer available due to
                // change in OS, or switching APIs)
...

        {
            ...
            this.supports_video_high_speed = camera_features.video_sizes_high_speed != null
                    && !camera_features.video_sizes_high_speed.isEmpty();
            ...
            // need to do zoom last, as applicationInterface.allowZoom() may depend on the
            // supported
            // camera features (e.g., zoom not necessarily supported with camera extensions,
            // so we need to have first
            // stored supported_extensions - otherwise starting up in an extension photo
            // mode will still
    ...
    private void setupCameraParameters() {
                    ...
                    private void reportFaces(CameraController.Face[] local_faces) {
                        {
                            ...
                            if (n_faces > 0) {
                                ...
                                for (CameraController.Face face : local_faces) {
                                    ...
                                    if (all_centre) {
                                        if (face_x < bdry_frac_c || face_x > 1.0f - bdry_frac_c || face_y < bdry_frac_c
                                                || face_y > 1.0f - bdry_frac_c)
                                            all_centre = false;
                                    }
                                    avg_x += face_x;
                                    avg_y += face_y;
                            ...
                            if (n_faces != last_n_faces || face_location != last_face_location) {
                                if (n_faces == 0 && last_n_faces == -1) {
                                    ...
                                    String string = n_faces + " " + getContext().getResources()
                                            .getString(n_faces == 1 ? R.string.face_detected : R.string.faces_detected);
                                    if (n_faces > 0 && face_location != FaceLocation.FACELOCATION_UNKNOWN) {
                                        switch (face_location) {
                                            case FACELOCATION_CENTRE:
                                                string += " " + getContext().getResources()
                                            ...
                                            case FACELOCATION_TOP:
                                                string += " "
                                                        + getContext().getResources().getString(R.string.top_of_screen);
                                                break;
                                    ...
                                    if (MyDebug.LOG)
                                        Log.d(TAG, string);
                                    // to avoid having a big queue of saying "one face detected, two faces detected"
                                    // etc, we only report
        ...
        if (MyDebug.LOG) {
            Log.d(TAG, "setupCameraParameters: time after setting face detection: "
                    + (System.currentTimeMillis() - debug_time));
        }
        ...
        if (MyDebug.LOG) {
            Log.d(TAG, "setupCameraParameters: time after video stabilization: "
                    + (System.currentTimeMillis() - debug_time));
        }
...

        {
            ...
            // for anti-banding, if the stored preference wasn't supported, we stick with
            // the device default - but don't
...

        {
            ...
            // for edge mode, if the stored preference wasn't supported, we stick with the
            // device default - but don't
            // write it back to the user preference
            if (supported_values != null) {
...

        {
            ...
            // for noise reduction mode, if the stored preference wasn't supported, we stick
        ...
        // must be done before setting flash modes, as we may remove flash modes if in
        // manual mode (update: we now support flash for manual ISO anyway)
        if (MyDebug.LOG)
        ...
        if (is_extension) {
            ...
            if (value.equals(CameraController.ISO_DEFAULT)) {
                ...
                if (iso >= 0) {
                    ...
                    // failed to parse
                    camera_controller.setManualISO(false, 0);
                    value = CameraController.ISO_DEFAULT; // so we switch the preferences back to auto mode, rather than
                                                          // the invalid value
                }
            ...
            // in this mode, any support for ISO is only the specific ISOs offered by the
            // CameraController
        ...
        if (is_manual_iso) {
            ...
            if (supported_flash_values != null) {
                 ...
                 * // (it's unclear flash is useful - ideally we'd at least offer torch, but ISO
                 * seems to reset to 100 when flash/torch is on!)
                 * supported_flash_values = null;
                 * if( MyDebug.LOG )
...

        {
            ...
            if (min_exposure != 0 || max_exposure != 0) {
                ...
                // if in manual ISO mode, we still want to get the valid exposure compensations,
                // but shouldn't set exposure compensation
                if (!is_manual_iso) {
...

        {
            ...
            }
            {
                ...
                if (current_size != null) {
                    if (MyDebug.LOG)
                        Log.d(TAG, "Current size index " + current_size_index + ": " + current_size.width + ", "
                                + current_size.height);
                    ...
                    // we intentionally do this after calling
                    // applicationInterface.setCameraResolutionPref() (as the constraints are
                    // used to just temporarily change resolution, e.g., if a maximum resolution has
                    // been enforced for HDR or NR photo
                    // mode, but we don't want to update the saved resolution preference in such
                    // cases
                    if (!photo_size_constraints.satisfies(current_size)) {
                        ...
                        if (MyDebug.LOG)
                            Log.d(TAG, "Updated size index " + current_size_index + ": " + current_size.width + ", "
                                    + current_size.height);
                    }
                }
            }
            ...
            // also note that we check for compatibility with burst
            // (CameraController.Size.supports_burst) and extensions later on
        }
        if (MyDebug.LOG) {
        ...
        if (!video_quality_value_s.isEmpty()) {
            ...
            for (int i = 0; i < video_quality_handler.getSupportedVideoQuality().size()
                    && video_quality_handler.getCurrentVideoQualityIndex() == -1; i++) {
                if (video_quality_handler.getSupportedVideoQuality().get(i).equals(video_quality_value_s)) {
                    ...
                    if (MyDebug.LOG)
                        Log.d(TAG,
                                "set current_video_quality to: " + video_quality_handler.getCurrentVideoQualityIndex());
                }
            }
        ...
        if (video_quality_handler.getCurrentVideoQualityIndex() == -1
                && !video_quality_handler.getSupportedVideoQuality().isEmpty()) {
            // default to FullHD if available, else pick highest quality
            // (FullHD will give smaller file sizes and generally give better performance
            // than 4K so probably better for most users; also seems to suffer from less
            // problems when using manual ISO in Camera2 API)
        ...
        if (video_quality_handler.getCurrentVideoQualityIndex() != -1) {
            ...
            // This means video_quality_handler.getSupportedVideoQuality().size() is 0 -
            // this could happen if the camera driver
        ...
        if (MyDebug.LOG) {
            Log.d(TAG, "setupCameraParameters: time after handling video quality: "
                    + (System.currentTimeMillis() - debug_time));
        }
        ...
        if (supports_video) {
            ...
            // should be done after checking the requested video size is available, and
            // after reading the requested capture rate
            video_high_speed = false;
            if (this.supports_video_high_speed) {
                VideoProfile profile = getVideoProfile();
                int capture_rate = (int) (profile.videoCaptureRate + 1.0e-5f);
                // We round to an int (a) to avoid risk of numerical wobble when comparing to
                // the integer supported fps ranges, and (b) due to the
                // "Nokia 8" hack in getVideoProfile().
                // Note that when using timelapse (capture_rate_factor > 1.0), it may be that
                // the capture rate is genuinely fractional, although these
                // should always be non-high-speed, and this code is just for high speed cases,
                // and if so for determining if the video resolution supports high speed
                if (MyDebug.LOG)
                    Log.d(TAG, "check if we need high speed video for " + profile.videoFrameWidth + " x "
                            + profile.videoFrameHeight + " at fps capture rate " + capture_rate);
                CameraController.Size best_video_size = video_quality_handler.findVideoSizeForFrameRate(
                        profile.videoFrameWidth, profile.videoFrameHeight, capture_rate, false);
                // n.b., we should pass videoCaptureRate (capture_rate) and not videoFrameRate
                // (as for slow motion, it's videoCaptureRate that will be high, not
                // videoFrameRate)

                if (best_video_size == null && fpsIsHighSpeed(String.valueOf(capture_rate))
                        && video_quality_handler.getSupportedVideoSizesHighSpeed() != null) {
                    Log.e(TAG,
                            "can't find match for capture rate: " + capture_rate + " and video size: "
                                    + profile.videoFrameWidth + " x " + profile.videoFrameHeight + " at fps "
                                    + profile.videoFrameRate);
                    // If fpsIsHighSpeed() returns true for capture_rate, then it means an fps is
                    // one that isn't
                    // supported by any standard video sizes, but it is supported by a high speed
                    // video size. If
                    ...
                    best_video_size = CameraController.CameraFeatures.findSize(
                            video_quality_handler.getSupportedVideoSizesHighSpeed(), requested_size, capture_rate,
                            false);
                    if (best_video_size != null) {
                        ...
                        for (int i = 0; i < video_quality_handler.getSupportedVideoQuality().size(); i++) {
                            if (MyDebug.LOG)
                                Log.d(TAG, "check video quality: "
                                        + video_quality_handler.getSupportedVideoQuality().get(i));
                            CamcorderProfile camcorder_profile = getCamcorderProfile(
                                    video_quality_handler.getSupportedVideoQuality().get(i));
                            if (camcorder_profile.videoFrameWidth == profile.videoFrameWidth
                                    && camcorder_profile.videoFrameHeight == profile.videoFrameHeight) {
                                video_quality_handler.setCurrentVideoQualityIndex(i);
                                break;
                            }
                        }
                        if (video_quality_handler.getCurrentVideoQualityIndex() != -1) {
                            ...
                            // MyApplicationInterface stores preferences separately for high speed fps and
                            // non high speed, so fine to save the preference
                            applicationInterface.setVideoQualityPref(video_quality_handler.getCurrentVideoQuality());
                        } else {
                            if (MyDebug.LOG)
                ...
                if (best_video_size == null) {
                    Log.e(TAG, "fps not supported for this video size: " + profile.videoFrameWidth + " x "
                            + profile.videoFrameHeight + " at fps capture rate " + capture_rate);
                    // we'll end up trying to record at the requested resolution and fps even though
...

        {
            ...
            if (supported_flash_values != null && supported_flash_values.size() > 1) {
                ...
                if (!flash_value.isEmpty()) {
                    ...
                    if (!updateFlash(flash_value, false)) { // don't need to save, as this is the value that's already
                                                            // saved
                        if (MyDebug.LOG)
                            Log.d(TAG, "flash value no longer supported!");
                        // if in manual ISO mode, we'll have restricted the available flash modes - so
                        // although we want to
                        // communicate this to the application, we don't want to save the new value
                        // we've chosen (otherwise
                        // if user goes to manual ISO and back, we might switch saved flash say from
                        // auto to off)
                    ...
                    // whilst devices with flash should support flash_auto, we'll also be in this
                    // codepath for front cameras with
                    // no flash, as instead the available options will be flash_off,
                    // flash_frontscreen_auto, flash_frontscreen_on
                     ...
                     */
                    // update, we now default to flash off - flash is increasingly less useful on
                    // modern cameras,
        ...
        if (MyDebug.LOG) {
            Log.d(TAG,
                    "setupCameraParameters: time after setting up flash: " + (System.currentTimeMillis() - debug_time));
        }
        ...
        if (MyDebug.LOG) {
            Log.d(TAG,
                    "setupCameraParameters: time after setting up focus: " + (System.currentTimeMillis() - debug_time));
        }

        {
            if (MyDebug.LOG)
                Log.d(TAG, "set up exposure lock");
            // exposure lock should always default to false, as doesn't make sense to save
            // it - we can't really preserve a "lock" after the camera is reopened
        ...
        if (MyDebug.LOG) {
            Log.d(TAG, "setupCameraParameters: total time for setting up camera parameters: "
                    + (System.currentTimeMillis() - debug_time));
        }
    }
    ...
    private void setPreviewSize() {
        ...
        if (preview_started_state != PREVIEW_NOT_STARTED) {
            ...
            // throw new RuntimeException(); // throw as RuntimeException, as this is a
            // programming error
            // Bizarrely I have seen the above crash reported from Google Play devices, but
            // inspection of the code leaves it unclear
            // why this can happen. So have disabled the exception since this evidently can
            // happen.
            return;
        }
        if (!using_android_l) {
            // don't do for Android L, else this means we get flash on startup autofocus if
            // flash is on
            this.cancelAutoFocus();
        }
        // first set picture size (for photo mode, must be done now so we can set the
        // picture size from this; for video, doesn't really matter when we set it)
        ...
        if (this.is_video) {
            ...
            if (video_high_speed) {
                // It's unclear it matters what size we set here given that high speed is only
                // for Camera2 API, and that
                // take photo whilst recording video isn't supported for high speed video - so
                // for Camera2 API, setting
    ...
    /**
     * Gets a CamcorderProfile associated with the supplied quality, for non-slow
     * motion modes. Note
     * that the supplied quality doesn't have to match whatever the current video
     * mode is (or indeed,
     * this might be called even in slow motion mode), since we use this for things
     * like setting up
            ...
            if (index != -1 && index + 1 < quality.length()) {
                ...
                if (override_string.charAt(0) == 'r' && override_string.length() >= 4) {
                    ...
                    if (index == -1) {
                        ...
                        // copy to local variable first, so that if we fail to parse height, we don't
                        // set the width either
    ...
    /**
     * Returns a profile describing the currently selected video quality. The
     * returned VideoProfile
     * will usually encapsulate a CamcorderProfile
     * (VideoProfile.getCamcorderProfile() will return
     * non-null), but not always (e.g., for slow motion mode).
     */
    public VideoProfile getVideoProfile() {
        VideoProfile video_profile;

        // 4K UHD video is not yet supported by Android API (at least testing on Samsung
        // S5 and Note 3, they do not return it via getSupportedVideoSizes(), nor via a
        // CamcorderProfile (either QUALITY_HIGH, or anything else)
        // but it does work if we explicitly set the resolution (at least tested on an
        // S5)
        if (camera_controller == null) {
        ...
        /*
         * if( video_high_speed ) {
         * // return a video profile for a high speed frame rate - note that if we have
         * a capture rate factor of say 0.25x,
         * // the actual fps and bitrate of the resultant video would also be scaled by
         * a factor of 0.25x
         * //return new VideoProfile(MediaRecorder.AudioEncoder.AAC,
         * MediaRecorder.OutputFormat.WEBM, 20000000,
         * // MediaRecorder.VideoEncoder.VP8, this.video_high_speed_size.height, 120,
         * // this.video_high_speed_size.width);
         * return new VideoProfile(MediaRecorder.AudioEncoder.AAC,
         * MediaRecorder.OutputFormat.MPEG_4, 4*14000000,
        ...
        // Use CamcorderProfile just to get the current sizes and defaults.
        {
            ...
            // video_high_speed should only be for Camera2, where we don't support force4k
            // option, but
            // put the check here just in case - don't want to be forcing 4K resolution if
            // high speed
            // frame rate!
            if (force4k && !video_high_speed) {
                ...
                cam_profile.videoBitRate = (int) (cam_profile.videoBitRate * 2.8); // need a higher bitrate for the
                                                                                   // better quality - this is roughly
                                                                                   // based on the bitrate used by an
                                                                                   // S5's native camera app at 4K (47.6
                                                                                   // Mbps, compared to 16.9 Mbps which
                                                                                   // is what's returned by the
        ...
        if (fps_value.equals("default")) {
            if (supports_video_high_speed && video_profile.videoFrameWidth != 0
                    && video_profile.videoFrameHeight != 0) {
                // Check videoFrameRate is actually supported by requested video resolution.
                // We need this as sometimes the CamcorderProfile we use may store a frame rate
                // not actually
                // supported for the resolution (e.g., on Pixel 6 Pro, 1920x1080 and 3840x2160
                // support 60fps,
                // and the CamcorderProfiles set 60fps, but the intermediate resolutions such as
                // 1920x1440 only
                // support 30fps).
                // Limited to supports_video_high_speed - at the least, we don't want this code
                // for old camera API where
                // supported frame rates aren't available.
                // N.B., we should pass videoCaptureRate and not videoFrameRate (as for slow
                // motion, it's videoCaptureRate
                // that will be high, not videoFrameRate).
                CameraController.Size best_video_size = video_quality_handler.findVideoSizeForFrameRate(
                        video_profile.videoFrameWidth, video_profile.videoFrameHeight, video_profile.videoCaptureRate,
                        true);
                if (best_video_size != null && !best_video_size.supportsFrameRate(video_profile.videoCaptureRate)) {
                    if (MyDebug.LOG)
                        Log.d(TAG,
                                "video resolution " + video_profile.videoFrameWidth + " x "
                                        + video_profile.videoFrameHeight + " doesn't support requested fps "
                                        + video_profile.videoFrameRate);
        ...
        if (has_capture_rate_factor) {
            if (MyDebug.LOG)
                Log.d(TAG, "set video profile frame rate for slow motion or timelapse, capture rate: "
                        + capture_rate_factor);
            if (capture_rate_factor < 1.0) {
                ...
                if (Math.abs(capture_rate_factor - 0.5f) < 1.0e-5f) {
                    // hack - on Nokia 8 at least, capture_rate_factor of 0.5x still gives a normal
                    // speed video, but a
                    // workaround is to increase the capture rate - even increasing by just 1.0e-5
                    // works
        ...
        // we repeat the Build.VERSION check to avoid Android Lint warning; also needs
        // to be an "if" statement rather than using the
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
            ...
            // needed for Android 6, in case users deny storage permission, otherwise we'll
            // crash
            // see https://developer.android.com/training/permissions/requesting.html
            // we request permission when switching to video mode - if it wasn't granted,
            // here we just switch it off
            // we restrict check to Android 6 or later just in case, see note in
            // LocationSupplier.setupLocationListener()
            if (MyDebug.LOG)
                Log.e(TAG, "don't have RECORD_AUDIO permission");
            // don't show a toast here, otherwise we'll keep showing toasts whenever
            // getVideoProfile() is called; we only
            // should show a toast when user starts recording video; so we indicate this via
            // the no_audio_permission flag
            record_audio = false;
        ...
        if (record_audio) {
            ...
            // else keep with the value already stored in VideoProfile (set from the
            // CamcorderProfile)
        }
        ...
        switch (pref_video_output_format) {
            case "preference_video_output_format_default":
                // n.b., although there is MediaRecorder.OutputFormat.DEFAULT, we don't
                // explicitly set that - rather stick with what is default in the
                // CamcorderProfile
                break;
            ...
            case "preference_video_output_format_webm": {
                // n.b., audio isn't recorded on any device I've tested with WEBM, seems this
                // may
    ...
    private static String getBurstString(Resources resources, boolean supports_burst) {
        // should return empty string if supports_burst==true, as this is also used for
    ...
    public static String getAspectRatioMPString(Resources resources, int width, int height, boolean supports_burst) {
        return "(" + getAspectRatio(width, height) + ", " + getMPString(width, height)
                + getBurstString(resources, supports_burst) + ")";
    }
    ...
    public String getCamcorderProfileDescription(String quality) {
        ...
        String type = getCamcorderProfileDescriptionType(profile);
        String space = type.isEmpty() ? "" : " ";
        return type + space + profile.videoFrameWidth + "x" + profile.videoFrameHeight + " "
                + getAspectRatioMPString(getResources(), profile.videoFrameWidth, profile.videoFrameHeight, true);
    }
    ...
    private double calculateTargetRatioForPreview(Point display_size) {
        ...
        // should always use wysiwig for video mode, otherwise we get incorrect aspect
        // ratio shown when recording video (at least on Galaxy Nexus, e.g., at 640x480)
        // also not using wysiwyg mode with video caused corruption on Samsung cameras
        // (tested with Samsung S3, Android 4.3, front camera, infinity focus)
        if (preview_size.equals("preference_preview_size_wysiwyg") || this.is_video) {
            ...
            // base target ratio from display size - means preview will fill the device's
            // display as much as possible
            // but if the preview's aspect ratio differs from the actual photo/video size,
            // the preview will show a cropped version of what is actually taken
    ...
    /**
     * Returns the size in sizes that is the closest aspect ratio match to
     * targetRatio, but (if max_size is non-null) is not
     * larger than max_size (in either width or height).
     */
    private static CameraController.Size getClosestSize(List<CameraController.Size> sizes, double targetRatio,
            CameraController.Size max_size) {
        if (MyDebug.LOG)
    ...
    public CameraController.Size getOptimalPreviewSize(List<CameraController.Size> sizes) {
        ...
        if (is_video && video_high_speed) {
            ...
            if (MyDebug.LOG)
                Log.d(TAG, "video size: " + profile.videoFrameWidth + " x " + profile.videoFrameHeight);
            // preview size must match video resolution for high speed, see doc for
        ...
        {
            applicationInterface.getDisplaySize(display_size, false); // don't exclude insets, as preview runs under
                                                                      // insets in edge-to-edge mode
            // getSize() is adjusted based on the current rotation, so should already be
            // landscape format, but:
            // (a) it would be good to not assume Open Camera runs in landscape mode (if we
            // ever ran in portrait mode,
            // we'd still want display_size.x > display_size.y as preview resolutions also
            // have width > height,
            // (b) on some devices (e.g., Nokia 8), when coming back from the Settings when
            // device is held in Preview,
            // display size is returned in portrait format! (To reproduce, enable "Maximise
            // preview size"; or if that's
        ...
        // Try to find the size which matches the aspect ratio, and is closest match to
        // display height
        for (CameraController.Size size : sizes) {
            if (MyDebug.LOG)
     ...
     * In theory, the picture size shouldn't matter in video mode, but the stock
     * Android camera sets a picture size
     * which is the largest that matches the video's aspect ratio.
     * This seems necessary to work around an aspect ratio bug introduced in Android
     * 4.4.3 (on Nexus 7 at least):
     ...
     * Setting the picture size in video mode is also needed for taking photos when
     * recording video. We need to make sure we
     * set photo resolutions that are supported by Android when recording video. For
     * old camera API, this doesn't matter so much
     * (if we request too high, it'll automatically reduce the photo resolution),
     * but still good to match the aspect ratio. For
     * Camera2 API, see notes at
     ...
     * android.os.Handler)" .
     */
    public static CameraController.Size getOptimalVideoPictureSize(List<CameraController.Size> sizes,
            double targetRatio, CameraController.Size max_video_size) {
        if (MyDebug.LOG)
            Log.d(TAG, "getOptimalVideoPictureSize()");
        final double ASPECT_TOLERANCE = 0.05;
        if (sizes == null)
        ...
        // But don't choose a size that's larger than the max video size (as this isn't
        // supported for taking photos when
        // recording video for devices with LIMITED support in Camera2 mode).
        // In theory, for devices FULL Camera2 support, if the current video resolution
        // is smaller than the max preview resolution,
        // we should be able to support larger photo resolutions, but this is left to
        // future.
        for (CameraController.Size size : sizes) {
            if (MyDebug.LOG)
                Log.d(TAG, "    supported preview size: " + size.width + ", " + size.height);
    ...
    /**
     * Returns the rotation in degrees of the display relative to the natural device
     * orientation.
     */
    ...
    // note, if orientation is locked to landscape this is only called when setting
    // up the activity, and will always have the same orientation
    ...
    /*
     * Returns the rotation (in degrees) to use for images/videos, taking the
    ...
    public void draw(Canvas canvas) {
        ...
        /*
         * if( MyDebug.LOG )
         * Log.d(TAG, "surface frame " + mHolder.getSurfaceFrame().width() + ", " +
    ...
    public int getScaledZoomFactor(float scale_factor) {
        ...
        if (this.camera_controller != null && this.has_zoom) {
            ...
            if (has_smooth_zoom) {
                ...
                if (MyDebug.LOG)
                    Log.d(TAG, "    use smooth_zoom: " + smooth_zoom + " instead of: "
                            + this.zoom_ratios.get(zoom_factor) / 100.0f);
            } else {
            ...
            if (zoom_ratio <= zoom_ratios.get(0) / 100.0f) {
                ...
                // Important to have same behaviour whether zooming in or out, otherwise problem
                // when touching with two fingers and not
                // moving - we'll get very small scale factors alternately between zooming in
                // and out.
                // The only reason we have separate codepath for zooming in or out is for
                // performance (since we know to only look at
                ...
                if (scale_factor > 1.0f) {
                    ...
                    for (int i = zoom_factor + 1; i < zoom_ratios.size(); i++) {
                        ...
                        if (this_dist < dist) {
                            ...
                            dist = this_dist;
                            if (MyDebug.LOG)
                                Log.d(TAG, "zoom in, found new zoom by comparing " + zoom_ratios.get(i) / 100.0f
                                        + " to " + zoom_ratio + " , dist " + dist);
                    ...
                    for (int i = zoom_factor - 1; i >= 0; i--) {
                        ...
                        if (this_dist < dist) {
                            ...
                            dist = this_dist;
                            if (MyDebug.LOG)
                                Log.d(TAG, "zoom out, found new zoom by comparing " + zoom_ratios.get(i) / 100.0f
                                        + " to " + zoom_ratio + " , dist " + dist);
                        } else if (this_dist > dist + 1.0e-5f) {
                            break;
                        }
            ...
            if (MyDebug.LOG) {
                ...
                Log.d(TAG, "    chosen new zoom_factor " + new_zoom_factor + " ratio "
    ...
    public void scaleZoom(float scale_factor) {
        ...
        if (this.camera_controller != null && this.has_zoom) {
            ...
            // else don't call zoomTo; this should be called indirectly by
     ...
     * @param allow_zoom_transition If true, then change zoom gradually towards the
     *                              requested zoom,
     *                              rather than zooming immediately to the requested
     *                              zoom. Only
    ...
    public void zoomTo(int new_zoom_factor, boolean allow_smooth_zoom, boolean allow_zoom_transition) {
        ...
        // problem where we crashed due to calling this function with null camera should
        // be fixed now, but check again just to be safe
        if (camera_controller != null) {
            if (this.has_zoom) {
                // don't cancelAutoFocus() here, otherwise we get sluggish zoom behaviour on
                // Camera2 API
                ...
                if (allow_zoom_transition) {
                    ...
                    zoom_transition_runnable = new Runnable() {
                        public void run() {
                            // check just in case camera is closed or changed to a state where
                            // has_zoom==false,
                            ...
                            if (time >= duration) {
                                ...
                                this_zoom_value = (int) ((1.0f - alpha) * start_zoom_value + alpha * target_zoom_value
                                        + 0.5f);
                            }
                            if (MyDebug.LOG)
                    ...
                    // if pinch zooming, pass through the "smooth" zoom factor so for Camera2 API we
                    // get perfectly smooth zoom, rather than it
                    // being snapped to the discrete zoom values
                    camera_controller.setZoom(new_zoom_factor,
                            (allow_smooth_zoom && has_smooth_zoom) ? smooth_zoom : -1.0f);
                }
    ...
    public void setFocusDistance(float new_focus_distance, boolean is_target_distance, boolean show_toast) {
        ...
        if (camera_controller != null) {
            ...
            if (is_target_distance) {
                ...
                // also set the focus distance, so the user can see what the target distance
                // looks like
                ...
                if (applicationInterface.isFocusBracketingSourceAutoPref()) {
                    // first record the current focus distance, in case needed for taking a photo
                    // whilst adjusting the target focus distance
            ...
            if (focus_changed) {
                ...
                if (show_toast) {
                    ...
                    if (new_focus_distance > 0.0f) {
                        ...
                        focus_distance_s = decimal_format_2dp_force0.format(real_focus_distance)
                    ...
                    if (this.supports_focus_bracketing && applicationInterface.isFocusBracketingPref())
                        id = is_target_distance ? R.string.focus_bracketing_target_distance
                                : R.string.focus_bracketing_source_distance;
    ...
    /**
     * Set a manual white balance temperature. The white balance mode must be set to
     * "manual" for
    ...
    public String getExposureCompensationString(int exposure) {
        ...
        // show a "+" even for exactly 0, so that we have a consistent text length
        // (useful for the toast when adjusting the exposure compensation slider)
        return getResources().getString(R.string.exposure_compensation) + " " + (exposure >= 0 ? "+" : "")
                + decimal_format_2dp_force0.format(exposure_ev) + " EV";
    }
    ...
    public String getExposureTimeString(long exposure_time) {
        ...
        if (exposure_time > 100000000) {
            ...
            string = decimal_format_1dp.format(exposure_time_s)
     ...
     * return decimal_format_2dp.format(real_dist) +
     * getResources().getString(R.string.metres_abbreviation);
     * }
     * 
     ...
     * //return f_s + getFocusOneDistanceString(dist_min) + "-" +
     * getFocusOneDistanceString(dist_max);
    ...
    public boolean canSwitchCamera() {
        if (this.phase == PHASE_TAKING_PHOTO || isVideoRecording()) {
            // just to be safe - risk of cancelling the autofocus before taking a photo, or
    ...
    public static int[] matchPreviewFpsToVideo(List<int[]> fps_ranges, int video_frame_rate) {
        ...
        if (selected_min_fps != -1) {
            ...
            for (int[] fps_range : fps_ranges) {
                ...
                if (MyDebug.LOG) {
                    Log.d(TAG, "    supported fps range: " + min_fps + " to " + max_fps + " has dist " + dist
                            + " and diff " + diff);
                }
            ...
            if (MyDebug.LOG)
                Log.e(TAG, "    can't find match for fps range, so choose closest: " + selected_min_fps + " to "
                        + selected_max_fps);
        }
    ...
    public static int[] chooseBestPreviewFps(List<int[]> fps_ranges) {
        ...
        // find value with lowest min that has max >= 30; if more than one of these,
        // pick the one with highest max
        ...
        if (selected_min_fps != -1) {
            ...
            if (MyDebug.LOG)
                Log.d(TAG, "    can't find fps range 30fps or better, so picked widest range: " + selected_min_fps
                        + " to " + selected_max_fps);
        }
        return new int[] { selected_min_fps, selected_max_fps };
    }
    ...
    /*
     * It's important to set a preview FPS using chooseBestPreviewFps() rather than
     * just leaving it to the default, as some devices
     * have a poor choice of default - e.g., Nexus 5 and Nexus 6 on original Camera
     * API default to (15000, 15000), which means very dark
     * preview and photos in low light, as well as a less smooth framerate in good
     * light.
     * See
     * http://stackoverflow.com/questions/18882461/why-is-the-default-android-camera
     * -preview-smoother-than-my-own-camera-preview .
     */
    private void setPreviewFps() {
        ...
        if (camera_controller.isCameraExtension()) {
            ...
            // For Nexus 5 and Nexus 6, we need to set the preview fps using
            // matchPreviewFpsToVideo to avoid problem of dark preview in low light, as
            // described above.
            // When the video recording starts, the preview automatically adjusts, but still
            // good to avoid too-dark preview before the user starts recording.
            // However I'm wary of changing the behaviour for all devices at the moment,
            // since some devices can be
            // very picky about what works when it comes to recording video - e.g.,
            // corruption in preview or resultant video.
            // So for now, I'm just fixing the Nexus 5/6 behaviour without changing
            // behaviour for other devices. Later we can test on other devices, to see if we
            // can
            // use chooseBestPreviewFps() more widely.
            // Update for v1.31: we no longer seem to need this - I no longer get a dark
            // preview in photo or video mode if we don't set the fps range;
            // but leaving the code as it is, to be safe.
            // Update for v1.43: implementing setPreviewFpsRange() for CameraController2
            // caused the dark preview problem on
            // OnePlus 3T. So enable the preview_too_dark for all devices on Camera2.
            // Update for v1.43.3: had reports of problems (e.g., setting manual mode with
            // video on camera2) since 1.43. It's unclear
            // if there is any benefit to setting the preview fps when we aren't requesting
            // a specific fps value, so seems safest to
            // revert to the old behaviour (where CameraController2.setPreviewFpsRange() did
            // nothing).
            boolean preview_too_dark = using_android_l || Build.MODEL.equals("Nexus 5")
                    || Build.MODEL.equals("Nexus 6");
            String fps_value = applicationInterface.getVideoFPSPref();
            if (MyDebug.LOG) {
            ...
            // note that setting an fps here in continuous video focus mode causes preview
            // to not restart after taking a photo on Galaxy Nexus
            // but we need to do this, to get good light for Nexus 5 or 6
            // we could hardcode behaviour like we do for video, but this is the same way
            // that Google Camera chooses preview fps for photos
            // or I could hardcode behaviour for Galaxy Nexus, but since it's an old device
            // (and an obscure bug anyway - most users don't really need continuous focus in
            // photo mode), better to live with the bug rather than complicating the code
            // Update for v1.29: this doesn't seem to happen on Galaxy Nexus with continuous
            // picture focus mode, which is what we now use
            // Update for v1.31: we no longer seem to need this for old API - I no longer
            // get a dark preview in photo or video mode if we don't set the fps range;
            // but leaving the code as it is, to be safe.
            // Update for v1.43.3: as noted above, setPreviewFpsRange() was implemented for
            // CameraController2 in v1.43, but no evidence this
            // is needed for anything, so thinking about it, best to keep things as they
            // were before for Camera2
            if (using_android_l) {
                if (MyDebug.LOG)
    ...
    public void switchVideo(boolean during_startup, boolean change_user_pref) {
        ...
        if (camera_controller == null && during_startup) {
            // if during_startup==false at least, we should allow switching to/from video
            // mode if
            // camera failed to open (it may be that the failure to open is specific to
            // video mode
            // for example, so should allow user to switch back to photo mode - e.g.,
            // setting
        ...
        if (is_video != old_is_video) {
            initZoom(); // needed for when in Panorama mode, but we're in video, when starting up, as
                        // zoom should still be shown - see testTakePhotoPanorama
            setFocusPref(false); // first restore the saved focus for the new photo/video mode; don't do
                                 // autofocus, as it'll be cancelled when restarting preview
            /*
             * if( !is_video ) {
             * // changing from video to photo mode
             * setFocusPref(false); // first restore the saved focus for the new photo/video
             * mode; don't do autofocus, as it'll be cancelled when restarting preview
             * }
             */
            ...
            if (!during_startup) {
                // if during startup, updateFlashForVideo() needs to always be explicitly called
                // anyway
                updateFlashForVideo();
            }
            ...
            if (!during_startup) {
                if (MyDebug.LOG) {
                    String focus_value = current_focus_index != -1 ? supported_focus_values.get(current_focus_index)
                            : null;
                    Log.d(TAG, "focus_value is " + focus_value);
                }
                // Although in theory we only need to stop and start preview, which should be
                // faster, reopening the camera allows that to
                // run on the background thread, thus not freezing the UI
                // Also workaround for bug on Nexus 6 at least where switching to video and back
                // to photo mode causes continuous picture mode to stop -
                // at the least, we need to reopen camera when: ( !is_video && focus_value !=
                // null && focus_value.equals("focus_mode_continuous_picture") ).
                // Lastly, note that it's important to still call setupCamera() when switching
                // between photo and video modes (see comment for setupCamera()).
                // So if we ever allow stopping/starting the preview again, we still need to
                // call setupCamera() again.
                // Update: and even if we want to go back to just stopping/starting the preview,
                // it's likely still a good idea to reopen the camera when
                // switching from/to vendor camera extensions, otherwise risk of hangs/crashes
                // on at least some devices (see note in MainActivity.updateForSettings)
                this.reopenCamera();
            }
            ...
            if (is_video) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && applicationInterface.getRecordAudioPref()) {
                    ...
                    if (ContextCompat.checkSelfPermission(getContext(),
                        ...
                        // we can now carry on - if the user starts recording video, we'll check then if
    ...
    public void setFocusPref(boolean auto_focus) {
        ...
        if (!focus_value.isEmpty()) {
            ...
            if (!updateFocus(focus_value, true, false, auto_focus)) { // don't need to save, as this is the value that's
                                                                      // already saved
                if (MyDebug.LOG)
                    Log.d(TAG, "focus value no longer supported!");
                // don't save, as we may be in a temporary mode where the saved focus isn't
                // supported - e.g., this could happen if switching to a specific physical
                // camera
            ...
            if (!updateFocus(is_video ? "focus_mode_continuous_video" : "focus_mode_continuous_picture", true, true,
                    auto_focus)) {
                if (MyDebug.LOG)
    ...
    /**
     * If in video mode, update the focus mode if necessary to be continuous video
     * focus mode (if that mode is available).
     * Normally we remember the user-specified focus value. And even setting the
     * default is done in setFocusPref().
     * This method is used as a workaround for a bug on Samsung Galaxy S5 with UHD,
     * where if the user switches to another
     * (non-continuous-video) focus mode, then goes to Settings, then returns and
     * records video, the preview freezes and the
     * video is corrupted.
     * 
     * @return If the focus mode is changed, this returns the previous focus mode;
    ...
    private String updateFocusForVideo() {
        ...
        if (this.supported_focus_values != null && camera_controller != null && is_video) {
            ...
            if (focus_is_video != is_video) {
                ...
                updateFocus("focus_mode_continuous_video", true, false, false); // don't save, as we're just changing
                                                                                // focus mode temporarily for the
                                                                                // Samsung S5 video hack
            }
        }
    ...
    /**
     * If we've switch to video mode, ensures that we're not in a flash mode other
     * than torch.
     * This only changes the internal user setting, we don't tell the application
     * interface to change
     * the flash mode.
     */
    ...
    public static boolean isFlashSupportedForVideo(String flash_mode) {
        return flash_mode != null && (flash_mode.equals("flash_off") || flash_mode.equals("flash_torch")
                || flash_mode.equals("flash_frontscreen_torch"));
    }
        ...
        if (supported_flash_values != null) {
            ...
            while (!done) {
                ...
                if (skip_torch && supported_flash_values.get(new_flash_index).equals("flash_torch")) {
                    ...
                    // don't bother setting done to false as we shouldn't have two torches in a
                    // row...
                }
    ...
    private void setFlash(String flash_value) {
        ...
        set_flash_value_after_autofocus = ""; // this overrides any previously saved setting, for during the startup
                                              // autofocus
        if (camera_controller == null) {
            if (MyDebug.LOG)
                Log.d(TAG, "camera not opened!");
    ...
    // this returns the flash value indicated by the UI, rather than from the camera
    // parameters (may be different, e.g., in startup autofocus!)
    ...
    // this returns the flash mode indicated by the UI, rather than from the camera
    // parameters (may be different, e.g., in startup autofocus!)
    /*
    ...
    public void updateFocus(String focus_value, boolean quiet, boolean auto_focus) {
        ...
        if (this.phase == PHASE_TAKING_PHOTO) {
            // just to be safe - otherwise problem that changing the focus mode will cancel
            // the autofocus before taking a photo, so we never take a photo, but
            // is_taking_photo remains true!
            if (MyDebug.LOG)
                Log.d(TAG, "currently taking a photo");
            return;
        }
    ...
    private void setFocusValue(String focus_value, boolean auto_focus) {
        ...
        cancelAutoFocus();
        removePendingContinuousFocusReset(); // this isn't strictly needed as the reset_continuous_focus_runnable will
                                             // check the ui focus mode when it runs, but good to remove it anyway
    ...
    public void takePicturePressed(boolean photo_snapshot, boolean continuous_fast_burst) {
        ...
        if (preview_started_state == PREVIEW_IS_STARTING) {
            // if it's PREVIEW_NOT_STARTED then we can start it, but if it's already
            // starting on a background thread, better to exit here
            if (MyDebug.LOG)
        ...
        // if( (is_video && is_video_recording && !photo_snapshot) || this.phase ==
        // PHASE_TAKING_PHOTO ) {
        if (is_video && isVideoRecording() && !photo_snapshot) {
            ...
            if (!video_start_time_set || System.currentTimeMillis() - video_start_time < 500) {
                ...
                // firstly to reduce risk of corrupt video files when stopping too quickly (see
                // RuntimeException we have to catch in stopVideo),
                // secondly, to reduce a backlog of events which slows things down, if user
                // presses start/stop repeatedly too quickly
                if (MyDebug.LOG)
            ...
            } else if (!is_video && camera_controller.getBurstType() == CameraController.BurstType.BURSTTYPE_FOCUS
                    && camera_controller.isCapturingBurst()) {
            ...
            public void run() {
                ...
                activity.runOnUiThread(new Runnable() {
                    public void run() {
                        ...
                        // but still need to check that the camera hasn't closed or the task halted,
                        // since TimerTask.run() started
    ...
    private void onVideoInfo(int what, int extra) {
        ...
        // n.b., we shouldn't refactor "Build.VERSION.SDK_INT >= Build.VERSION_CODES.O"
        // to a single variable, as it means we'll then get the Android
        // warnings of "Call requires API level 26"
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
                && what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_APPROACHING
                && video_restart_on_max_filesize) {
            if (MyDebug.LOG)
            ...
            if (video_recorder == null) {
                ...
                // OnePlus3T: setMaxDuration is ignored if we hit max filesize and call
                // setNextOutputFile before
                // this would cause testTakeVideoMaxFileSize3 to fail
                // Nokia 8: the camera server dies when restarting with setNextOutputFile, if
                // setMaxDuration has been set!
            } else {
                // First we need to see if there's enough free storage left - it might be that
                // we hit the max filesize that was
                // set in MyApplicationInterface.getVideoMaxFileSizePref() due to the remaining
                // disk space.
                // Potentially we could just modify getVideoMaxFileSizePref() to not set
                // VideoMaxFileSize.auto_restart if the
                // max file size was set due to remaining disk space rather than user
                // preference, but worth rechecking in case
                // disk space has been freed up; also we might encounter a device limit on max
                // filesize that's less than the
                ...
                if (profile.fileExtension.equals("3gp")) {
                    // at least on Nokia 8 with Camera2, 3gpp format crashes with
                    // IllegalStateException in setNextOutputFile below
                    ...
                    // only assign to videoFileInfo after setNextOutputFile in case it throws an
            ...
            // no need to explicitly stop if createVideoFile() or setNextOutputFile() fails
            // - just let video reach max filesize
            // normally
            ...
            if (nextVideoFileInfo == null) {
                ...
                applicationInterface.restartedVideo(videoFileInfo.video_method, videoFileInfo.video_uri,
                        videoFileInfo.video_filename);
            ...
            // note, if the restart was handled via
            // MEDIA_RECORDER_INFO_MAX_FILESIZE_APPROACHING, then we shouldn't ever
        ...
        applicationInterface.onVideoInfo(what, extra); // call this last, so that toasts show up properly (as we're
                                                       // hogging the UI thread here, and mediarecorder takes time to
                                                       // stop)
    }
    ...
    private void onVideoError(int what, int extra) {
        ...
        stopVideo(false);
        applicationInterface.onVideoError(what, extra); // call this last, so that toasts show up properly (as we're
                                                        // hogging the UI thread here, and mediarecorder takes time to
                                                        // stop)
    }

    /**
     * Initiate "take picture" command. In video mode this means starting video
     * command. In photo mode this may involve first
     * autofocusing.
     * 
     * @param photo_snapshot        If true, then the user has requested taking a
     *                              photo whilst video
    ...
    private void startVideoRecording(final boolean max_filesize_restart) {
        ...
        if (info == null) {
            ...
            try {
                ApplicationInterface.VideoMaxFileSize video_max_filesize = applicationInterface
                ...
                if (max_filesize > 0) {
                    ...
                    try {
                        ...
                        // Google Camera warns this can happen - for example, if 64-bit filesizes not
                        // supported
                        MyDebug.logStackTrace(TAG, "failed to set max filesize of: " + max_filesize, e);
                    }
                }
                video_restart_on_max_filesize = video_max_filesize.auto_restart; // note, we set this even if
                                                                                 // max_filesize==0, as it will still
                                                                                 // apply when hitting device max
                                                                                 // filesize limit
                ...
                if (max_filesize_restart) {
                    if (video_max_duration > 0) {
                        video_max_duration -= video_accumulated_time;
                        // this should be greater or equal to min_safe_restart_video_time, as too short
                        // remaining time should have been caught in restartVideo()
                        if (video_max_duration < min_safe_restart_video_time) {
                            if (MyDebug.LOG)
                ...
                boolean want_photo_video_recording = supportsPhotoVideoRecording()
                        && applicationInterface.usePhotoVideoRecording();
                ...
                try {
                    ...
                    // needed for emulator at least - although MediaRecorder not meant to work with
                    // emulator, it's good to fail gracefully
                    MyDebug.logStackTrace(TAG, "runtime exception starting video recorder", e);
                    this.video_recorder = local_video_recorder; // still assign, so failedToStartVideoRecorder() will
                                                                // release the video_recorder
                 ...
                 * }
                 * catch(RuntimeException e) {
                 * // needed for emulator at least - although MediaRecorder not meant to work
                 * with emulator, it's good to fail gracefully
                 * MyDebug.logStackTrace(TAG, "runtime exception starting video recorder", e);
                 * return false;
                 * }
                 * return true;
                 ...
                 * }
                 * // still assign even if success==false, so failedToStartVideoRecorder() will
                 * release the video_recorder
    ...
    private void videoRecordingStarted(boolean max_filesize_restart) {
        ...
        // Don't send intent for ACTION_MEDIA_SCANNER_SCAN_FILE yet - wait until
        // finished, so we get completed file.
        // Don't do any further calls after applicationInterface.startedVideo() that
        // might throw an error - instead video error
        // should be handled by including a call to stopVideo() (since the
        // video_recorder has started).
        ...
        if (applicationInterface.getVideoLowPowerCheckPref()) {
            /*
             * When a device shuts down due to power off, the application will receive
             * shutdown signals, and normally the video
             * should stop and be valid. However it can happen that the video ends up
             * corrupted (I've had people telling me this
             * can happen; Googling finds plenty of stories of this happening on Android
             * devices). I think the issue is that for
             * very large videos, a lot of time is spent processing during the
             * MediaRecorder.stop() call - if that doesn't complete
             * by the time the device switches off, the video may be corrupt.
             * So we add an extra safety net - devices typically turn off abou 1%, but we
             * stop video at 3% to be safe. The user
             * can try recording more videos after that if the want, but this reduces the
             * risk that really long videos are entirely
             * lost.
             */
            class BatteryCheckVideoTimerTask extends TimerTask {
                public void run() {
                    ...
                    // note that it's fine to call registerReceiver repeatedly - we pass a null
                    // receiver, so this is fine as a "one shot" use
                        ...
                        activity.runOnUiThread(new Runnable() {
                            public void run() {
                                ...
                                if (camera_controller != null && batteryCheckVideoTimerTask != null) {
                                    ...
                                    showToast(null, toast); // show the toast afterwards, as we're hogging the UI thread
                                                            // here, and media recorder takes time to stop
                                } else {
                                    if (MyDebug.LOG)
                                        Log.d(TAG,
            ...
            // Since we only first check after battery_check_interval_ms, this means users
            // will get some video recorded even if the battery is already too low.
            // But this is fine, as typically short videos won't be corrupted if the device
            // shuts off, and good to allow users to try to record a bit more if they want.
            batteryCheckVideoTimer.schedule(batteryCheckVideoTimerTask = new BatteryCheckVideoTimerTask(),
                    battery_check_interval_ms, battery_check_interval_ms);
        }
    }
     ...
     * This does nothing if isVideoRecording() returns false, or not on Android 7 or
     * higher.
     */
    ...
    /**
     * Take photo. The caller should already have set the phase to
     * PHASE_TAKING_PHOTO.
     */
        ...
        if (autofocus_in_continuous_mode) {
            ...
            synchronized (this) {
                ...
                if (focus_success == FOCUS_WAITING) {
                    ...
                    // when autofocus_in_continuous_mode==true, it means the user recently touched
                    // to focus in continuous focus mode, so don't do another focus
                    if (MyDebug.LOG)
            ...
            if (optimise_for_latency) {
                ...
                // we call via autoFocus(), to avoid risk of taking photo while the continuous
                // focus is focusing - risk of blurred photo, also sometimes get bug in such
                // situations where we end of repeatedly focusing
                // this is the case even if skip_autofocus is true (as we still can't guarantee
                // that continuous focusing might be occurring)
                // note: if the user touches to focus in continuous mode, we camera controller
                // may be in auto focus mode, so we should only enter this codepath if the
                // camera_controller is in continuous focus mode
            ...
            boolean wait_for_focus;
            // n.b., we check focus_value rather than camera_controller.supportsAutoFocus(),
            // as we want to discount focus_mode_locked
            synchronized (this) {
                if (focus_success == FOCUS_WAITING) {
                    // Needed to fix bug (on Nexus 6, old camera API): if flash was on, pointing at
                    // a dark scene, and we take photo when already autofocusing, the autofocus
                    // never returned so we got stuck!
                    // In general, probably a good idea to not redo a focus - just use the one
            ...
            if (wait_for_focus) {
                ...
                CameraController.AutoFocusCallback autoFocusCallback = new CameraController.AutoFocusCallback() {
                    ...
                    public void onAutoFocus(boolean success) {
                        ...
                        ensureFlashCorrect(); // need to call this in case user takes picture before startup focus
                                              // completes!
     ...
     * This is needed for a workaround for Camera2 bug (at least on Nexus 6) where
     * photos sometimes come out dark when using flash
     * auto, when the flash fires. This happens when taking a photo in autofocus
     * mode (including when continuous mode has
     * transitioned to autofocus mode due to touching to focus). Seems to happen
     * with scenes that have bright and dark regions,
     ...
     * applicationInterface.cameraInOperation(true, false) has
     * already been called.
     * Note that even if a caller wants to take a photo without focusing, you
     * probably want to call takePhoto() with skip_autofocus
    ...
    private void takePhotoWhenFocused(boolean continuous_fast_burst) {
        ...
        if (focus_value != null && focus_value.equals("focus_mode_locked") && focus_success == FOCUS_WAITING) {
            // make sure there isn't an autofocus in progress - can happen if in locked mode
            // we take a photo while autofocusing - see testTakePhotoLockedFocus() (although
            // that test doesn't always properly test the bug...)
            // we only cancel when in locked mode and if still focusing, as I had 2 bug
            // reports for v1.16 that the photo was being taken out of focus; both reports
            // said it worked fine in 1.15, and one confirmed that it was due to the
            // cancelAutoFocus() line, and that it's now fixed with this fix
            // they said this happened in every focus mode, including locked - so possible
            // that on some devices, cancelAutoFocus() actually pulls the camera out of
            // focus, or reverts to preview focus?
            cancelAutoFocus();
        }
        removePendingContinuousFocusReset(); // to avoid switching back to continuous focus mode while taking a photo -
                                             // instead we'll always make sure we switch back after taking a photo
        updateParametersFromLocation(); // do this now, not before, so we don't set location parameters during focus
                                        // (sometimes get RuntimeException)
        ...
        // if focus_set_for_target_distance==true, then we stick with the last set focus
        // bracketing source distance, as the current focus distance will
        // be set to the target
        if (applicationInterface.isFocusBracketingPref() && applicationInterface.isFocusBracketingSourceAutoPref()
                && !focus_set_for_target_distance) {
            camera_controller.setFocusBracketingSourceDistanceFromCurrent();
        }
        ...
        CameraController.PictureCallback pictureCallback = new CameraController.PictureCallback() {
            ...
            public void onStarted() {
                ...
                if (applicationInterface.getBurstForNoiseReduction()
                        && applicationInterface.getNRModePref() == ApplicationInterface.NRModePref.NRMODE_LOW_LIGHT) {
            ...
            public void onCompleted() {
                ...
                if (!using_android_l) {
                    // is_preview_started = false; // preview automatically stopped due to taking
                    // photo on original Camera API
                    preview_started_state = PREVIEW_NOT_STARTED; // preview automatically stopped due to taking photo on
                                                                 // original Camera API
                }
                phase = PHASE_NORMAL; // need to set this even if remaining repeat photos, so we can restart the
                                      // preview
                if (remaining_repeat_photos == -1 || remaining_repeat_photos > 0) {
                    ...
                    if (preview_started_state == PREVIEW_NOT_STARTED) {
                        ...
                        if (MyDebug.LOG)
                            Log.d(TAG, "repeat mode photos remaining: onPictureTaken about to start preview: "
                                    + remaining_repeat_photos);
                        startCameraPreview(true, null);
                        if (MyDebug.LOG)
                            Log.d(TAG, "repeat mode photos remaining: onPictureTaken started preview: "
                                    + remaining_repeat_photos);
                    }
                    applicationInterface.cameraInOperation(false, false);
                } else {
                    ...
                    if (pause_preview && success) {
                        ...
                        if (preview_started_state == PREVIEW_STARTED) {
                            ...
                            // also note: even though we now draw the last image on top of the screen
                            // instead of relying on the
                            // camera preview being paused, it's still good practice to pause the
                            // preview/camera for privacy reasons
                            if (camera_controller != null) {
                                camera_controller.stopPreview();
                            }
                ...
                continuousFocusReset(); // in case we took a photo after user had touched to focus (causing us to switch
                                        // from continuous to autofocus mode)
                if (camera_controller != null && focus_value != null
                ...
                if (camera_controller != null
                        && camera_controller.getBurstType() == CameraController.BurstType.BURSTTYPE_CONTINUOUS) {
                    if (MyDebug.LOG)
            ...
            /**
             * Ensures we get the same date for both JPEG and RAW; and that we set the date
             * ASAP so that it corresponds to actual
             * photo time.
             */
    ...
    private void takeRemainingRepeatPhotos() {
        ...
        if (remaining_repeat_photos == -1 || remaining_repeat_photos > 0) {
            if (camera_controller == null) {
                ...
                // just in case?
                Log.e(TAG,
                        "remaining_repeat_photos still set, but preview is still opening!: " + remaining_repeat_photos);
                cancelRepeat();
            } else {
                // check it's okay to take a photo
                if (!applicationInterface.canTakeNewPhoto()) {
                ...
                if (timer_delay == 0) {
                    // we set skip_autofocus to go straight to taking a photo rather than
    ...
    private void tryAutoFocus(final boolean startup, final boolean manual) {
        // manual: whether user has requested autofocus (e.g., by touching screen, or
        // volume focus, or hardware focus button)
        // consider whether you want to call requestAutoFocus() instead (which properly
        // cancels any in-progress auto-focus first)
        if (MyDebug.LOG) {
        ...
        // else if( !this.is_preview_started ) {
            ...
            // testTakeVideoBitrate() on Nexus 7 at 30Mbs or 50Mbs, where the startup
            // autofocus would cause a problem here
            if (MyDebug.LOG)
                Log.d(TAG, "currently taking a photo");
        } else {
            if (manual) {
            ...
            if (manual && !is_video && camera_controller.focusIsContinuous()
            ...
            // it's only worth doing autofocus when autofocus has an effect (i.e., auto or
            // macro mode)
            // but also for continuous focus mode, triggering an autofocus is still
            // important to fire flash when touching the screen
            if (camera_controller.supportsAutoFocus()) {
                ...
                if (!using_android_l) {
                    ...
                    String old_flash_value = camera_controller.getFlashValue();
                    // getFlashValue() may return "" if flash not supported!
                    if (startup && !old_flash_value.isEmpty() && !old_flash_value.equals("flash_off")
                            && !old_flash_value.equals("flash_torch")) {
                        set_flash_value_after_autofocus = old_flash_value;
                        camera_controller.setFlashValue("flash_off");
                    }
                ...
                // do this so we get the focus box, for focus modes that support focus area, but
                // don't support autofocus
                focus_success = FOCUS_SUCCESS;
                focus_complete_time = System.currentTimeMillis();
                // n.b., don't set focus_started_time as that may be used for application to
                // show autofocus animation
            }
        }
    }

    /**
     * If the user touches the screen in continuous focus mode, we switch the
     * camera_controller to autofocus mode.
     * After the autofocus completes, we set a reset_continuous_focus_runnable to
     * switch back to the camera_controller
     ...
     * If the user touches the screen in continuous focus mode, we switch the
     * camera_controller to autofocus mode.
     * This function is called to see if we should switch from autofocus mode back
     * to continuous focus mode.
    ...
    private void continuousFocusReset() {
        ...
        if (camera_controller != null && autofocus_in_continuous_mode) {
            ...
            // check again
            String current_ui_focus_value = getCurrentFocusValue();
            if (current_ui_focus_value != null && !camera_controller.getFocusValue().equals(current_ui_focus_value)
    ...
    private void ensureFlashCorrect() {
        // ensures flash is in correct mode, in case where we had to turn flash
        // temporarily off for startup autofocus
    ...
    private void autoFocusCompleted(boolean manual, boolean success, boolean cancelled) {
        ...
        if (this.using_face_detection && !cancelled) {
            // On some devices such as mtk6589, face detection does not resume as written in
    ...
    public void startCameraPreview(boolean wait_until_started, Runnable preview_opened) {
        ...
        if (camera_controller != null && !this.isTakingPhotoOrOnTimer()
            ...
            try {
                ...
                camera_controller.startPreview(wait_until_started, new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (camera_controller == null) {
                            // Although at CameraController2 we have some checks for the camera closing in
                            // the meantime, this isn't
                            // robust as if wait_until_started==false, both starting the preview and closing
                            // the camera may be running
                            // on background threads.
                            // However closeCamera() will set camera_controller to null on the UI thread
                    ...
                    public void run() {
                        ...
                        // if we update this code, remember to also update the CameraControllerException
                        // code below
                        preview_started_state = PREVIEW_NOT_STARTED;
                        applicationInterface.onFailedStartPreview();
                        if (preview_opened != null) {
                            // unclear if we need to run the caller's runnable on failure, but do so for
                            // consistency (either
                            // with wait_until_started==true, or when a CameraControllerException was thrown
                            // instead)
                            preview_opened.run();
                        }
                    }
            ...
            if (MyDebug.LOG) {
                Log.d(TAG, "startCameraPreview: time after starting camera preview: "
                        + (System.currentTimeMillis() - debug_time));
            }
        } else {
        ...
        if (MyDebug.LOG) {
            Log.d(TAG, "startCameraPreview: total time for startCameraPreview: "
                    + (System.currentTimeMillis() - debug_time));
        }
    }
    ...
    private void setPreviewPaused(boolean paused) {
        ...
        if (paused) {
            ...
            // shouldn't call applicationInterface.cameraInOperation(true, ...), as should
            // already have done when we started to take a photo (or above when exiting
            // immersive mode)
        } else {
            this.phase = PHASE_NORMAL;
            /*
             ...
             * applicationInterface.cameraInOperation(false, true);
             */
            // Need to call camerainOperation for when taking photo with pause preview
            // option;
            // also needed so that the GUI is set up correctly (via MainUI.showGUI()), for
            // things like on-screen icons that are
    ...
    public void onAccelerometerSensorChanged(SensorEvent event) {
        /*
         * if( MyDebug.LOG )
         * Log.d(TAG, "onAccelerometerSensorChanged: " + event.values[0] + ", " +
    ...
    /**
     * This method should be called when the natural level angle, or the calibration
     * angle, has been updated, to update the other level angle variables.
     *
     */
    ...
    /*
     * Returns true if we have the level angle ("roll"), but the pitch is not near
     * vertically up or down (70 degrees to level).
     * This is useful as the level angle becomes unstable when device is near
     * vertical
     */
    public boolean hasLevelAngleStable() {
        if (!is_test && has_pitch_angle && Math.abs(pitch_angle) > 70.0) {
            // note that if is_test, we always set the level angle - since the device
            // typically lies face down when running tests...
            return false;
        }
        return this.has_level_angle;
    }
    ...
    private void calculateGeoDirection() {
        ...
        SensorManager.remapCoordinateSystem(this.deviceRotation, SensorManager.AXIS_X, SensorManager.AXIS_Z,
                this.cameraRotation);
         ...
         * if( MyDebug.LOG ) {
         * Log.d(TAG, "###");
         * Log.d(TAG, "old geo_direction: " + (geo_direction[0]*180/Math.PI) + ", " +
         * (geo_direction[1]*180/Math.PI) + ", " + (geo_direction[2]*180/Math.PI));
         * }
         */
        for (int i = 0; i < 3; i++) {
        ...
        /*
         * if( MyDebug.LOG ) {
         * Log.d(TAG, "new_geo_direction: " + (new_geo_direction[0]*180/Math.PI) + ", "
         * + (new_geo_direction[1]*180/Math.PI) + ", " +
         * (new_geo_direction[2]*180/Math.PI));
         * Log.d(TAG, "geo_direction: " + (geo_direction[0]*180/Math.PI) + ", " +
         * (geo_direction[1]*180/Math.PI) + ", " + (geo_direction[2]*180/Math.PI));
         * }
         */
    }
    ...
    /**
     * Returns true iff we're in video mode, and a high speed fps video mode is
     * selected.
     */
    ...
    /**
     * Whether manual white balance temperatures can be specified via
    ...
    /**
     * Returns whether a range of manual ISO values can be set. If this returns
     * true, use
     * getMinimumISO() and getMaximumISO() to return the valid range of values. If
     * this returns
    ...
    /**
     * If supportsISORange() returns false, use this method to return a list of
     * supported ISO values:
     * - If this is null, then manual ISO isn't supported.
     * - If non-null, this will include "auto" to indicate auto-ISO, and one or more
     * numerical ISO
     * values.
     * If supportsISORange() returns true, then this method should not be used (and
     * it will return
    ...
    public long getMaximumExposureTime() {
        ...
        if (applicationInterface.isExpoBracketingPref() || applicationInterface.isFocusBracketingPref()
                || applicationInterface.isCameraBurstPref()) {
    ...
    public boolean supportsCameraExtension(int extension) {
        if (extension == CameraExtensionCharacteristics.EXTENSION_HDR) {
            // blocked for now, as have yet to be able to test this (seems to have no effect
            // on Galaxy S10e;
    ...
    public float getViewAngleX(boolean for_preview) {
        ...
        if (Math.abs(actual_aspect_ratio - view_aspect_ratio) < 1.0e-5f) {
            ...
            float actual_view_angle_x = (float) Math
                    .toDegrees(2.0 * Math.atan(aspect_ratio_scale * Math.tan(Math.toRadians(view_angle_x) / 2.0)));
            /*
             * if( MyDebug.LOG )
    ...
    public float getViewAngleY(boolean for_preview) {
        ...
        if (Math.abs(actual_aspect_ratio - view_aspect_ratio) < 1.0e-5f) {
            ...
            float actual_view_angle_y = (float) Math
                    .toDegrees(2.0 * Math.atan(aspect_ratio_scale * Math.tan(Math.toRadians(view_angle_y) / 2.0)));
            /*
             * if( MyDebug.LOG )
    ...
    /**
     * @param check_supported If true, and a burst mode is in use (fast burst, expo,
     *                        HDR), or
     *                        a camera vendor extension mode, and/o a constraint was
     *                        set via
     *                        getCameraResolutionPref(), then the returned list will
     *                        be filtered to
     *                        remove sizes that don't support burst and/or these
     *                        constraints.
     */
     ...
     * getVideoQualityHander().getSupportedVideoQuality(), allows filtering to the
     * supplied
     * fps_value.
     * 
     * @param fps_value If not "default", the returned video qualities will be
     *                  filtered to those that supported the requested
     *                  frame rate.
     */
        ...
        if (!fps_value.equals("default") && supports_video_high_speed) {
            try {
                ...
                for (String quality : video_quality_handler.getSupportedVideoQuality()) {
                    ...
                    CameraController.Size best_video_size = video_quality_handler
                            .findVideoSizeForFrameRate(profile.videoFrameWidth, profile.videoFrameHeight, fps, false);
                    if (best_video_size != null) {
                        if (MyDebug.LOG)
    ...
    /**
     * Returns whether the user's fps preference is both non-default, and is
     * considered a
     * "high-speed" frame rate, but not a normal frame rate. (Note, we go by the
     * supplied
     * fps_value, and not what the user's preference necessarily is; so this doesn't
     * say whether
    ...
    public boolean fpsIsHighSpeed(String fps_value) {
        ...
        if (!fps_value.equals("default") && supports_video_high_speed) {
            try {
                ...
                if (MyDebug.LOG)
                    Log.d(TAG, "fps: " + fps);
                // need to check both, e.g., 30fps on Nokia 8 is in fps ranges of both normal
                // and high speed video sizes
    ...
    public void onResume() {
        ...
        if (camera_open_state == CameraOpenState.CAMERAOPENSTATE_CLOSING) {
            // when pausing, we close the camera on a background thread - so if this is
            // still happening when we resume,
    ...
    /**
     * Call when activity is paused, or the application wants to put the Preview
     * into a paused
     * state (closing the camera etc).
     * 
     * @param activity_is_pausing Set to true if this is called because the activity
     *                            is being paused;
    ...
    public void onPause(boolean activity_is_pausing) {
        ...
        if (activity_is_pausing)
            this.app_is_paused = true; // note, if activity_is_paused==false, we don't change app_is_paused, in case
                                       // app was paused indicated via a separate call to onPause
        if (camera_open_state == CameraOpenState.CAMERAOPENSTATE_OPENING) {
            if (MyDebug.LOG)
    ...
    public void onDestroy() {
        ...
        if (refreshPreviewBitmapTaskIsRunning()) {
            // if we're being destroyed, better to wait until completion rather than just
            // cancelling
            try {
        ...
        if (camera_open_state == CameraOpenState.CAMERAOPENSTATE_CLOSING) {
            // If the camera is currently closing on a background thread, then wait until
            // the camera has closed to be safe
            if (MyDebug.LOG) {
            ...
            if (close_camera_task != null) { // just to be safe
                ...
                try {
                    close_camera_task.get(3000, TimeUnit.MILLISECONDS); // set timeout to avoid ANR (camera resource
                                                                        // should be freed by the OS when destroyed
                                                                        // anyway)
                ...
                if (MyDebug.LOG) {
                    ...
                    Log.d(TAG,
                            "### time after waiting for close_camera_task: " + (System.currentTimeMillis() - time_s));
                }
            } else {
                Log.e(TAG, "onResume: state is CAMERAOPENSTATE_CLOSING, but close_camera_task is null");
            }
     ...
     * // we cache the preference_ui_placement to save having to check it in the
     * draw() method
     * SharedPreferences sharedPreferences =
     * PreferenceManager.getDefaultSharedPreferences(this.getContext());
     * String ui_placement =
     * sharedPreferences.getString(MainActivity.getUIPlacementPreferenceKey(),
     * "ui_right");
     ...
     * 
     * @param called_from_handler Should be false, unless called from the
     *                            fake_toast_handler.
     */
    private void clearActiveFakeToast(boolean called_from_handler) {
        if (!called_from_handler) {
            // important to remove the callback, otherwise when it runs, it may end up
            // deleting a
            // new fake toast that is created after this method call, but before the
            // callback runs
    ...
    /*
     * public void showToast(final ToastBoxer clear_toast, final String message,
     * final boolean use_fake_toast, int duration) {
     * showToast(clear_toast, message, 32, use_fake_toast, duration);
     * }
     */

    /*
     * public void showToast(final String message, final int offset_y_dp, final
     * boolean use_fake_toast) {
    ...
    public void showToast(final ToastBoxer clear_toast, final String message, final int offset_y_dp,
            final boolean use_fake_toast) {
        showToast(clear_toast, message, offset_y_dp, use_fake_toast, false);
    }

    /**
     * Displays a "toast", but has several advantages over calling Android's Toast
     * API directly.
     * We use a custom view, to rotate the toast to account for the device
     * orientation (since
     * Open Camera always runs in landscape).
     * 
     * @param clear_toast    Only relevant if use_fake_toast is false. If non-null,
     *                       calls to this method
     *                       with the same clear_toast value will overwrite the
     *                       previous ones rather than
     *                       being queued. Note that toasts no longer seem to be
     *                       queued anyway on
     *                       Android 9+.
     *                       (N.B., some callers with use_fake_toast==true still
     *                       supply a use_fake_toast
     *                       for historical reasons, from when previously those
     *                       calls weren't using a fake
     *                       toast.)
     * @param message        The message to display.
     * @param offset_y_dp    The y-offset from the centre of the screen. Only
     *                       relevant if use_fake_toast is
     *                       true.
     * @param use_fake_toast If true, don't use Android's Toast system at all, and
     *                       instead display a message
     *                       on the Preview.
     *                       This is due to problems on Android 9+ where rapidly
     *                       displaying toasts (e.g., to
     *                       display values from a seekbar being modified) cause
     *                       problems where toast sometimes
     *                       disappear (this happens whether using clear_toast or
     *                       not). Note that using
     *                       use_fake_toast means that the toasts don't have the
     *                       fade out effect.
     *                       Update: Toasts with custom views (Toast.setView()) are
     *                       now deprecated. So
     *                       use_fake_toast==false no longer uses a custom view. So
     *                       we should now only set
     *                       use_fake_toast==false for when we really want to use
     *                       the system toast (e.g.,
     *                       anything that isn't when the Preview is showing such as
     *                       from Settings, or when
     *                       we want the Android toast look such as for an error
     *                       message).
     *                       Usages where we want to display info on the Preview
     *                       should always set
     *                       use_fake_toast==true for a consistent look.
     * @param dont_clear     If true, then the toast will remain until explicitly
     *                       cleared via
     *                       clearActiveFakeToast(). Only supported if
     *                       use_fake_toast==true.
     */
    public void showToast(final ToastBoxer clear_toast, final String message, final int offset_y_dp,
            final boolean use_fake_toast, boolean dont_clear) {
        ...
        if (this.app_is_paused && use_fake_toast) {
            ...
            // When targeting Android 11+, toasts with custom views won't be shown in
            // background anyway - in theory we
            // shouldn't be making toasts when in background, but check just in case.
            // However we no longer use custom views when use_fake_toast==false, so fine to
            // allow those - and indeed this
            // is useful for cases where the toast is created shortly before Open Camera
            // resumes, e.g., cancelling SAF
            // (see toast in MainActivity.onActivityResult()), or denying location
            // permission (see toast from
            // PermissionHandler.onRequestPermissionsResult()).
            return;
        }

        final Activity activity = (Activity) this.getContext();
        // We get a crash on emulator at least if Toast constructor isn't run on main
        // thread (e.g., the toast for taking a photo when on timer).
        // Also see
        // http://stackoverflow.com/questions/13267239/toast-from-a-non-ui-thread
        // Also for the use_fake_toast code, running the creation code, and the
        // postDelayed code (and the code in clearActiveFakeToast()), on the UI thread
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                ...
                if (use_fake_toast) {
                    ...
                    // in theory the fake_toast_handler should only have a callback on it if
                    // re-using an existing fake toast,
                ...
                // clear_toast) never seems to clear if we repeatedly issue new toasts - this
                // doesn't happen if we reuse existing toasts if possible
                // However should only do this if the previous toast was the most recent toast
                // (to avoid messing up ordering)
                Toast toast;
                long time_now = System.currentTimeMillis();
                /*
                 * // We recreate a toast every 2s, to workaround Android toast bug that calling
                 * show() no longer seems to extend the toast duration!
                 * // (E.g., see bug where toasts for sliders disappear after a while if
                 * continually moving the slider.)
                 * if( clear_toast != null && clear_toast.toast != null && clear_toast.toast ==
                 * last_toast && time_now < last_toast_time_ms+2000) {
                 * if( MyDebug.LOG )
                 * Log.d(TAG, "reuse last toast: " + last_toast);
                 * toast = clear_toast.toast;
                 * // for performance, important to reuse the same view, instead of creating a
                 * new one (otherwise we get jerky preview update e.g. for changing manual focus
                 * slider)
                ...
                if (!((Activity) getContext()).isFinishing()) {
                    // Workaround for crash due to bug in Android 7.1 when activity is closing
                    // whilst toast shows.
                    // This was fixed in Android 8, but still good to fix the crash on Android 7.1!
                    // See
    ...
    /**
     * If geotagging is enabled, pass the location info to the camera controller
     * (for photos).
     */
    ...
    /**
     * Runs the supplied runnable, but waits until the refreshPreviewBitmapTask is
     * no longer running.
     */
    ...
    /*
     * Recycles the supplied bitmap, but if the refreshPreviewBitmapTask is running,
     * waits until
     * it isn't running.
     */
    ...
    private void freePreviewBitmap() {
        ...
        if (preview_bitmap != null) {
            ...
            // It's okay to set preview_bitmap to null even if refreshPreviewBitmapTask is
            // currently running in the background
            // as it takes it's own reference. But we shouldn't recycle until the background
            // thread is complete.
            preview_bitmap = null;
        }

        // It's okay to set these to -1 even if refreshPreviewBitmapTask is currently
        // running in the background
        // as it takes it's own reference. But we shouldn't recycle until the background
        // thread is complete.
        preview_bitmap_full_w = -1;
        preview_bitmap_full_h = -1;
        ring_buffer.flush(); // even if we're recreating the preview_bitmap_full, it might be at a different
                             // resolution, so safest to flush the previous pre-shots
        if (use_preview_bitmap_full) {
            runForPreviewTask(new Runnable() {
                @Override
                public void run() {
                    ring_buffer.flush(); // important to flush again, in case the refreshPreviewBitmapTask already
                                         // running in the background added a new image
                }
            });
        }
    ...
    private void recreatePreviewBitmap() {
        ...
        // Note we need to take into account getDisplayRotationDegrees(), as
        // TextureView.getBitmap()
        // returns the texture in the "natural" orientation of the device - it doesn't
        // take the transform
        ...
        if (want_preview_bitmap && use_preview_bitmap_small) {
            ...
            try {
                ...
                // Note if we failed to create the preview_bitmap, we don't call
                // disablePreviewBitmap() or set want_preview_bitmap to false,
    ...
    private void createZebraStripesBitmap() {
        ...
        if (want_zebra_stripes && preview_bitmap != null) {
            try {
                 ...
                 * throw new IllegalArgumentException(); // test
                 */
                zebra_stripes_bitmap_buffer = Bitmap.createBitmap(preview_bitmap.getWidth(), preview_bitmap.getHeight(),
                        Bitmap.Config.ARGB_8888);
                // zebra_stripes_bitmap itself is created dynamically when generating the zebra
                // stripes
            } catch (IllegalArgumentException e) {
    ...
    private void createFocusPeakingBitmap() {
        ...
        if (want_focus_peaking & preview_bitmap != null) {
            try {
                 ...
                 * throw new IllegalArgumentException(); // test
                 */
                focus_peaking_bitmap_buffer = Bitmap.createBitmap(preview_bitmap.getWidth(), preview_bitmap.getHeight(),
                        Bitmap.Config.ARGB_8888);
                focus_peaking_bitmap_buffer_temp = Bitmap.createBitmap(preview_bitmap.getWidth(),
    ...
    public void enableZebraStripes(int zebra_stripes_threshold, int zebra_stripes_color_foreground,
            int zebra_stripes_color_background) {
    ...
    // use static class, and WeakReferences, to avoid memory leaks:
    // https://stackoverflow.com/questions/44309241/warning-this-asynctask-class-should-be-static-or-leaks-might-occur/46166223
    private static class RefreshPreviewBitmapTask extends AsyncTask<Void, Void, RefreshPreviewBitmapTaskResult> {
        ...
        // we take references to the bitmaps, so the Preview class can set this to null
        // even whilst the background thread is running
        ...
        RefreshPreviewBitmapTask(Preview preview, boolean update_histogram, boolean update_preshot,
            ...
            this.focus_peaking_bitmap_buffer_tempReference = new WeakReference<>(
                    preview.focus_peaking_bitmap_buffer_temp);
        ...
        protected RefreshPreviewBitmapTaskResult doInBackground(Void... voids) {
            ...
            try {
                ...
                if (preview_bitmap_full_w != -1 && preview_bitmap_full_h != -1 && update_preshot) {
                    // much faster to create a fresh preview_bitmap_full to read into, instead of
                    // copying it after
                    try {
                        if (MyDebug.LOG)
                            Log.d(TAG, "time before creating preview_bitmap_full_copy: "
                                    + (System.currentTimeMillis() - debug_time));
                        result.preview_bitmap_full_copy = Bitmap.createBitmap(preview_bitmap_full_w,
                                preview_bitmap_full_h, Bitmap.Config.ARGB_8888);
                        if (MyDebug.LOG)
                            Log.d(TAG, "time after creating preview_bitmap_full_copy: "
                                    + (System.currentTimeMillis() - debug_time));
                        textureView.getBitmap(result.preview_bitmap_full_copy);
                        if (MyDebug.LOG)
                            Log.d(TAG, "time after getBitmap for preview_bitmap_full_copy: "
                                    + (System.currentTimeMillis() - debug_time));
                        // See comments below for zebra stripes for why we need to rotate
                        // But since rotating is slower (and presumably more CPU intensive) than taking
                        // a copy, we leave this to the ImageSaver thread -
                        // although that'll also be just as slow, better to only do it when we're
                        // actually saving pre-shots,
                        // rather than having this run all the time.
                        // Also see above - it's much faster to create a new bitmap to read into, than
                        // to copy a bitmap
                    } catch (IllegalArgumentException e) {
                ...
                if (update_histogram && preview_bitmap != null) {
...

                    {
                        ...
                        JavaImageFunctions.ComputeHistogramApplyFunction function = new JavaImageFunctions.ComputeHistogramApplyFunction(
                                java_type);
                        JavaImageProcessing.applyFunction(function, preview_bitmap, null, 0, 0,
                                preview_bitmap.getWidth(), preview_bitmap.getHeight());
                ...
                if (preview.want_zebra_stripes && preview_bitmap != null && zebra_stripes_bitmap_buffer != null) {
                    ...
                    JavaImageFunctions.ZebraStripesApplyFunction function = new JavaImageFunctions.ZebraStripesApplyFunction(
                            preview.zebra_stripes_threshold, preview.zebra_stripes_color_foreground,
                            preview.zebra_stripes_color_background, zebra_stripes_width);
                    JavaImageProcessing.applyFunction(function, preview_bitmap, zebra_stripes_bitmap_buffer, 0, 0,
                            preview_bitmap.getWidth(), preview_bitmap.getHeight());

                    // The original orientation of the bitmap we get from textureView.getBitmap()
                    // needs to be rotated to
                    // account for the orientation of camera vs device, but not to account for the
                    // current orientation
                    // of the device.
                    // This is because TextureView.getBitmap() returns the texture in the "natural"
                    // orientation of the device - it doesn't take the transform
                    // we've applied in configureTransform() into account.
                    int rotation_degrees = preview.getDisplayRotationDegrees(false);
                    /*
                     * if( MyDebug.LOG ) {
                     * Log.d(TAG, "orientation of display relative to natural orientation: " +
                     * rotation_degrees);
                     * }
                     */
                    if (MyDebug.LOG)
                        Log.d(TAG, "time before creating new_zebra_stripes_bitmap: "
                                + (System.currentTimeMillis() - debug_time));
                    Matrix matrix = new Matrix();
                    matrix.postRotate(-rotation_degrees);
                    result.new_zebra_stripes_bitmap = Bitmap.createBitmap(zebra_stripes_bitmap_buffer, 0, 0,
                            zebra_stripes_bitmap_buffer.getWidth(), zebra_stripes_bitmap_buffer.getHeight(), matrix,
                            false);

                    if (MyDebug.LOG)
                        Log.d(TAG, "time after creating new_zebra_stripes_bitmap: "
                                + (System.currentTimeMillis() - debug_time));
                     ...
                     * DIRECTORY_DCIM) + "/zebra_stripes_bitmap_buffer.jpg");
                     * File file = new
                     * File(Environment.getExternalStoragePublicDirectory(Environment.
                     * DIRECTORY_DCIM) + "/zebra_stripes_bitmap.jpg");
                     * try {
                     * OutputStream outputStream = new FileOutputStream(file);
                     * //zebra_stripes_bitmap_buffer.compress(Bitmap.CompressFormat.JPEG, 90,
                     * outputStream);
                     * preview.zebra_stripes_bitmap.compress(Bitmap.CompressFormat.JPEG, 90,
                     * outputStream);
                     * outputStream.close();
                ...
                if (preview.want_focus_peaking && preview_bitmap != null && focus_peaking_bitmap_buffer != null
                        && focus_peaking_bitmap_buffer_temp != null) {
                    if (MyDebug.LOG)
                    ...
                    JavaImageFunctions.FocusPeakingApplyFunction function = new JavaImageFunctions.FocusPeakingApplyFunction(
                            preview_bitmap);
                    JavaImageProcessing.applyFunction(function, preview_bitmap, focus_peaking_bitmap_buffer_temp, 0, 0,
                            preview_bitmap.getWidth(), preview_bitmap.getHeight());

                    JavaImageFunctions.FocusPeakingFilteredApplyFunction function_filtered = new JavaImageFunctions.FocusPeakingFilteredApplyFunction(
                            focus_peaking_bitmap_buffer_temp);
                    JavaImageProcessing.applyFunction(function_filtered, focus_peaking_bitmap_buffer_temp,
                            focus_peaking_bitmap_buffer, 0, 0, preview_bitmap.getWidth(), preview_bitmap.getHeight());

                    // See comments above for zebra stripes
                    int rotation_degrees = preview.getDisplayRotationDegrees(false);
                    if (MyDebug.LOG)
                        Log.d(TAG, "time before creating new_focus_peaking_bitmap: "
                                + (System.currentTimeMillis() - debug_time));
                    Matrix matrix = new Matrix();
                    matrix.postRotate(-rotation_degrees);
                    result.new_focus_peaking_bitmap = Bitmap.createBitmap(focus_peaking_bitmap_buffer, 0, 0,
                            focus_peaking_bitmap_buffer.getWidth(), focus_peaking_bitmap_buffer.getHeight(), matrix,
                            false);
                    if (MyDebug.LOG)
                        Log.d(TAG, "time after creating new_focus_peaking_bitmap: "
                                + (System.currentTimeMillis() - debug_time));

                    if (MyDebug.LOG) {
                        Log.d(TAG,
                                "time for focus peaking: " + (System.currentTimeMillis() - debug_time_focus_peaking));
                    }
                }
            ...
            if (MyDebug.LOG) {
                Log.d(TAG,
                        "time taken for RefreshPreviewBitmapTaskResult: " + (System.currentTimeMillis() - debug_time));
            }
            return result;
        }
        ...
        protected void onPostExecute(RefreshPreviewBitmapTaskResult result) {
            ...
            if (preview.want_pre_shots && result.preview_bitmap_full_copy != null) {
                if (preview.isTakingPhoto()) {
                    // don't add pre-shots once already taking a photo (otherwise we may have
                    // pre-shots after the photo was taken)
    ...
    private void refreshPreviewBitmap() {
        ...
        // if want_pre_shots==true, this should take priority over other options as it
        // affects the interval between the pre-shots
        // but the value shouldn't be too long, as then zebra stripes or focus peaking
        // (if they are enabled) would be too jerky
        final long refresh_time = want_pre_shots ? preshot_interval_ms
                : (want_zebra_stripes || want_focus_peaking) ? 83 : refresh_histogram_rate_ms;
        long time_now = System.currentTimeMillis();
        if (want_preview_bitmap &&
                ((use_preview_bitmap_small && preview_bitmap != null)
                        || (use_preview_bitmap_full && preview_bitmap_full_w != -1 && preview_bitmap_full_h != -1))
            ...
            // even if we're running the background task at a faster rate (due to zebra
            // stripes etc), we still update the histogram
            ...
            refreshPreviewBitmapTask = new RefreshPreviewBitmapTask(this, update_histogram, update_preshot,
                    preview_bitmap_full_w, preview_bitmap_full_h);
    ...
    private void cancelRefreshPreviewBitmap() {
        ...
        if (refreshPreviewBitmapTaskIsRunning()) {
            ...
            // we don't set refreshPreviewBitmapTask to null - this will be done by the task
            // itself when it completes;
            // and we want to know when the task is no longer running (e.g., for
            // freePreviewBitmap()).
        }
    }
     ...
     * In case of restarting due to max filesize (whether on Android 8+ or not),
     * this includes the
    ...
    /**
     * Returns the frame rate that the preview's surface or canvas view should be
     * updated.
     */
    public long getFrameRate() {
        /*
         * See
         * https://stackoverflow.com/questions/44594711/slow-rendering-when-updating-
         * textview ,
         * https://stackoverflow.com/questions/44233870/how-to-fix-slow-rendering-
         * android-vitals -
         * there is evidence that using an infrequent update actually results in poorer
         * performance,
         * due to devices running in a lower power state, but Google Play analytics do
         * not take this
         * into consideration. Thus we are forced to request updates at 60fps whether we
         * need them
         * or not. I can reproducing this giving improved performance on OnePlus 3T for
         * old and
         * Camera2 API. Testing suggests this does not seem to adversely affect battery
         * life.
         * This is limited to Android 7+, to avoid causing problems on older devices
         * (which don't
         * contribute to Google Analytics anyway).
         * If we ever are able to use lower frame rates in future, remember we'll still
         * need a high
         * frame rate when applying the dimming effect when reopening or updating the
         * camera (see
         * DrawPreview.setDimPreview()) (especially for MainActivity.updateForSettings()
         * when we
         * pause/unpause the preview instead of reopening the camera).
         * Update: On more recent Android versions, this effect no longer seems to
         * happen, and on
         * Android 13 (at least Pixel 6 Pro), we see the reverse (but more reasonable)
         * behaviour
         * where we have fewer janky frames with a longer frame rate. Behaviour is much
         * better at
         * 32ms compared to 16ms; and we shouldn't go any slower (firstly so that UI
         * still runs
    ...
    public Pair<Integer, Integer> getFocusPos() {
        // note, we don't store the screen coordinates, as they may become out of date
        // in the
    ...
    /**
     * Note this means the state of being paused after taking a photo (when pause
     * preview option is
     * enabled). Callers wanting to know if the camera preview is started or not
     * should use
     * isPreviewStarted().
     */
    ...
    /**
     * If non-null, this returned array will stored the currently detected faces (if
     * face recognition
     * is enabled). The face.temp rect will store the face rectangle in screen
     * coordinates.
     */
    public CameraController.Face[] getFacesDetected() {
        ...
        // FindBugs warns about returning the array directly, but in fact we need to
        // return direct access rather than copying, so that the on-screen display of
        // faces rectangles updates
        return this.faces_detected;
    }

    /**
     * Returns the current zoom factor of the camera. Always returns 1.0f if zoom
     * isn't supported.
     */
